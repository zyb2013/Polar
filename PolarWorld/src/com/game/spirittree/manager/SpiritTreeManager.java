			...
			if (spiritTree != null) {
				...
				if (fruit != null ) {
					if (fruit.getType() == 1 || fruit.getType() == 2) {
						...
						if (kiwibean != null) {
//							计算公式 = （1 - 已成长时间/所有状态秒数相加之和）* 所需钻石
//							计算结果予以取整，最低为1钻石；
//							其中”所需钻石“填写于”神树组包表“中的”手动催熟所需钻石“字段；
	...
	public void stReqGuildFruitOperatingToWorldMessage(ReqGuildFruitOperatingToWorldMessage msg) {
		...
		if (player != null) {
			...
			if (membermap.containsKey(msg.getHostid()) == false) {
				/*xiaozhuoming: 暂时没有用到
				MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("对方已经离开您所在战盟，不能对他的果实进行操作。"));
				*/
...
//							Q_spirittree_packetBean pacdata = getpacketdata().get(fruit.getGroupidlist().get(0));
	...
	public void clearupSpiritdata(){
		...
		while (it.hasNext()) {
			...
			if(ManagerPool.guildWorldManager.getGuildByUserId(entry.getKey()) == null){//没有战盟
