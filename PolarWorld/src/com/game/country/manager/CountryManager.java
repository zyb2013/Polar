		...
		if ( player != null) {
			...
			if (guild != null) {
				for (MemberInfo member : guild.getMemberinfoHashMap().values()) {
					if (member.getGuildPowerLevel() < 4 && member.getGuildPowerLevel() > 0) {
			...
			countryStructureInfo.setHoldDay(msg.getDay());
			countryStructureInfo.setHoldReward(msg.getReward());
