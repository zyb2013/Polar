...
public class YbcardManager {
	...
	//数据库连接
	private YbCardDao ybcardDao = new YbCardDao();	//玩家公测钻石储存
	private UserDao userDao=new UserDao();			//玩家帐号
	private ActivitySetDao actDao = new ActivitySetDao();	//公测钻石卡获得常量
	...
	private byte YBCRAD_TYPE = 1;	//1封测期间（抽奖），2公测期间(领取钻石)
	private int YB_DAY_MAX = 500000000;	//每日最大钻石数量
	private int YB_DAY_RND = 18000;	//每日随机钻石停止数量
	...
	public void stReqYBCardToWorldMessage(ReqYBCardToWorldMessage msg) throws SQLException {
		...
		if (player != null) {
			...
			if (day != activitySetBean.getDay_value() ) {	//天数不同，进行钻石清零
			...
			if (msg.getType() == 0 || msg.getType() == 3) { 
				...
				for (int i = 0; i < msg.getNum(); i++) {
					if (dataybnum < YB_DAY_RND) {
						...
						if(ybcardmap.containsKey(tab[idx][0]) &&  minute >= ybcardmap.get(tab[idx][0])){
							...
							ybsum += tab[idx][0];	//钻石数量
							dataybnum += tab[idx][0];	//钻石数量
							if(tab[idx][0] > 5){
								ybms.add(tab[idx][0]);
							}
						}else{
				...
				if (isinsert) {
					...
					ybCardBean.setYuanbao(ybsum);		//加钻石
					
					if(ybcardDao.insert(ybCardBean)== 0){
						log.error("钻石insert失败="+ybsum);
						stYbcaedLog(yblog,0);
						status = 0;
					}else {
						cmsg.setYuanbaonum(ybsum);
						MessageUtil.tell_player_message(player, cmsg);
						MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM,ResManager.getInstance().getString("使用{1}公测钻石卡，获得{2}公测绑钻"),msg.getNum()+"",ybsum+"");
						status = 1;
					}
					
				}else {
					ybCardBean.setYuanbao(ybCardBean.getYuanbao()+ ybsum);	//加钻石
					if(ybcardDao.update(ybCardBean)== 0){
						log.error(player.getId()+":钻石update失败="+ybsum);
						status = 0;
					}else {
						cmsg.setYuanbaonum(ybCardBean.getYuanbao());
						MessageUtil.tell_player_message(player, cmsg);
				...
				if (status == 1 && ybms.size() > 0) {
					for (int i = 0; i < ybms.size(); i++) {
						MessageUtil.notify_All_player(Notifys.CUTOUT,ResManager.getInstance().getString("恭喜『{1}』使用公测钻石卡获得了 {2}公测绑钻"),player.getName(),ybms.get(i)+"");
					}
				}
...
//				int sum = dataybnum + ybsum;
			...
			//-------------------------------------------------领取钻石
				...
				if (isinsert) {
					MessageUtil.notify_player(player, Notifys.ERROR,ResManager.getInstance().getString("没有找到您的公测钻石卡记录"));
					return;
				}else {
					if (ybCardBean.getYuanbao() == 0) {
						MessageUtil.notify_player(player, Notifys.ERROR,ResManager.getInstance().getString("您已经领取全部公测卡钻石。"));
						return ;
					}
					...
					if(ybcardDao.update(ybCardBean) == 1){
						...
						log.error(player.getName()+"=领取时，钻石清零失败");
					}
				}	
			}
		}
