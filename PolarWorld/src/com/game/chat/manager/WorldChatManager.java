...
public class WorldChatManager {
	 ...
	 * @param senderwebvip
	 *            发送人WEB VIP类型
	 * @param receiverVIPtype
	 *            收到人VIP类型
	 * @param receiverwebvip
	 *            收到人WEB VIP类型
	 */
	public void chat(long sender, String name, int kinglv, int country,
			String receiver, int chattype, String condition,
			List<GoodsInfoRes> list, boolean isProhibit, boolean isgm,
			short sendervip, int senderwebvip, short receiverVIPtype,
			int receiverwebvip, short senderviplevel) {
		Player player = PlayerManager.getInstance().getPlayer(sender);
		if (player == null) {
			log.error("发送者不在线");
			return;
		}
		chatLog.info(isProhibit + "\t" + player.getName() + "\t" + receiver
				+ "\t" + chattype + "\t" + condition + "\t"
				+ JSONserializable.toString(list));
		switch (chattype) {
		case CHATTYPE_ROLE:// 私聊
			chatToRole(player, kinglv, country, receiver, chattype, list,
					condition, isProhibit, isgm, sendervip, senderwebvip,
					receiverVIPtype, receiverwebvip, senderviplevel);
			break;
		case CHATTYPE_TEAM:// 队伍
			chatToTeam(player, kinglv, country, chattype, list, condition,
					isProhibit, isgm, sendervip, senderwebvip, senderviplevel);
			break;
		case CHATTYPE_GROUP:// 战盟
			chatToGroup(player, kinglv, country, chattype, list, condition,
					isProhibit, isgm, sendervip, senderwebvip, senderviplevel);
			break;
		case CHATTYPE_GM:
			chatToGm(player, kinglv, country, chattype, list, condition,
					isProhibit, isgm);
		default:
			break;
		}
	 ...
	 * 私聊
	 */
	private void chatToRole(Player source, int kinglv, int country,
			String receiver, int chattype, List<GoodsInfoRes> other,
			String condition, boolean isProhibit, boolean isgm,
			short sendervip, int senderwebvip, short receiverVIPtype,
			int receiverwebvip, short senderviplevel) {
		Player player = PlayerManager.getInstance().getOnlinePlayerByName(
				receiver);
		if (player == null) {
			MessageUtil.notify_player(source, Notifys.CHAT_ROLE, receiver
					+ ResManager.getInstance().getString("不在线"));
			return;
		}
		try {
		...
		msg.setSenderviplevel(senderviplevel);
		...
		if (isProhibit) {
			...
			ManagerPool.friendManager.addRelation(source.getId(), (byte) 3,
					player.getId(), true);
		} else {
			...
			ManagerPool.friendManager.addRelation(source.getId(), (byte) 3,
					player.getId(), true);
			ManagerPool.friendManager.addRelation(player.getId(), (byte) 3,
					source.getId(), true);
		}
	}
	 ...
	 * @param condition
	 */
	private void chatToTeam(Player source, int kinglv, int country,
			int chattype, List<GoodsInfoRes> other, String condition,
			boolean isProhibit, boolean isgm, short vip, int webvip, 
			short senderviplevel) {
	 ...
	 * @param isgm
	 */
	private void chatToGroup(Player source, int kinglv, int country,
			int chattype, List<GoodsInfoRes> other, String condition,
			boolean isProhibit, boolean isgm, short vip, int webvip,
			short senderviplevel) {
		...
		if (isProhibit) {
			...
			HashMap<Long, MemberInfo> memberinfoHashMap = guild
	...
	private void chatToGm(Player source, int kinglv, int country, int chattype,
			List<GoodsInfoRes> other, String content, boolean isProhibit,
			boolean isgm) {
		...
		// PlayerManager.getInstance().getPlayerWorldInfo(source.getId());
		...
		String createUrlParam = HttpUtil.createUrlParam("context",
				CodedUtil.encodeBase64(buff.toString()));
		GmChatManager.getInstance().sendMsg(createUrlParam);
	}

	public void gmToRole(Player receiver, String source, String content,
			String time) {
		ChatResponseWCMessage msg = new ChatResponseWCMessage();
		msg.setChater(-2);
