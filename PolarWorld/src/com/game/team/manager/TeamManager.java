...
public class TeamManager {
	...
	//请求超时
	private int TIMEOUT = Integer.MAX_VALUE;//30;
	...
	public TeamInfo getTeamInfoByPlayerId(long playerId) {
		for (TeamInfo teamInfo : teamlist.values()) {
			for(TeamMemberInfo teamMemberInfo : teamInfo.getMemberinfo()) {
				if (teamMemberInfo != null && teamMemberInfo.getMemberid() == playerId) {
					return teamInfo;
				}
			}
		}
	...
	public TeamMemberInfo getTeamMemberInfoByPlayerId(long playerId) {
		for (TeamInfo teamInfo : teamlist.values()) {
			for(TeamMemberInfo teamMemberInfo : teamInfo.getMemberinfo()) {
				if (teamMemberInfo != null && teamMemberInfo.getMemberid() == playerId) {
	...
	public TeamMemberInfo fillinmemberinfo(Player player,TeamMemberInfo memberinfo){
		...
		memberinfo.setJob(player.getJob());
		memberinfo.setIsonline((byte)1);
		return memberinfo;
	}
		...
		if(teaminfo != null) {
			...
			if(tab != null) {
				if(tab.size() < largestmember) {
					...
					notify_team_all_player(tid,MessageFormat.format(ResManager.getInstance().getString("{0}（{1}级{2}）加入了队伍。"), player.getName(), player.getLevel(),PlayerManager.getPlayerJobName(player)));
					return true;
				}
			}
		}
	...
	public void notify_team_all_player(long tid, String message) {
		...
		if(teaminfo != null ) {
			...
			if (tab.size() > 0) {
				...
				MessageUtil.notify_player(list, Notifys.NORMAL, message);
	...
	public void stReqApply(long pid ,long tid) {
		...
		if(bteamid > 0) {
			MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("{1}已经加了别的队伍。"),player.getName());
			return;
		}		
		...
		if(teaminfo.getMemberinfo().size() < largestmember) {
			...
			MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("{1}所在队伍人数已满。"),cplayer.getName());
		}
	}
	...
	public void applydealwith(long tid ,long pid,byte select) {
		...
		if(select == 0) {//同意申请
			...
			long bteamid = player.getTeamid();
			if(bteamid > 0) {
				MessageUtil.notify_player(cplayer,Notifys.ERROR,ResManager.getInstance().getString("{1}已经加了别的队伍。"),player.getName());
				removetmpteamlist( tid , pid);
				return;
			}
			...
			if(teaminfo.getMemberinfo().size() < largestmember) {
				...
				MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("{1}所在队伍人数已满。"),cplayer.getName());
				MessageUtil.notify_player(cplayer,Notifys.ERROR,ResManager.getInstance().getString("{1}所在队伍人数已满。"),cplayer.getName());
			}	
		}else {//拒绝
			if (select == 2) {
				MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("对不起，队长『{1}』没有响应您的入队申请。"),cplayer.getName());
			}else {
				MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("{1}拒绝了您的组队申请。"),cplayer.getName());
			}
			removetmpteamlist( tid , pid);
		}
	}
	...
	public void stReqInviteWorld(long teamid, long playerid,long captainid) {
		...
		if (player.getId() == cplayer.getId()) {
			MessageUtil.notify_player(cplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，不能对自己发送邀请。"));
			return;
		}
		...
		if(teamid > 0 ) {
			...
			if(teaminfo.getMemberinfo().size() < largestmember) {
				...
				if (player.getAutoteaminvited() == 1 && player.getTeamid() <= 0) {	//检测玩家是否设置自动邀请
			...
			this.stCreateateamWorld(captainid);//创建新队伍
			addtmpteamlist(cplayer.getTeamid(), playerid); //加入临时组队验证表
			if (player.getAutoteaminvited() == 1  && player.getTeamid() <= 0) {	//检测玩家是否设置自动邀请
				Invitedealwith(cplayer.getTeamid(),  playerid,captainid,(byte) 0);
			}else {
				addtmpcaptain(playerid,captainid);//加入临时队长表，玩家同意后，根据这里的信息创建队伍
				ResInviteClientMessage msg = new ResInviteClientMessage();
				TeamMemberInfo memberinfo = fillinmemberinfo(cplayer, new TeamMemberInfo());
				msg.setCaptaininfo(memberinfo);
				msg.setTeamid(cplayer.getTeamid());
				MessageUtil.tell_player_message(player,msg);//发送消息给玩家做选择
				MessageUtil.notify_player(cplayer,Notifys.NORMAL,ResManager.getInstance().getString("已发送组队邀请，请等待回应。"));
			}
		}
	...
	public void Invitedealwith(long tid, long pid,long cid ,byte select) {
		...
		if(select == 0) {
			...
			if(tid > 0) {
				...
				TeamInfo teaminfo = getTeam(tid);
				if(teaminfo == null) {
					MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("找不到{1}的队伍，队伍已解散。"),cplayer.getName());
					return;//没有得到队伍
				}
				if(teaminfo.getMemberinfo().size() < largestmember) {
					if(cktmpteamlist(tid,pid)) {
//						if (teaminfo.getMemberinfo().get(0).getMemberid() != cid) return;
							if (cplayer !=  null ) {
								if(cplayer.getCountry() != player.getCountry() )return;
								//xiaozhuoming: 如果玩家已经有队伍, 把玩家从该队伍退出
								if(player.getTeamid() > 0) {
									this.stOfflineLeavetheTeam(pid, (byte)1, player.getTeamid());
									removetmpteamlist(tid , pid);
								}
								addmembers(tid ,pid);//加入
								ManagerPool.friendManager.addRelation(cplayer.getId(),(byte)3,player.getId(),true);
			...
			if (select == 2) {
				...
				if(tmpcid != 0 || cktmpteamlist(tid,pid)) {
					Player cplayer = ManagerPool.playerManager.getPlayer(cid);
					if(cplayer !=null) {
						MessageUtil.notify_player(cplayer,Notifys.ERROR,ResManager.getInstance().getString("{1}拒绝了您的组队邀请。"),player.getName());
					}
				}
			}
			removetmpcaptain(pid);
	 ...
	 * 
	 */
	public void stOfflineLeavetheTeam(long pid, byte type, long tid){
...
//			Player cplayer = ManagerPool.playerManager.getPlayer(cid);
			...
			if (type == 0 ) {
				if (cid == pid && player != null) {
					MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("您不能开除自己，若需退出，请点退出队伍"));
					return;
				}
			}
			
			//xiaozhuoming: 如果玩家是队长,就自动任命队长
			List<TeamMemberInfo> teamMemberInfoList = teaminfo.getMemberinfo();
			if(teamMemberInfoList.size() > 1 && cid == pid) {
				this.Appointdealwith(tid, teamMemberInfoList.get(1).getMemberid(), (byte)0);
			}
			...
			if (pos >= 0) {	//位置从0开始
				...
				if (player != null) {
					...
					if (type == 0) {
						MessageUtil.notify_player(player,Notifys.NORMAL,ResManager.getInstance().getString("您被队长请出了队伍。"));
						MessageUtil.notify_player(player,Notifys.CHAT_TEAM,ResManager.getInstance().getString("您被队长请出了队伍。"));
					}else {
						 if(teaminfo.getMemberinfo().size() < 3 && type == 1){
							 MessageUtil.notify_player(player,Notifys.NORMAL, MessageFormat.format(ResManager.getInstance().getString("{0}({1}级{2})离开了队伍，队伍自动解散。"),player.getName(), player.getLevel(),PlayerManager.getPlayerJobName(player)));
							 MessageUtil.notify_player(player,Notifys.CHAT_TEAM,MessageFormat.format(ResManager.getInstance().getString("{0}({1}级{2})离开了队伍，队伍自动解散。"),player.getName(), player.getLevel(),PlayerManager.getPlayerJobName(player)));
						 } else {
							 MessageUtil.notify_player(player,Notifys.NORMAL, MessageFormat.format(ResManager.getInstance().getString("{0}({1}级{2})离开了队伍。"),player.getName(), player.getLevel(),PlayerManager.getPlayerJobName(player)));
							 MessageUtil.notify_player(player,Notifys.CHAT_TEAM,MessageFormat.format(ResManager.getInstance().getString("{0}({1}级{2})离开了队伍。"),player.getName(), player.getLevel(),PlayerManager.getPlayerJobName(player)));
						 }
					}
				}
				...
				if(player != null) {
					if (type == 0) {
						notify_team_all_player(tid,MessageFormat.format(ResManager.getInstance().getString("{0}({1}级{2})被队长请出队伍。"),player.getName(),player.getLevel()+"",PlayerManager.getPlayerJobName(player)));
					}else{
						if(teaminfo.getMemberinfo().size() < 2 && type == 1){
							 notify_team_all_player(tid,MessageFormat.format(ResManager.getInstance().getString("{0}({1}级{2})离开了队伍，队伍自动解散。"), player.getName(), player.getLevel(), PlayerManager.getPlayerJobName(player)));
						 } else {
							 notify_team_all_player(tid,MessageFormat.format(ResManager.getInstance().getString("{0}({1}级{2})离开了队伍。"), player.getName(), player.getLevel(), PlayerManager.getPlayerJobName(player)));
						 }
					}
				}
				/*
			...
			long playerId = 0;
			if((teaminfo.getMemberinfo().size() < 2 && type == 1) || teaminfo.getMemberinfo().size() == 0) {
				if(teaminfo.getMemberinfo().size() == 1 && teaminfo.getMemberinfo().get(0) != null) {
					playerId = teaminfo.getMemberinfo().get(0).getMemberid();
				}
				
				delTeam(tid);	//删除队伍（解散）
			}else if(cid == pid) {
				//xiaozhuoming: 如果所有队员均不在线, 则解散队伍
				boolean flag = true; //假设全部队员不在线
				for (TeamMemberInfo teamMemberInfo : teaminfo.getMemberinfo()) {
					Player member = ManagerPool.playerManager.getPlayer(teamMemberInfo.getMemberid());
					if(member != null) {
						flag = false; //有队员在线
						break;
					}
			...
			if(player != null) {
				...
				msgclient.setTeamid(0l);
				MessageUtil.tell_player_message(player,msgclient);
				ReqMapSearchTeamInfoWorldMessage msg = new ReqMapSearchTeamInfoWorldMessage();
				msg.setPlayerid(pid);
				stMapSearchTeamInfo(msg);
			}
			
			Player otherPlayer = ManagerPool.playerManager.getPlayer(playerId);
			if(otherPlayer != null) {
				ResToleaveWorldMessage msg = new ResToleaveWorldMessage();
				msg.setPlayerid(playerId);
				msg.setTeamid(tid);
				otherPlayer.setTeamid(0);
				MessageUtil.send_to_game(otherPlayer, msg);// 发离队消息到game
				
				ResTeamClientRefreshMessage msgclient = new ResTeamClientRefreshMessage();
				msgclient.setTeamid(0l);
				MessageUtil.tell_player_message(otherPlayer,msgclient);
				ReqMapSearchTeamInfoWorldMessage sendMessasge = new ReqMapSearchTeamInfoWorldMessage();
				sendMessasge.setPlayerid(playerId);
				stMapSearchTeamInfo(sendMessasge);
			}
		}
		removetmpcaptain(pid);
	}
	 ...
	 */
	public void stLeavetheteam(long pid ,byte type, long tid) {
		/*xiaozhuoming: 不管玩家是否在线, 都可以被踢
		Player player = ManagerPool.playerManager.getPlayer(pid);
		if (player != null ) {
			stOfflineLeavetheTeam(player, type);
		}*/
		stOfflineLeavetheTeam(pid, type, tid);
	 ...
	 * @param cid
	 */
	public void stReqAppoint(long tid , long pid, long cid) {
		Player player = ManagerPool.playerManager.getPlayer(pid);
		if(player == null) {
			Player captain = ManagerPool.playerManager.getPlayer(cid);
			TeamMemberInfo teamMemberInfo = this.getTeamMemberInfoByPlayerId(pid);
			if(captain != null) MessageUtil.notify_player(captain, Notifys.ERROR, "很抱歉，"+ teamMemberInfo.getMembername() + "已经下线了。");
			return;
		}
//		ResAppointClientMessage msg = new ResAppointClientMessage();
		...
		if(teaminfo !=null) {
			...
			if(info != null && info.size() > 1) {
				...
				if (info.get(0).getMemberid() != cid) {
					MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("您不是队长，不能任命。"));
					return;
				}
				
				/*xiaozhuoming: 自动任命新队长，不需要玩家同意
				...
				*/
				addtmpcaptain(info.get(0).getMemberid() ,pid);
				this.Appointdealwith(tid, pid, (byte)0);
			}
		}
	}
			...
			if(info != null && info.size() > 1) {
				...
				if(cid == pid) {
					...
					if(tmemberInfo != null) {	//互换位置
						...
						refreshTeam(teaminfo,tid);	//刷新存入队伍
						notify_team_all_player(tid,MessageFormat.format(ResManager.getInstance().getString("{0}({1}级{2})成为了新的队长。"),tmemberInfo.getMembername(),tmemberInfo.getMemberlevel(),PlayerManager.getPlayerJobName(player)));
					}	
				}else {
					MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您的操作已经过期了"));
				}
	 ...
	 * @param player
	 */
	public void logoutRemoveTeamMemberInfo(Player player) {
		if(player == null) {
			return;
		}
		
		TeamInfo teamInfo = ManagerPool.teamManager.getTeam(player.getTeamid());
		if(teamInfo == null || teamInfo.getMemberinfo().size() < 1) {
			return;
		}
		
		if(teamInfo.getMemberinfo().get(0).getMemberid() == player.getId()) {
			boolean flag = true; //假设其他队员不在线
			for (TeamMemberInfo teamMemberInfo : teamInfo.getMemberinfo()) {
				Player member = ManagerPool.playerManager.getPlayer(teamMemberInfo.getMemberid());
				if(member != null && teamMemberInfo.getMemberid() != player.getId()) {
					flag = false; //有队员在线
					break;
				}
			}
			
			if(teamInfo.getMemberinfo().size() == 1 || flag == true) {
				delTeam(player.getTeamid());
				if(player != null) MessageUtil.notify_player(player,Notifys.NORMAL,ResManager.getInstance().getString("队伍被解散了。"));
			}else {
				ManagerPool.teamManager.addtmpcaptain(teamInfo.getMemberinfo().get(0).getMemberid(), teamInfo.getMemberinfo().get(1).getMemberid());
				ManagerPool.teamManager.Appointdealwith(teamInfo.getTeamid(), teamInfo.getMemberinfo().get(1).getMemberid(), (byte)0);
			}
		}
		
		TeamMemberInfo teamMember = this.getTeamMemberInfoByPlayerId(player.getId());
		if(teamMember != null) {
			teamMember.setIsonline((byte)0);
			for(TeamMemberInfo teamMemberInfo : teamInfo.getMemberinfo()) {
				if(teamMemberInfo == null) continue;
				Player member = ManagerPool.playerManager.getPlayer(teamMemberInfo.getMemberid());
				if(member == null ) continue;
				ResTeamClientRefreshMessage sendMessage = new ResTeamClientRefreshMessage();
				sendMessage.setMemberinfo(teamInfo.getMemberinfo());
				sendMessage.setTeamid(player.getTeamid());
				MessageUtil.tell_player_message(member, sendMessage);
			}
		}
	}
}
