		...
		if (items.size() > 0) {
			...
			for (int i = 0; i < items.size(); i++) {
				if (items.get(i) == null) {
					...
					if(items.get(i).getItem().getItemModelId() == -1) {	//钻石数量
						info.setSellgold(items.get(i).getItem().getNum());
					}else if (items.get(i).getItem().getItemModelId() == -2) {//钻石数量
...
//			}
	...
	public void  stReqStallsProductWasAddedToWorldMessage(ReqStallsProductWasAddedToWorldMessage msg) {
		...
		for (int i = 0; i < pinof.getStallsgoodsinfo().size(); i++) {
			...
			if (gitem.getItemModelId() > 0) {//道具数量
				...
				if (item.getItemModelId() == gitem.getItemModelId()) {
					MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("您已经上架钻石或者金币，同类型不能多次上架。"));
		...
		if (msg.getPricegold() > 0) {
			pricestrString = ResManager.getInstance().getString("金币")+msg.getPricegold()+" ";
		}
		if (msg.getPriceyuanbao() > 0) {
			pricestrString = pricestrString+ResManager.getInstance().getString("钻石:")+msg.getPriceyuanbao() ;
		}
		...
		String nameString = "";
		if (itemid == -1 ) {	//上架金币
			nameString = ResManager.getInstance().getString("金币");
			if (stallsitem == null) {
			...
			stallsitem.setPriceyuanbao(msg.getPriceyuanbao()); //设定金币的钻石售价
			issuccess = true;
			
		}else if (itemid == -2) {//上架钻石
			nameString = ResManager.getInstance().getString("钻石");
			if (stallsitem == null) {
				stallsitem = new StallsItem();
				stallsitem.setItem(item);
				tab.add(stallsitem);
			...
			stallsitem.setPricegold(msg.getPricegold());	//设定钻石的金币售价
			issuccess = true;
	...
	public void stReqStallsAdjustPricesToWorldMessage(ReqStallsAdjustPricesToWorldMessage msg){
		...
		if(msg.getPos() >= 0 && msg.getPos() < goodsinfo.size() && msg.getGoodsid() > 0) {	//更换位置
			...
			if (laoitem != null && laoitem.getItem().getId() != msg.getGoodsid()) {
				...
				StallsItem 	yuanbao = getstallsitem(goodsinfo,-2);	//钻石和金币放在链表最后
...
//		}
	...
	public void stReqStallsSearchToWorldMessage(ReqStallsSearchToWorldMessage wmsg) {
		...
		if (goodsname != null) {
			if (goodsname.equals("金币")) {
				isyuanbaogold = 1;
			}else if(goodsname.equals("钻石")) {
				isyuanbaogold = 2;
			}
		}
					...
					if(tmplist.get(i).getPlayername().contains(pname)){
						...
						if (xplayer != null) {
							...
							for (int j = 0; j < items.size(); j++) {
								if(items.get(j).getItem().getItemModelId()> 0){
									Q_itemBean q_itemBean = DataManager.getInstance().q_itemContainer.getMap().get(items.get(j).getItem().getItemModelId());
									if(checkItem(q_itemBean, wmsg, items.get(j).getItem())){
										stinfo.add(tmplist.get(i));
										break;
									}
								}
...
//					return;
//				}
			}else if (isyuanbaogold == 1 ) {		//0不搜索，1金币，2钻石，3 两个都搜索
	...
	public boolean checkItem(Q_itemBean model,ReqStallsSearchToWorldMessage wmsg,Item item) {
		//多选条件   卓越
		if(wmsg.getZhuoyue()==1&&model.getQ_remarkable()!=1){
			return false;
		}
		
		//多选条件  追加
		if(wmsg.getAddAttribut()==1){
			if(item instanceof Equip){
				if(((Equip)item).getAddAttributeLevel()<=0){
					return false;
				}
			}else{
				return false;
		...
		//强化等级
		if(wmsg.getIntensify()>0){
			if(item instanceof Equip){
				if(((Equip)item).getGradeNum()!=wmsg.getIntensify()){
					return false;
				}
			}else{
				return false;
		...
		//这不是件通用物品
		//! changed by xuliang
		if(wmsg.getQ_job_limit()!=0 && !model.getQ_job_limit().equals("0")){
			int job=wmsg.getQ_job_limit();

			if (model.getQ_job_limit().charAt(job - 1) != 49){
				return false;
			}
		}
		//验证是否可用
		if(wmsg.getCan_use()==1){
			//职业满足后 验证 属性是否满足
			if(model.getQ_str_limit()>wmsg.getStrength() || model.getQ_dex_limit() >wmsg.getAgile()){
				//你的属性装不上
				return false;
			}
		}
		//绑定部位
		if(wmsg.getQ_kind()>0&&wmsg.getQ_kind()!=model.getQ_kind()){
			return false;
		}
		//验证主类型
		if(!wmsg.getQ_type().equals("0") && !wmsg.getQ_type().equals(model.getQ_type()+"")){
			return false;
		}
		...
		//如果有 追加的 验证名字  
		if(wmsg.getHidden()!=null&&!"".equals(wmsg.getHidden())){
			if(!model.getQ_name().contains(wmsg.getHidden())){
				return false;
			}
		}
		//名字验证			
		if(wmsg.getGoodsname()!=null&&!"".equals(wmsg.getGoodsname())){
			if(model.getQ_name().contains(wmsg.getGoodsname())){
				return true;
			}else{
				return false;
			}
	...
	public void stReqChangeStallsNameToWorldMessage(ReqChangeStallsNameToWorldMessage wmsg) {
		...
		if (stname.equals("") || stname.equals(dfname)) {
			msg.setName(dfname);
			stname = "";
			MessageUtil.notify_player(player,Notifys.NORMAL,ResManager.getInstance().getString("摊位名修改成功。"));
		}else {
			msg.setName(stname);
			MessageUtil.notify_player(player,Notifys.NORMAL,ResManager.getInstance().getString("摊位名修改成功。"));
		}
