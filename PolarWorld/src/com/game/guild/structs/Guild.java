...
/**
 *
 * @author xiaozhuoming 公会类
 */
public class Guild {

	private Logger log = Logger.getLogger(Guild.class);
	private int Event_MaxNum = 100;		//事件保存最大数
	private int country;			//战盟所属国家
	...
	private List<ApplyAndInvite> saveSendMsgList = new ArrayList<ApplyAndInvite>();	//保存要给盟主下次上线发送的消息
...
//			memberInfo.setBasicInfo(player, getGuildInfo());
	...
	public void calFightPowerAndMemberNum() {
		...
		//排行榜数据覆盖
		GuildFightPowerTop fightPowerTop = new GuildFightPowerTop(getGuildInfo().getGuildId(), getGuildInfo().getMemberFightPower(), getGuildInfo().getBannerLevel());
//		 TopListManager.getInstance().getGuildTopMap().get(fightPowerTop.getId());
	...
	public int getPowerLevelCount(int powerLevel) {
		if (powerLevel == 4) {
			return 0;
		}
		int sum = 0;
...
//					memberInfo.setBasicInfo(player, getGuildInfo());
	...
	public void sendAllMemberNotifyNoMe(Player noplayer, Notifys type, String message, int subType, String... values) {
		sendAllMemberNotifyNoMe(noplayer, type.getValue(), message, subType, values);
	}
	
	public void sendAllMemberNotifyNoMe(Player noplayer, byte type, String message, int subType, String... values) {
		for (Map.Entry<Long, MemberInfo> entry : getMemberinfoHashMap().entrySet()) {
			MemberInfo memberInfo = entry.getValue();
			if (memberInfo != null) {
				Player player = PlayerManager.getInstance().getPlayer(memberInfo.getUserId());
	...
	public boolean deleteMember(MemberInfo memberInfo) {
		try {
			if (GuildWorldManager.getInstance().getGuildmemberDao().delete(memberInfo.getUserId()) == 0) {
				log.error(String.format("战盟成员数据删除错误，玩家ID[%s]", String.valueOf(memberInfo.getUserId())));
				return false;
			}
			return true;
		} catch (Exception e) {
		...
		try {
			...
			if (GuildWorldManager.getInstance().getGuildmemberDao().update(guildmemberBean) == 0) {
				if (GuildWorldManager.getInstance().getGuildmemberDao().insert(guildmemberBean) == 0) {
					log.error(String.format("战盟成员数据保存错误，玩家ID[%s]", String.valueOf(guildmemberBean.getUserid())));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public boolean deleteEvent(EventInfo eventInfo) {
		try {
			if (GuildWorldManager.getInstance().getGuildeventDao().delete(eventInfo.getEventId()) == 0) {
				log.error(String.format("战盟事件数据删除错误，事件消息[%s]", eventInfo.getMessage()));
				return false;
			}
			return true;
		} catch (Exception e) {
		...
		try {
			...
			if (GuildWorldManager.getInstance().getGuildeventDao().update(guildeventBean) == 0) {
				if (GuildWorldManager.getInstance().getGuildeventDao().insert(guildeventBean) == 0) {
					log.error(String.format("战盟事件数据保存错误，事件消息[%s]", eventInfo.getMessage()));
					return false;
				}
			}
			return true;
			...
			while (iterator.hasNext()) {
				...
				if (guildmemberBean != null) {
					...
					if (memberInfo != null) {
						...
						log.error(String.format("战盟成员数据读取错误，成员id[%s]", String.valueOf(guildmemberBean.getUserid())));
					}
				}
			}
		} catch (Exception e) {
			...
			while (iterator.hasNext()) {
				...
				if (guildeventBean != null) {
					...
					if (eventInfo != null) {
						...
						log.error(String.format("战盟事件数据读取错误，数据内容[%s]", guildeventBean.getEventdata()));
					}
				}
			}
		} catch (Exception e) {
