...
public class TopPlayer {
	...
	private final int MaxMaxMpCof = 1;			//最大魔法值	1
	...
	public void updateTopData(TopData topData) {
		...
		if (topData instanceof LevelTop) {
			...
			this.setHorseLevel(horseTop.getHorsejieshu());
			this.setHorseExp(horseTop.getHorseexp());
		...
		} else if(topData instanceof RechargeTop){
			RechargeTop rechargeTop = (RechargeTop) topData;
			this.setRechargeGold(rechargeTop.getRechargeNum());
			this.setRechargeTime(rechargeTop.getRechargeTime());
		} else if(topData instanceof PataTop){
			PataTop pataTop = (PataTop)topData;
			this.setPataId(pataTop.getPataId());
			this.setPataTime(pataTop.getPataTime());
		}
	}
	...
	public TopData getTopData(TopData topData) {
		...
		if (topData instanceof LevelTop) {
			...
			retTopData = new HorseTop(getId(), getHorseLevel(), getCurHorse().getHorselevel(), getCurHorse().getSkilllevelsum(), getCurHorse().getHorseUpTime(), getHorseExp());
		} else if (topData instanceof GestTop) {
			...
			retTopData = new RechargeTop(getId(), getRechargeGold(), getRechargeTime());
		} else if (topData instanceof PataTop){
			retTopData = new PataTop(getId(), getPataId(), getPataTime());
		}
		return retTopData;
	}
