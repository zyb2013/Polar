...
public class ScriptsUtils {
	 ...
	 * @param paras
	 */
	public static void callFromGame(int scriptId, String method, List<String> paras){
		ArrayList<Object> parameters = new ArrayList<Object>();
...
//	}
	 ...
	 * @param paras
	 */
	public static void callGame(Player player, int scriptId, String method, String... paras){
		ReqScriptToGameMessage msg = new ReqScriptToGameMessage();
		msg.setScriptId(scriptId);
		msg.setMethod(method);
		for (int i = 0; i < paras.length; i++) {
			msg.getParas().add(paras[i]);
