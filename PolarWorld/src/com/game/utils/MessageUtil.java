...
public class MessageUtil {
	 ...
	 * 每三位为一个类型
	 */
	public static long getNotifyType(byte... notifys){
		long notify = 0;
		long parm = 1;
		
		for(int i = 0; i < notifys.length; i ++){
			notify += notifys[i] * parm;
			parm *= 1000;
		}
	...
	public static void notify_player(Player me, byte type, String message, String... values) {
		notify_player(me, type + "", message, values);
	}
	...
	public static void notify_player(List<Long> players, Notifys type, String message, String... values){
		notify_player(players, type.getValue() + "", message, values);
	}
	
	public static void notify_player(List<Long> players, String type,String message,String... values){
		PersonalNoticeWorldMessage msg = new PersonalNoticeWorldMessage();
		msg.setType(type);
		msg.setContent(message);
		if (values != null) {
	...
	public static void notify_user(User user, Notifys type, String message, String... values){
		notify_user(user, type.getValue() + "", message, values);
	}
	
	public static void notify_user(User user, String type, String message, String... values) {
		PersonalNoticeWorldMessage msg = new PersonalNoticeWorldMessage();
		msg.setType(type);
		msg.setContent(message);
		if (values != null) {
