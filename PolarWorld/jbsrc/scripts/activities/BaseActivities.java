...
//		}
	...
	@Override
	public void getReward(Player player, ActivityInfo ac) {
		Map<String, String> playerparams = QueryRecharge.queryPlayerAwardParam(player);
		if (ac != null) {
			int state = -1;
...
//			}
			...
			QueryRecharge.updatePlayerParams(player, playerparams);
			ScriptsUtils.callGame(player, ScriptEnum.BASEACTIVITIES, "worldCallGetReward", Long.toString(player.getId()), Integer.toString(ac.getActivityId()), Integer.toString(player.getRepeatNum()));
