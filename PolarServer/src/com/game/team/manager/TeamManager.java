...
public class TeamManager {
	...
	/**
	 * 创建队伍，转发到世界服务器
	 * 队伍ID为0，则在世界服务器自动生成ID
	 * @param player
	 */
	public void stCreateateam(Player player,long newtid) {
		...
		if (player.getMapModelId() == CsysManger.CSYS_MAPID) {
			MessageUtil.notify_player(player,Notifys.ERROR, ResManager.getInstance().getString("很抱歉，您处于副本中，不能创建队伍。"));
			return;
		}
	...
	public void stReqInviteGame(Player player ,long tid,long pid) {
		...
		if (player.getMapModelId() == CsysManger.CSYS_MAPID) {
			MessageUtil.notify_player(player,Notifys.ERROR, ResManager.getInstance().getString("很抱歉，您处于副本中，无法发起组队邀请。"));
			return;
		}
		
		Player destPlayer = ManagerPool.playerManager.getPlayer(pid);
		if(destPlayer != null && destPlayer.getMapModelId() == CsysManger.CSYS_MAPID) {
			MessageUtil.notify_player(player,Notifys.ERROR, ResManager.getInstance().getString("很抱歉，对方处于副本中，无法对其发起组队邀请。"));
			return;
		}
	 ...
	 * @return
	 */
	public List<Player> getAreaSameTeam(Player player) {
		...
		if (map != null) {
			...
			for (int i = 0; i < rounds.size(); i++) {
				...
				if (area != null) {
					Iterator<Player> it = area.getPlayers().iterator();
					while (it.hasNext()) {
						Player member = it.next();
						if (member.getTeamid() > 0
								&& member.getTeamid() == player.getTeamid()) {
							sameteam.add(member);
						}
					}
	 ...
	 * @return
	 */
	public int getTeamTotalLevel(List<Player> members) {
		int num = 0;
		for ( Player member : members) {
			num = num + member.getLevel();
		}
	...
	public void stMapSearchPlayerInfo(Player player, ReqMapSearchPlayerInfoGameMessage msg) {
		...
		if (player != null) {
			...
			ConcurrentHashMap<Long, Player>	tmpplayers = ManagerPool.playerManager.getOnline();
			...
			while (iterator.hasNext()) {
				...
				if (bplayer.getMap() == mapid && bplayer.getId() != player.getId() &&  bplayer.getCountry() == player.getCountry()) {
					...
					if (bplayer.getGuildInfo() != null) {
						pInfo.setGuildname(bplayer.getGuildInfo().getGuildName());
					}
	...
	public void TeammateDieNotify(Player player,Player attacker){
		//本队成员:[XXXXX],在[XXX地图名]X线坐标:XX,XX被【X国】[XXX战盟][XXXX]玩家击败"
		...
		if (guild != null && attacker.getGuildId() != 0 ) {
			gname = "『"+guild.getGuildName()+ResManager.getInstance().getString("战盟』");
		}
		
//		GameConfig config = new GameConfig();
//		String cname = config.getCountryNameByConutry(attacker.getCountry());
		ParseUtil par = new ParseUtil();
		par.setValue(ResManager.getInstance().getString("本队成员:『{@}』在『{@}』被")+gname+ResManager.getInstance().getString("『{@}』击败。"),new ParseUtil.PlayerParm(player.getId(), player.getName()),new ParseUtil.MapParm(mapid,line,x,y),new ParseUtil.PlayerParm(attacker.getId(), attacker.getName()));
	...
	public void stResTeamchangeToGameMessage(ResTeamchangeToGameMessage msg) {
		...
		if (player != null) {
			/*boolean is =false;
			if (player.getTeamid() == 0 && player.getPkState() == 2) {
				is=true;
			}*/
