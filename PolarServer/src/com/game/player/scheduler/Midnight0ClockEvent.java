...
//		if(System.currentTimeMillis()-clockTimer>=24*60*60*1000){
		...
		try {
			// panic god ManagerPool.dianjiangchunManager.ClearDianjiangchunInfo(player);
		} catch (Exception e) {
			logger.error(e, e);
		}
		try {
		...
		try // 活跃度数据清空
		{
			ManagerPool.livenessManager.reset(player, true);
		} catch (Exception e) {
			logger.error(e, e);
		}
		try {
			// 处理vip经验点
			VipManager.getInstance().zeroCheck(player);
		} catch (Exception e) {
			logger.error("VipManager", e);
		}
