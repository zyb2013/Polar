...
public class ResMyPlayerInfoMessage extends Message {
	 ...
	 * 正在进行倒计时开启的背包格子
	 */
	private short coolDowningCellGridId;
	
	/**
	 * 正在进行倒计时开启的背包格子的结束时间，秒
	 */
	private int coolDowningCellGridTime;
	// 金币
	private int money;
	...
	public boolean write(IoBuffer buf) {
		...
		// 角色职业
		writeInt(buf, this.job);
		//GM
		writeInt(buf, this.gm);
		//创建时间
		writeLong(buf, this.createTime);
		// 角色等级
...
//			System.out.println("key:"+attributes.get(i).getType()+" value:"+attributes.get(i).getValue() );
		...
		writeShort(buf,coolDowningCellGridId);
		writeInt(buf,coolDowningCellGridTime);
		// 金币
		...
		// 一级属性【力量，体力，敏捷，智力，预留属性】
		writeInt(buf, this.attibute_one[0]);// 力量
		writeInt(buf, this.attibute_one[1]);// 体力
		writeInt(buf, this.attibute_one[2]);// 敏捷
		writeInt(buf, this.attibute_one[3]);// 智力
		writeInt(buf, this.attibute_one[4]);// 预留属性
		//队伍Id
...
//			System.out.println("key:"+attributes.get(i).getType()+" value:"+attributes.get(i).getValue() );
	...
	public boolean read(IoBuffer buf) {
		...
		for (int i = 0; i < attributes_length; i++) {
			attributes.add((PlayerAttributeItem) readBean(buf,
					PlayerAttributeItem.class));
		}
		// 跑步坐标集合
		int positions_length = readShort(buf);
		for (int i = 0; i < positions_length; i++) {
			positions.add((com.game.structs.Position) readBean(buf,
					com.game.structs.Position.class));
		}
		// 装备列表信息
		int equips_length = readShort(buf);
		for (int i = 0; i < equips_length; i++) {
			equips.add((com.game.equip.bean.EquipInfo) readBean(buf,
					com.game.equip.bean.EquipInfo.class));
		}
		// 背包格子数
		...
		for (int i = 0; i < items_length; i++) {
			items.add((com.game.backpack.bean.ItemInfo) readBean(buf,
					com.game.backpack.bean.ItemInfo.class));
		}
		// 金币
		...
		for (int i = 0; i < posallgeminfo_length; i++) {
			posallgeminfo.add((com.game.gem.bean.PosGemInfo) readBean(buf,
					com.game.gem.bean.PosGemInfo.class));
		}
		// 主线任务ID
		...
		// 弓箭信息
		this.arrowInfo = (com.game.arrow.bean.ArrowInfo) readBean(buf,
				com.game.arrow.bean.ArrowInfo.class);
		// 平台VIP
		...
		// 一级属性【力量，体力，敏捷，智力，预留属性】
		this.attibute_one[0] = readInt(buf);
		this.attibute_one[1] = readInt(buf);
		this.attibute_one[2] = readInt(buf);
		this.attibute_one[3] = readInt(buf);
		this.attibute_one[4] = readInt(buf);
		//队伍Id
	...
	/**
	 * @return the coolDowningCellGridId
	 */
	public short getCoolDowningCellGridId() {
		return coolDowningCellGridId;
	}

	/**
	 * @param coolDowningCellGridId the coolDowningCellGridId to set
	 */
	public void setCoolDowningCellGridId(short coolDowningCellGridId) {
		this.coolDowningCellGridId = coolDowningCellGridId;
	}

	/**
	 * @return the coolDowningCellGridTime
	 */
	public int getCoolDowningCellGridTime() {
		return coolDowningCellGridTime;
	}

	/**
	 * @param coolDowningCellGridTime the coolDowningCellGridTime to set
	 */
	public void setCoolDowningCellGridTime(int coolDowningCellGridTime) {
		this.coolDowningCellGridTime = coolDowningCellGridTime;
	}
	 ...
	 * set 装备部位全部宝石信息
	 */
	public void setPosallgeminfo(
			List<com.game.gem.bean.PosGemInfo> posallgeminfo) {
	...
	public String toString() {
		...
		if (this.arrowInfo != null)
			buf.append("arrowInfo:" + arrowInfo.toString() + ",");
		// 平台VIP
