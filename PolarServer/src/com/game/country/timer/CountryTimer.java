			...
			if(map.getParameters().containsKey(CountryManager.WANGCHENGDIAOXIANG)){
				...
				if (status == 1) {//展示
					...
					if (kingCity.getGuildid() > 0 && kingCity.getHoldplayername() != null && !kingCity.getHoldplayername().equals("")) {
						 kingname= ResManager.getInstance().getString("【国王】")+kingCity.getHoldplayername();
					}
...
//		}
		...
		if (map.getMapModelid() == ManagerPool.csysManger.CSYS_MAPID) {
			...
			if( systime %10 == 0){
				if(ManagerPool.csysManger.getCstsstate()==2){
...
//		System.out.println("玉玺持有者："+kingCity.getHoldplayerid() );
...
//						int exp = multiple*model.getQ_dazuoexp();
						int keys =	DataManager.getInstance().q_characterContainer.getKey(player.getJob(), player.getLevel());
						Q_characterBean q = 	DataManager.getInstance().q_characterContainer.getMap().get(keys);
//						2650*15*0.08
						int exp = (int)(multiple*q.getQ_rexp()*0.1);
//						int exp = 0;
