...
public class CountryAwardManager {
	...
	public static int KINGCITYCHEST = 8022;		//每日俸禄物品id
	public static int KINGCITYCHESTTYPE = 4;	//每日俸禄灵树奖励类型

	//-----------------------成员函数-------------------------//
	/**
	 * 判断是否可以领取奖励(圣盟争夺战时间)
	 *
	 * @return true 不在圣盟争夺战期间，可以领取薪水等 false 在圣盟争夺战期间，不可以领取薪水等
	 */
				...
				switch (kingBuff) {
					case 1: {
						...
						if (buffs.isEmpty()) {
							...
							BuffManager.getInstance().addBuff(player, player, BuffConst.KINGCITY_KING1, 0, 0, 0);
							BuffManager.getInstance().addBuff(player, player, BuffConst.KINGCITY_KING2, 0, 0, 0);
						}
	...
	public void removeKingCityBuff(Player player) {
		if (player != null) {
			...
			if (!buffs.isEmpty()) {
				...
				BuffManager.getInstance().removeByBuffId(player, BuffConst.KINGCITY_KING1);
				BuffManager.getInstance().removeByBuffId(player, BuffConst.KINGCITY_KING2);
			}
//			buffs = BuffManager.getInstance().getBuffByModelId(player, BuffConst.KINGCITY_HEGEMONY);
//			if (!buffs.isEmpty()) {
...
//						if(!TimeUtil.isToday(player.getLastChangePowerLevelTime())){
							...
							if (BackpackManager.getInstance().getEmptyGridNum(player) >= getKingCityChestNum(player)) {
								List<Item> items = Item.createItems(700160, getKingCityChestNum(player), true, 0);
								BackpackManager.getInstance().addItems(player, items, Reasons.KINGCITY_GETCHEST, Config.getId());
								getKingCity().sCurdayToSalary(player.getMemberInfo().getGuildPowerLevel(),player.getId());
								CountryManager.getInstance().savekingcity(getKingCity());
								MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("领取成功"));
							} else {
								MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("您的包裹已满，请清理后再来领取！"));
							}
//						}else{
//							MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("您的职位刚变更过，今天无法领取！"));
//						}
					} else {
						MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("您无法领取每日俸禄，已经被领取过了！"));
					}
//				} else {
//					MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("您不在本国，无法领取每日俸禄！"));
//				}
			} else {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("现在正在进行攻城战，无法领取每日俸禄！"));
			}
		} else {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("您不是占领盟成员，不能领取每日俸禄！"));
		}
	}
	//-----------------------每日俸禄-----------------------------//
	...
	// 获得  大天使 武器箱子
	public void getAngleBox(Player player,int count) {
		if (isKingCity(player)) {
					if ( player.getMemberInfo().getGuildPowerLevel() == 1 ) {
								...
								if(day>=count){
									...
									if(city.getSalarymap().get("getAngleDay_"+count)!=null){
										MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("已经领取过了"));
										return;
									}
									
									Q_hold_rewardBean hold_rewardBean= DataManager.getInstance().q_hold_rewardContainer.getMap().get(count);
									if(hold_rewardBean!=null){			
										String rewards =hold_rewardBean.getQ_rewards();
										
										
										String[] reward  =  rewards.split(";");
										
										 if (BackpackManager.getInstance().getEmptyGridNum(player) >= reward.length) {
											 
											for (int i = 0; i < reward.length; i++) {
												String  id = reward[i].split("_")[0];
												String num = reward[i].split("_")[1];
												List<Item> items = Item.createItems(Integer.parseInt(id), Integer.parseInt(num), false, 0);
												
												BackpackManager.getInstance().addItems(player, items, Reasons.KINGCITY_GETCHEST, Config.getId());
											}
											city.getSalarymap().put("getAngleDay_"+count, count);
											CountryManager.getInstance().savekingcity(city);
											MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("领取成功"));
										} else {
											MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("您的包裹已满，请清理后再来领取！"));
										}
								} else {
											MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("没有相关的物品配置"));
								}	
						} else {
									MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("未达到领取条件！"));
						}
					} else {
						MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("盟主才能领取奖励！"));
					}
		} else {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("只有城主才能领取！"));
		}
...
//									}
	...
	public void giveRewarded(Player player, FruitReward fruitReward, int type) {
		String rewardedname = ResManager.getInstance().getString("每日俸禄奖励");
...
//			itemname = ResManager.getInstance().getString("钻石");
