...
//	108		1001|5|2000;	地宫至宝道具奖励（ID|数量|几率）
	...
	public void epalaceReward(Player player,int type){
		...
		if (type == 1004) {
			...
			int money = ManagerPool.dataManager.q_globalContainer.getMap().get(105).getQ_int_value();//金币;
			int zhenqi = ManagerPool.dataManager.q_globalContainer.getMap().get(106).getQ_int_value();//真气;
			int lijing = ManagerPool.dataManager.q_globalContainer.getMap().get(107).getQ_int_value();//绑钻;

			int key = ManagerPool.dataManager.q_characterContainer.getKey(player.getJob(), player.getLevel());
			Q_characterBean basisdb = ManagerPool.dataManager.q_characterContainer.getMap().get(key);
			ResEpalaceRewardInfoToClientMessage rewardinfomsg=new ResEpalaceRewardInfoToClientMessage();
			if (rndtyoe > 0) {
				rewardinfomsg.setType(rndtyoe);
			}else {
					...
					if (msg.getTaskid() == treasureHuntTask.getModelId()) {
						if (data.getQ_usegold() > 0) {
							if (ManagerPool.backpackManager.checkGold(player, data.getQ_usegold())) {
								...
								MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，完成此任务需要{1}钻石"), data.getQ_usegold()+"");
								return;
							}
						}else {
							MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，此任务不能使用钻石立即完成功能"));
							return;
						}
