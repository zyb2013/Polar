...
//	private static final byte RIDE_TIME=5;	//骑乘需要的时间(秒)
	...
	private static  short LAYER_MAX=150;	//目前开放的最高等级
	...
	public void  stChangeRidingState( Player player ,  byte status ,  short currency  ){
		...
		if (horse != null) {
			if (status == 2/* || player.getMapModelId() == 300010*/) {	//跨地图自动下马
				unride(player);
				return;
			}
			if ( horse.getStatus() == UNRIDE) {	//如果未骑乘，则上马
				autorideHorse(player, horse.getCurlayer());
			}else {			
				unride(player);
			}
		}
...
//			MessageUtil.notify_player(player, Notifys.ERROR, "游泳区内无法骑乘");
//			return;
//		}
//		Horse horse = getHorse(player);
...
//				}else {
//					horse.setCurlayer((short) cur);	//骑乘
...
//	
	...
	private void autorideHorse(Player player,int cur) {
		// 游泳中
		if (PlayerState.SWIM.compare(player.getState())) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("游泳区内无法骑乘"));
			return;
		}
		//战斗中
		if (PlayerState.FIGHT.compare(player.getState())) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("战斗中无法骑乘"));
			return;
		}
		...
		if (mapmodel != null && mapmodel.getQ_map_ride() == 1) {//检测地图是否可骑乘
			ResRidingStateMessage smsg = new ResRidingStateMessage();
			if (horse.getCurlayer() >= cur ) {
				if (cur == 0) {
					horse.setCurlayer(horse.getCurlayer());	//骑最高阶马
					smsg.setCurlayer(horse.getCurlayer());
				}else {
					horse.setCurlayer((short) cur);	//骑乘
					smsg.setCurlayer((short) cur);
				}
				...
				if (map != null) {
					PlayerDaZuoManager.getInstacne().breakDaZuo(player);
					MessageUtil.tell_round_message(player, smsg);
					ManagerPool.playerAttributeManager.countPlayerSpecailAttribute(player, PlayerAttributeType.HORSE, 0);
					//重新计算属性
					/* xiaozhuoming: 因为新的需求，不管上下马，马的属性都会保存在人的身上
				    ManagerPool.playerAttributeManager.countPlayerAttribute(player, PlayerAttributeType.HORSE);
					ManagerPool.playerAttributeManager.countPlayerAttribute(player, PlayerAttributeType.HORSE_EQUIP);
					ManagerPool.playerAttributeManager.countPlayerAttribute(player, PlayerAttributeType.HORSE_WEAPON);
					*/
					ManagerPool.playerManager.stSyncExterior(player);
					TaskManager.getInstance().action(player, Task.ACTION_TYPE_RANK, TaskEnum.HORSE_RIDE_UP, 1);
				}
...
//			MessageUtil.tell_player_message(player, smsg);
	 ...
	 * @param addexp
	 */
	public void addHorseExp(Player player, int addexp){
		Horse horse = this.getHorse(player);
		if (horse == null) {
			MessageUtil.notify_player(player, Notifys.NORMAL, ResManager.getInstance().getString("你没有坐骑."));
			return;
		}

		if (horse.getCurlayer() >= LAYER_MAX) {
			MessageUtil.notify_player(player, Notifys.NORMAL, ResManager.getInstance().getString("当前坐骑已经是最高阶."));
			return;
		}
		
		horse.setCurExp(horse.getCurExp() + addexp);
		...
		while (true){
			...
			Q_horse_basicBean horseBasic = getDBHorseBasic(horse.getCurlayer());
			if(horse.getCurExp() >= horseBasic.getQ_horse_exp()){
				horse.setCurExp(horse.getCurExp() - horseBasic.getQ_horse_exp());
				horse.setCurlayer((short)(horse.getCurlayer() + 1));
			}else{
				break;
			}
		}
	...
	public void stReqhorseReceiveMessage(Player player,ReqhorseReceiveMessage msg) {
		...
		if (horse != null) {
			if (horse.getCurlayer() == 0) {
				...
				ManagerPool.playerAttributeManager.countPlayerAttribute(player, PlayerAttributeType.HORSE);
				//自动上马
				this.autorideHorse(player, horse.getCurlayer());
			}
		}
	}
...
//					//判断升阶金币是否足够
//					boolean moneyenough = true; // 金币是否够升阶
//					int goldtong = player.getMoney();	//身上金币
////					int danjia = 0;
...
//						money = 0;
//					}
//					if ( goldtong < money) { //金币不足
//						moneyenough = false; //金币不足标记 并不立刻返回 如果满足完美坐骑丹的条件 也可继续升阶
//					}
//					HorseLog log = new HorseLog();
...
////						danjia = getItemYuanbao(oldBasic.getQ_up_item_id());	//道具钻石单价
////						if (danjia < 1) {
...
////							int ybsum = danjia*shengyu;//需要的钻石数量
////							if( gold == null || gold.getGold() < ybsum){
////								errmsg.setType((byte) 1);
////								errmsg.setErrint(ybsum);
////								MessageUtil.tell_player_message(player, errmsg);
////								MessageUtil.notify_player(player, Notifys.ERROR, "升阶停止，需要：{1}钻石才能自动购买到升阶所需的材料.",String.valueOf(ybsum));	
////								return;
////							}
////						}
//						if (horse.getLayer() >= 7) {
...
//
//					// 金币是否足够 -检查
//					if(!ck && !moneyenough){ //不满足用完美坐骑丹条件 并且 金币不足 给予提示并且返回
//						MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("需要金币：{1}，才可继续进阶."),String.valueOf(money));	
//						return;
//					}
//					
...
//							//MessageUtil.notify_player(player, Notifys.SUCCESS, "进阶成功，进阶使用了：[{1}]{2}个，[金币]{3}.",itemBean.getQ_name(),String.valueOf(oldBasic.getQ_up_item_num()),String.valueOf(oldBasic.getQ_up_gold_num()));	
...
//	}
	 ...
	 * @param type
	 */
	public void stReqhorseStageUpStartMessage(Player player, byte type) {
		if(PlayerState.DIE.compare(player.getState())) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("处于死亡状态，无法进行坐骑升级"));
			return;
		}
		...
		if (horse.getCurlayer() >= LAYER_MAX) {
			MessageUtil.notify_player(player, Notifys.NORMAL, ResManager.getInstance().getString("当前坐骑已经是最高级"));
			return;
		}

		int nextLayer = horse.getCurlayer() + 1;
		Q_horse_basicBean nextHorseBasic = getDBHorseBasic(nextLayer); //下一级坐骑的模板
		Q_horse_basicBean curHorseBasic = getDBHorseBasic(horse.getCurlayer()); //当前坐骑的模板
		if (curHorseBasic == null || nextHorseBasic == null) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("坐骑信息出错"));
			return;
		}

		if(curHorseBasic.getQ_up_figure_level() > player.getLevel()) {
			ReshorseErrInfoMessage errmsg = new ReshorseErrInfoMessage();
			errmsg.setType((byte) 3);
			MessageUtil.tell_player_message(player, errmsg);
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("人物等级达到[{1}]级后，才可继续升级"),
					String.valueOf(curHorseBasic.getQ_up_figure_level()));
			return;
		}

		Q_itemBean itemBean = ManagerPool.dataManager.q_itemContainer.getMap().get(curHorseBasic.getQ_up_item_id());
		if(itemBean == null) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("物品信息出错"));
			return;
		}
		...
		long action = Config.getId();
		HorseLog log = new HorseLog();
		int itemNum = ManagerPool.backpackManager.getItemNum(player, curHorseBasic.getQ_up_item_id());
		
		if(type == 0) { //普通驯养
			if (itemNum < curHorseBasic.getQ_up_item_num()) {
				ReshorseErrInfoMessage errmsg = new ReshorseErrInfoMessage();
				errmsg.setType((byte) 2);
				MessageUtil.tell_player_message(player, errmsg);
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("需要{1}[{2}颗]，才可继续升级"),
						itemBean.getQ_name(), String.valueOf(curHorseBasic.getQ_up_item_num()));	
				return;
			}
			
			if(ManagerPool.backpackManager.removeItem(player,curHorseBasic.getQ_up_item_id(),curHorseBasic.getQ_up_item_num(),Reasons.HORSE_UP,action) == false){
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("移除物品{1}出错"), itemBean.getQ_name());
				return;
			}
			
			addExp = ManagerPool.backpackManager.getEffictValue(EffectType.ADDHORSEEXP, itemBean) * curHorseBasic.getQ_up_item_num();
			log.setItemnum(curHorseBasic.getQ_up_item_num());
			log.setItemid(curHorseBasic.getQ_up_item_id());
			MessageUtil.notify_player(player, Notifys.HORSE, String.format("+<font color='#FCF600'>%d</font>坐骑经验", addExp));
		}else if(type == 1) { //自动驯养
			if(itemNum >= curHorseBasic.getQ_up_item_num()) {
				if(ManagerPool.backpackManager.removeItem(player,curHorseBasic.getQ_up_item_id(),curHorseBasic.getQ_up_item_num(),Reasons.HORSE_UP,action) == false){
					MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("移除物品{1}出错"), itemBean.getQ_name());
				...
				Q_globalBean globalBean = null;
				int vipLevel = ManagerPool.vipManager.getVIPLevel(player);
				Q_vipBean q_vipBean = ManagerPool.dataManager.q_vipContainer.getMap().get(vipLevel);
				
				if(curHorseBasic.getQ_up_item_id() == 100016) {
					globalBean = ManagerPool.dataManager.q_globalContainer.getMap().get(CommonConfig.HORSE_STAGE_UP_GEM_LOW.getValue());
				}else if(curHorseBasic.getQ_up_item_id() == 100015){
					globalBean = ManagerPool.dataManager.q_globalContainer.getMap().get(CommonConfig.HORSE_STAGE_UP_GEM_MIDDLE.getValue());
				}else if(curHorseBasic.getQ_up_item_id() == 100014) {
					if(vipLevel < 6 || q_vipBean == null || q_vipBean.getQ_advanced_horse_strengthen() == 0) {
						return;
					}
					
					globalBean = ManagerPool.dataManager.q_globalContainer.getMap().get(CommonConfig.HORSE_STAGE_UP_GEM_HIGH.getValue());
				}else if(curHorseBasic.getQ_up_item_id() == 100013) {
					if(vipLevel < 8 || q_vipBean == null || q_vipBean.getQ_super_horse_strengthen() == 0) {
						return;
					}
					
					globalBean = ManagerPool.dataManager.q_globalContainer.getMap().get(CommonConfig.HORSE_STAGE_UP_GEM_SUPER.getValue());
				}
				
				if(globalBean == null) {
					MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("坐骑配置信息出错"));
					return;
				}
				
				gem = globalBean.getQ_int_value() * curHorseBasic.getQ_up_item_num(); //花费的钻石数量
				int gold = player.getGold() == null ? 0 : player.getGold().getGold(); //身上钻石
				if(gold < gem) {
					MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("需要钻石：{1}，才可继续升级"), String.valueOf(gem));	
					return;
				}
				
				if(ManagerPool.backpackManager.changeGold(player, -gem, Reasons.HORSE_UP, action) == false) { //扣钱
			...
			logx.error("stReqhorseStageUpStartMessage type error! type = " + type);
			return;
		}
		
		/*xiaozhuoming: 现在是自动驯养
		else if(msg.getType() == 1) { //钻石驯养
			boolean canStrengthHorse = ManagerPool.vipManager.canStrengthHorseByGold(player);
			if (!canStrengthHorse) {
				MessageUtil.notify_player(player, Notifys.NORMAL, ResManager.getInstance().getString("只有vip才能使用钻石提升坐骑哦."));
				return;
			}
			
			Q_globalBean golbalBean_num = ManagerPool.dataManager.q_globalContainer.getMap().get(CommonConfig.HORSE_STAGE_UP_GEM_COUNT.getValue()); 
			gem = golbalBean_num.getQ_int_value(); //每次使用的钻石数量
			int gold = player.getGold() == null ? 0 : player.getGold().getGold();// 身上钻石
			if(gold < gem) {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("需要钻石：{1}，才可继续升级."), String.valueOf(gem));	
				return;
			...
			if(ManagerPool.backpackManager.changeGold(player, -gem, Reasons.HORSE_UP, action) == false) { //扣钱
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("扣钱失败."));	
				return;
			}
				
			log.setZuanshi(gem);
			Q_globalBean golbalBean_exp = ManagerPool.dataManager.q_globalContainer.getMap().get(CommonConfig.HORSE_STAGE_UP_GEM_EXP.getValue()); 
			addExp = golbalBean_exp.getQ_int_value(); //钻石驯养坐骑升级增加的经验
			issuccess = true;
			MessageUtil.notify_player(player, Notifys.NORMAL, String.format("钻石驯养获得<font color='#FCF600'>%d</font>经验", addExp));
		}else if(msg.getType() == 2) { //一键驯养
			boolean isVIP = ManagerPool.vipManager.isVIP(player);
			boolean canStrengthHorse = ManagerPool.vipManager.canStrengthHorseByGold(player);
			if(isVIP == false || canStrengthHorse == false) {
				MessageUtil.notify_player(player, Notifys.NORMAL, ResManager.getInstance().getString("您当前的VIP等级不能使用钻石提升坐骑."));
				return;
			}
			
			int upLayer = msg.getUpLayer();
			Q_horse_basicBean upHorseBasic = getDBHorseBasic(upLayer);
			if (upHorseBasic == null) {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("坐骑信息出错."));
				return;
			}
			
			for(int i=horse.getCurlayer(); i<upLayer; i++) {
				Q_horse_basicBean temp = getDBHorseBasic(i);
				addExp += temp.getQ_horse_exp();
			}
			
			if(addExp > 0 && addExp > horse.getCurExp()) {
				addExp = addExp - horse.getCurExp();
			}
			
			Q_globalBean golbalBean_num = ManagerPool.dataManager.q_globalContainer.getMap().get(CommonConfig.HORSE_STAGE_UP_GEM_COUNT.getValue());
			Q_globalBean golbalBean_exp = ManagerPool.dataManager.q_globalContainer.getMap().get(CommonConfig.HORSE_STAGE_UP_GEM_EXP.getValue());
			gem = (int) Math.ceil(addExp * ((double) golbalBean_num.getQ_int_value() / golbalBean_exp.getQ_int_value())); //需要的钻石数量
			int gold = player.getGold().getGold(); //身上钻石
			if(gold < gem) {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("需要钻石：{1}，才可继续进阶."), String.valueOf(gem));	
				return;
			}
			...
			log.setZuanshi(gem);
			issuccess = true;
			MessageUtil.notify_player(player, Notifys.NORMAL, String.format("一键驯养获得<font color='#FCF600'>%d</font>经验，坐骑提升1阶", addExp));
		}*/
		
		int dayupnum = 0; //升级次数
		int newExp = horse.getCurExp() + addExp;
		int maxExp = curHorseBasic.getQ_horse_exp();
		while(maxExp > 0 && newExp >= maxExp) {
			newExp = newExp - maxExp;
			nextHorseBasic = getDBHorseBasic(nextLayer ++);
			if(nextHorseBasic == null) {
				newExp = 0;
				break;
			}
			
			maxExp = nextHorseBasic.getQ_horse_exp();
			horse.setCurlayer((short)(horse.getCurlayer() + 1));
			horse.setLayer(horse.getCurlayer());
			horse.setHorselevel(horse.getCurlayer());
			dayupnum ++;
			if(nextHorseBasic.getQ_notice() == 1) { //全服公告
				ParseUtil parseUtil = new ParseUtil();
				parseUtil.setValue(
						String.format(ResManager.getInstance().getString("恭喜玩家【%s】将【%s】升阶至：【%s】战斗力获得大量提升！{@}"), player.getName(), curHorseBasic.getQ_name(),
								nextHorseBasic.getQ_name()), new ParseUtil.VipParm(VipManager.getInstance().getVIPLevel(player), GuideType.HORSE.getValue()));
				MessageUtil.notify_All_player(Notifys.CHAT_BULL, parseUtil.toString(), new ArrayList<GoodsInfoRes>(), 
						GuideType.HORSE.getValue());
			}
		}
		
		log.setDayupnum(dayupnum);
		horse.setCurExp(newExp);
		horse.setDayblessvalue(0);
		horse.setDayupnum(0);
		horse.setHisblessvalue(0);
		horse.setHisupnum(0);
		...
		ReshorseStageUpResultMessage smsg = new ReshorseStageUpResultMessage();
		smsg.setType((byte) 1);
		smsg.setExp(horse.getCurExp());
		smsg.setCurlayer(horse.getCurlayer());
		smsg.setItemmodelid(curHorseBasic.getQ_up_item_id());
		smsg.setItemnum(curHorseBasic.getQ_up_item_num());
		smsg.setMoney(gem);
		MessageUtil.tell_player_message(player, smsg);
		stHorseLog(player, smsg, log);
		
		//通知任务
		TaskManager.getInstance().action(player, Task.ACTION_TYPE_SPECIAL, TaskEnum.HORSE_STAGE_UP, horse.getCurlayer());
		IHorseUpScript script = (IHorseUpScript)ManagerPool.scriptManager.getScript(ScriptEnum.HORSE_UP);
		if(script != null) {
			try {
				script.onHorseUp(player, (byte)1, horse.getCurlayer());
			}catch (Exception e) {
				logx.error(e, e);
			}
		}
...
//		log.setFailexp(smsg.getExp());
		...
		log.setCurlayer(horse.getCurlayer());
	...
	public List<HorseSkill> selectActSkill(Player player){
		...
		if (horse.getCurlayer() > 0) {
			...
			if (horse.getMixingbone() >= 50 && horse.getLayer() > 9) {
			...
			for (HorseSkill horseSkill : skills) {
				Q_horse_skillsBean skillModel = getDBHorseDataSkill(horseSkill.getSkillModelId());
				if (skillModel.getQ_activate_need_layer() <= horse.getCurlayer()) {	//达到激活技能所需等级
					skilltab.add(horseSkill);
				}
			}
			return skilltab;
		...
		if(horse.getLayer() >= activationlv){
			if (horse.getBoxnum() < upmax) {
				...
				if (ManagerPool.backpackManager.changeMoney(player, -money,Reasons.HORSE_LG,action)) {
					...
					stReqOpenSkillUpPanelMessage(player);//发送下一次消耗的金币和钻石
					MessageUtil.notify_player(player,Notifys.CHAT_SYSTEM,ResManager.getInstance().getString("拉杆消耗{1}金币。"),String.valueOf(money));
					return;
				}else {
					MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，拉杆所需消耗的金币不足,需要{1}金币"),String.valueOf(money));	
				}
			}else {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，请等待冷却时间结束."));	
			}
...
//				Gold gold = ManagerPool.backpackManager.getGold(player);
				if(ManagerPool.backpackManager.checkGold(player, xyb)) {
					...
					stHorseSkillLog(player,log);
					stReqOpenSkillUpPanelMessage(player);//发送下一次消耗的金币和钻石
					MessageUtil.notify_player(player,Notifys.SUCCESS,ResManager.getInstance().getString("幸运连珠消耗{1}钻石。"),String.valueOf(xyb));
					return;
				}else {
					MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，幸运连珠所需消耗的钻石不足,需要{1}钻石。"),String.valueOf(xyb));
				}
			}else {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，请等待冷却时间结束."));	
			}
	...
	public int getUpYuanbao(Player player){
		/* xiaozhuoming: 暂时不需要
		Horse horse = getHorse(player);
		// 根据玩家当前最大所激活的技能ID，进入坐骑技能数据库中“历次拉动消耗钻石（XXXX;xxxx;依次类推）”字段中查出来的值  * （1 - 服务器开放天数 * 坐骑技能经验消耗钻石衰减系数）
		HorseSkill maxskill = getMaxActSkill(player);
		Q_horse_skillsBean skillModel = ManagerPool.dataManager.q_horse_skillsContainer.getMap().get(maxskill.getSkillModelId());
	...
	public void sethorselevel(Player player, boolean sync){
		Horse horse = getHorse(player);
		int  curlayer = horse.getCurlayer();
		Q_horse_basicBean db = getDBHorseBasic(curlayer);
...
//		Gold gold = ManagerPool.backpackManager.getGold(player);
		...
		if(ManagerPool.backpackManager.checkGold(player, yb)) {
			...
			horse.setBoxnum(0);
			stResHorseInfo(player);
			MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("您使用{1}钻石清除冷却时间."),String.valueOf(yb));	
		}else {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，需要{1}钻石才能清除冷却时间."),String.valueOf(yb));
			ReshorseErrInfoMessage errmsg =new ReshorseErrInfoMessage();
	...
	public int getClearCDYuanBao(Player player){
		/* xiaozhuoming: 暂时不需要
		//（1 + 玩家今日已清除技能经验获取的升级冷却时间次数） * “清除坐骑升级经验冷却次数所需消耗钻石数倍率” * “每日清除坐骑升级经验冷却次数所需消耗钻石数”
		Horse horse = getHorse(player);
