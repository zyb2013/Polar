...
public class MeltingManager {
	...
	public int getTrueAttrType(int attrtype) {
		...
		switch (attrtype) {
			...
			case DEFENSE_TYPE:
				...
				break;
			case CRIT_TYPE:
				result = Attributes.PERFECT_ATTACKPERCENT.getValue();
				break;
			case DODGE_TYPE:
	...
	public int getFalseAttrType(int trueattrtype) {
		...
		if (trueattrtype == Attributes.ATTACK.getValue()) {
			...
			result = DEFENSE_TYPE;
		} else if (trueattrtype == Attributes.PERFECT_ATTACKPERCENT.getValue()) {
			result = CRIT_TYPE;
	...
	public void reqMeltingItemToServer(Player player, ReqMeltingItemToServerMessage message) {
		...
		if (needgold == 0) {
			/* xuliang
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("熔炼所需钻石数据错误"));
			*/
			return;
		}
		if (!BackpackManager.getInstance().checkGold(player, needgold)) {
			/* xuliang
			MessageUtil.notify_player(player, Notifys.MOUSEPOS, ResManager.getInstance().getString("熔炼所需钻石不足"));
			*/
			return;
		}
		...
		if (needmoney == 0) {
			/* xuiang
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("熔炼所需金币数据错误"));
			*/
			return;
		}
		if (player.getMoney() < needmoney) {
			/* xuliang
			MessageUtil.notify_player(player, Notifys.MOUSEPOS, ResManager.getInstance().getString("您的金币不足，建议您开通VIP每日可免费领取大量金币"));
			*/
			return;
		}
