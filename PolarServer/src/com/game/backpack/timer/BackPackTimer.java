...
//				}
			...
			// Q_backpack_gridBean StoreConfig =
			// DataManager.getInstance().q_backpack_gridContainer.getMap().get(Global.STORE_TYPE+"_"+(storeCellsNum+1));
			...
			// StoreManager.getInstance().cellTimeQuery(player, storeCellsNum+1,
			// 0);
...
//					}
			...
			// Notifys.CHAT_SYSTEM,"成功扩容仓库到{1}个!奖励经验{2}",(storeCellsNum+1)+"",StoreConfig.getQ_exp()+"");
			// // try{
			// // CellOpenLog log=new CellOpenLog();
			// // log.setAction((byte)0);
