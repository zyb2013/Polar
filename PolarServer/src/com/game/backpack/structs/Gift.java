...
//				if (player.getSex() == q_itemBean.getQ_sex() || q_itemBean.getQ_sex() == 0) {
					if (q_itemBean.getQ_gift() != 0) {
						if (q_itemBean.getQ_gift() == CountryAwardManager.KINGCITYCHEST) {
							if (num > 1) {
								MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("圣盟宝箱不能批量使用！"));
								num = 1;
							}
							...
							MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("您的圣盟宝箱打开成功，获得了物品"));
						} else {
