...
//	public static int explimitgrade=40;
	...
	public void use(Player player, String... parameters) {
		...
		if (model.getQ_secondary_type() == 1 && player.getGuildId() == 0) {
			// 限制只能有战盟才能使用
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("你还没加入战盟"));
			return;
		}
		//冷却判定
		...
		for (String buffidstr : buffs) {
			...
			if(buff.getQ_action_type() == BuffType.MP){
				...
				if(map != null && map.getBanusemp() != 0){
					MessageUtil.notify_player(player, Notifys.MOUSEPOS, "本地图禁止使用加魔法道具");
					return;
				}
			...
			if (buff.getQ_action_type() == BuffType.HP) {
				...
				//add by xuliang
				Map map = ManagerPool.mapManager.getMap(player);
				if(map != null && map.getBanusehp() != 0){
					MessageUtil.notify_player(player, Notifys.MOUSEPOS, "本地图禁止使用加生命道具");
					return;
				}
				// spousenum = buff.getQ_effect_value();
