...
import com.game.backpack.log.SpiritChangeLog;
...
import com.game.backpack.message.ResSpiritChangeMessage;
...
import com.game.newactivity.NewActivityEnum;
...
public class BackpackManager {
	...
	//
	private boolean isopentmpgold = true;// 是否开启临时钻石
	...
	public boolean addItem(Player player, Item item, Reasons resons, long action) {
		...
		if (model.getQ_max() == 1) 
		{
			...
			// 不存在叠加问题
			// Item item2 =
			// player.getBackpackItems().get(String.valueOf(item.getGridId()));
			if (item.getGridId() <= 0) {
			...
			//hongxiao.z 前端新表现添加 2014-1-16
			showAs_1(player, resons, item, model.getQ_type(), model.getQ_auto_use());
			...
			if (model.getQ_type() == 11 && resons.equals(Reasons.TAKEUP)) {
				log.error("玩家(" + player.getId() + ")" + model.getQ_name()
						+ "任务物品加入包裹成功" + action);
			}
		} else {
			// 可堆叠物品
			...
			for (int i = 1; i <= player.getBagCellsNum() && count > 0; i++) {
				...
				item.setGridId(target.getGridId());
...
//				}
			...
			if (count > 0) 
			{
				...
				item.setNum(count);
				player.getBackpackItems().put(String.valueOf(item.getGridId()),
						item);
				MessageUtil.tell_player_message(player, getItemAddMessage(item, resons));
				if (model.getQ_type() == 11 && resons.equals(Reasons.TAKEUP)) 
				{
					log.error("玩家(" + player.getId() + ")" + model.getQ_name()
							+ "任务物品加入包裹检查通过" + action + "位置35");
				}
				// String after=JSONserializable.toString(target);
				...
				// {
				// ResGetItemReasonsMessage sendMessage = new
				// ResGetItemReasonsMessage();
				// ItemReasonsInfo itemReasonsInfo = new ItemReasonsInfo();
		...
		//判断是否自动使用 hongxiao.z add
		if(model.getQ_auto_use() == 1)
		{
			useItem(player, item.getId(), item.getNum());
			return true;
		}
	 ...
	 * @return
	 */
	public boolean addItems(Player player, Collection<Item> items,
			Reasons reasons, long action) {
	...
	public void moveItem(Player player, long itemId, int num, int toGridId) {
		...
		if (player.getBackpackItems().get(String.valueOf(toGridId)) == null) {
			...
			if (num == 0 || num >= item.getNum()) {
				...
				if (emptyGridNum < 1) {
					MessageUtil.notify_player(player, Notifys.ERROR, ResManager
							.getInstance().getString("包裹空间不足,移动失败"));
					return;
				}
				if (toGridId == -1) {
					toGridId = getBackpackFirstEmptGridId(player);
				}
				player.getBackpackItems().remove(
				...
				MessageUtil.tell_player_message(player,
						getItemChangeMessage(item));
				try {
					Q_itemBean model = DataManager.getInstance().q_itemContainer
							.getMap().get(item.getItemModelId());
					if (model.getQ_log() == 1) {
				...
				if (player.getBackpackItems().get(String.valueOf(toGridId)) != null
						&& player.getBackpackItems()
								.get(String.valueOf(toGridId)).isTrade()) {
					MessageUtil.notify_player(player, Notifys.ERROR, ResManager
							.getInstance().getString("物品正在交易中,不可以执行该操作"));
					return;
				}
				try {
					if (toGridId == -1) {
						toGridId = getBackpackClosestEmptGridId(player,
								item.getGridId());
					}
					if (toGridId == -1) {
					...
					Item togrid = BackpackManager.getInstance()
							.getItemByCellId(player, toGridId);
					if (togrid != null) {
						// 目标格子不为空 不允许 拆分
						return;
					}
					...
					player.getBackpackItems().put(
							String.valueOf(clone.getGridId()), clone);
					MessageUtil.tell_player_message(player,
							getItemAddMessage(clone, null));
					// 拆分
					...
					try {
						...
						if (model.getQ_log() == 1) {
							...
							addlog.setChangeAction(ItemChangeAction.ADD
									.toString());
							addlog.setItemafterInfo(JSONserializable
									.toString(clone));
							...
							addlog.setRolename(player.getName());
							addlog.setUsername(player.getUserName());
							addlog.setRolelevel(player.getLevel());
							...
							log.setActionId(actionId);
							log.setAction(ItemAction.MOVE.toString());
							log.setChangeAction(ItemChangeAction.CHANGE
									.toString());
							log.setItemafterInfo(JSONserializable
									.toString(item));
							log.setItembeforeInfo("");
							log.setItemid(item.getId());
							log.setModelid(item.getItemModelId());
			...
			Q_itemBean model = ManagerPool.dataManager.q_itemContainer.getMap()
					.get(item.getItemModelId());
			Item toGrid = player.getBackpackItems().get(
					String.valueOf(toGridId));
			if (toGrid.isTrade()) {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager
						.getInstance().getString("物品正在交易中"));
				return;
			...
			if (isMergeAble(item, toGrid)) {
				if (item.getNum()
						+ player.getBackpackItems()
								.get(String.valueOf(toGridId)).getNum() <= model
							.getQ_max()) {
					String toGridbefore = JSONserializable.toString(toGrid);
					// 完全合并
					// 合并
					toGrid.setNum(item.getNum() + toGrid.getNum());
					MessageUtil.tell_player_message(
							player,
							getItemChangeMessage(player.getBackpackItems().get(
									String.valueOf(toGridId))));
					// 移除原来物品
					player.getBackpackItems().remove(
							String.valueOf(item.getGridId()));
					MessageUtil.tell_player_message(player,
							getItemRemoveMessage(item));
					...
					try {
						if (model.getQ_log() == 1) {
							...
							removelog.setChangeAction(ItemChangeAction.REMOVE
									.toString());
							...
							removelog.setRolename(player.getName());
							removelog.setUsername(player.getUserName());
							removelog.setRolelevel(player.getLevel());
							log.setSid(player.getCreateServerId());
					...
					try {
						if (model.getQ_log() == 1) {
							...
							removelog.setActionId(actionId);
							removelog.setAction(ItemAction.MOVE.toString());
							removelog.setChangeAction(ItemChangeAction.CHANGE
									.toString());
							removelog.setItemafterInfo(JSONserializable
									.toString(item));
				...
				player.getBackpackItems().remove(
						String.valueOf(item.getGridId()));
				player.getBackpackItems().remove(
						String.valueOf(toGrid.getGridId()));
				item.setGridId(toGrid.getGridId());
				toGrid.setGridId(tmpgrid);
				player.getBackpackItems().put(String.valueOf(item.getGridId()),
						item);
				player.getBackpackItems().put(
						String.valueOf(toGrid.getGridId()), toGrid);
				MessageUtil.tell_player_message(player,
						getItemChangeMessage(item));
				MessageUtil.tell_player_message(player,
						getItemChangeMessage(toGrid));
				try {
					if (model.getQ_log() == 1) {
						...
						removelog.setActionId(actionId);
						removelog.setAction(ItemAction.MOVE.toString());
						removelog.setChangeAction(ItemChangeAction.MOVE
								.toString());
						removelog.setItemafterInfo(JSONserializable
								.toString(item));
						removelog.setItembeforeInfo(itembefore);
	...
	public boolean hasAddSpace(Player player, Item item) {
		...
		if (emptyGridNum > 0) {
			...
			Q_itemBean q_itemBean = DataManager.getInstance().q_itemContainer
					.getMap().get(item.getItemModelId());
			if (q_itemBean == null) {
	...
	public boolean hasAddSpace(Player player, Collection<Item> items) {
		...
		for (Item item : items) {
			...
			String key = item.getItemModelId() + "_" + item.getLosttime() + "_"
					+ item.isBind();
	...
	private int getCombinNum(Player player, Item item) {
		int count = 0;
		Collection<Item> allItem = BackpackManager.getInstance().getAllItem(
				player);
		Q_itemBean model = DataManager.getInstance().q_itemContainer.getMap()
				.get(item.getItemModelId());
	...
	public void useItem(Player player, long itemId, int num) {
		...
		if(checkItem(player,item,num)) {
			if (model.getQ_bind() == 2) {
				item.setBind(true);
			}
			 if(model.getQ_use_price() != 0) {
				if(!changeGold(player, -model.getQ_use_price(), Reasons.GOODUSE, Config.getId())) {
					log.error("使用物品时扣除钻石失败："+player.getId()+","+model.getQ_id());
					return;
				}
			}
			item.use(player, num + "");
	...
	public boolean checkItem(Player player,Item item, int num) {
		Q_itemBean model = DataManager.getInstance().q_itemContainer.getMap()
				.get(item.getItemModelId());
		if (model == null) {
			return false;
		...
		if (num > 1 && model.getQ_max() <= 1) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("很抱歉，该位置物品数量不足，该物品不支持批量使用"));
			return false;
		}
		if (num > item.getNum()) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("很抱歉，该位置物品数量不足，请求数量{1}当前数量{2}"),
					num + "", item.getNum() + "");
			// 请求非法
			return false;
		}
		...
		if (num > 1 && model.getQ_whether_batch() != 1) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("很抱歉，该物品不支持批量使用"));
			return false;
		}
		 ...
		 * if (model.getQ_sex() != 0 && model.getQ_sex() != player.getSex()) {
			// 装备性别不符合
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("很抱歉，该物品限{1}性使用"), model
					.getQ_sex() == 1 ? ResManager.getInstance().getString("男")
					: ResManager.getInstance().getString("女"));
			return;
		}*/
		...
		if (player.getLevel() < model.getQ_level()) {
			// 装备等级不符合
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("很抱歉，该物品需要人物等级达到{1}后才可以使用"),
					String.valueOf(model.getQ_level()));
			return false;
		}
		if (item.isLost()) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("很抱歉，该物品已过期"), String
					.valueOf(model.getQ_level()));
			return false;
		}
		if (item.isTrade()) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
		...
		if (checkHaveEffict(EffectType.ADDEXP, model)
				&& PlayerManager.getInstance().isTopLevel(player)) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("很抱歉，您的等级已达到最高无法使用该用品"));
			return false;
		}
		// 检查真气上限
		// if (checkHaveEffict(EffectType.ADDZHENQI, model)
		// && PlayerManager.getInstance().isFullZq(player)) {
		// MessageUtil.notify_player(player, Notifys.ERROR, ResManager
		// .getInstance().getString("很抱歉，您的真气已满无法使用该用品"));
		// return;
		// }
		if (checkHaveEffict(EffectType.ADDHP, model)
				&& PlayerManager.getInstance().isFullHp(player)) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，您的生命已满无法使用该用品"));
			return false;
		}
		if (checkHaveEffict(EffectType.ADDMP, model)
				&& PlayerManager.getInstance().isFullMp(player)) {
			/* xuliang 暂时屏蔽
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，您的魔法已满无法使用该用品"));
			*/
			return false;
		}
/*		
 * 	luminghua hide
 * if (checkHaveEffict(EffectType.ADDSP, model)
				&& PlayerManager.getInstance().isFullSp(player)) {
		...
		if (canuse > 0) {
			long alreadynum = ManagerPool.countManager.getCount(player,
					CountTypes.ITEM_USE, String.valueOf(item.getItemModelId()));
			if (alreadynum >= canuse || (num > canuse - (int) alreadynum)) {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("该物品使用次数超过上限。"));
				return false;
			} else {
		...
		//增加新的约束条件：职业，力量，敏捷，智力，体力
		int job=player.getJob();
		if (!StringUtils.isBlank(model.getQ_job_limit())) {
			boolean canUse = PlayerManager.checkJob(job, model.getQ_job_limit());
			if (!canUse) {
				// 职业不满足
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("职业不满足，不能使用。"));
				return false;
			}
		}
		int length=player.getAttibute_one()[0];
		if(length<model.getQ_str_limit()){
			//力量不够
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("力量不够，不能使用。"));
			return false;
		}
		int vitality=player.getAttibute_one()[1];
		if(vitality<model.getQ_vit_limit()){
			//体力
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("体力不够，不能使用。"));
			return false;
		}
		int agile=player.getAttibute_one()[2];
		if(agile<model.getQ_dex_limit()){
			//敏捷不够
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("敏捷不够，不能使用。"));
			return false;
		}
		int intelligence=player.getAttibute_one()[3];
		if(intelligence<model.getQ_int_limit()){
			//智力不够
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("智力不够，不能使用。"));
			return false;
		}
		if(!checkGold(player, model.getQ_use_price())) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("钻石不够，不能使用。"));
			return false;
		}
		return true;
	}
	...
	private boolean checkHaveEffict(int type, Q_itemBean model) {
		if (model.getQ_effict_type() == null
				|| model.getQ_effict_type().equals("")) {
			return false;
		}
	 ...
	 * @return
	 */
	public int getEffictValue(int type, Q_itemBean model) {
		if (model.getQ_effict_type() == null
				|| model.getQ_effict_type().equals("")) {
			return 0;
		}
				
		String effictType = model.getQ_effict_type();
		effictType = effictType.replace("[", "");
		effictType = effictType.replace("]", "");
		String[] effictTypeArr = effictType.split(Symbol.DOUHAO_REG);
		
		int index = -1;
		for (int i=0; i<effictTypeArr.length; i++) {
			int parseInt = Integer.parseInt(effictTypeArr[i]);
			if (parseInt == type) {
				index = i;
				break;
			}
		...
		String effictValue = model.parseEffict_value();
		effictValue = effictValue.replace("[", "");
		effictValue = effictValue.replace("]", "");
		String[] effictValueArr = effictValue.split(Symbol.DOUHAO_REG);
		if(index >= 0 && index < effictValueArr.length) {
			return Integer.parseInt(effictValueArr[index]);
		}
	 ...
	 *            物品id
	 */
	public boolean removeItem(Player player, long itemId, Reasons reasons,
			long action) {
		// 获得要移动物品
		Item item = getItemById(player, itemId);
		if (item == null) {
	 ...
	 *  @param action          
	 */
	public boolean removeItem(Player player, int itemModelId, int num,
			Reasons reasons, long action) {
		if (num <= 0) {
			return false;
		}
		...
		// DataManager.getInstance().q_itemContainer.getMap().get(itemModelId);
		...
		if (binditemNum > 0) {
			if (binditemNum >= tmpnum) {
				...
				// "失去物品{1}", q_itemBean.getQ_name());
				// } else {
				// MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM,
				// "失去物品{1}({2})", q_itemBean.getQ_name(), tmpnum + "");
				// }
			} else {
				removeItem(player, itemModelId, binditemNum, true, reasons,
						actionId);
				tmpnum -= binditemNum;
				...
				// } else {
				// MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM,
				// "失去物品{1}({2})", q_itemBean.getQ_name(), binditemNum + "");
				// }
			}
		}
		if (itemNum > 0) {
			if (itemNum >= tmpnum) {
				removeItem(player, itemModelId, tmpnum, false, reasons,
						actionId);
				tmpnum = 0;
				// if (tmpnum == 1) {
				// MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM,
		...
		TaskManager.getInstance().action(player, Task.ACTION_TYPE_GOODS,
				itemModelId, -num);
		ManagerPool.playerManager.savePlayer(player);
		return true;
	}
	 ...
	 */
	public boolean removeItem(Player player, int itemModelId, int num,
			boolean bind, Reasons reasons, long action) {
		...
		for (int i = player.getBagCellsNum(); i >= 1 && num > 0; i--) {
			...
			Q_itemBean model = DataManager.getInstance().q_itemContainer
					.getMap().get(itemModelId);
			if (item.isBind() == bind && item.getItemModelId() == itemModelId
					&& !item.isLost() && !item.isTrade()) {
				if (item.getNum() <= num) {
		...
		// DataManager.getInstance().q_itemContainer.getMap().get(itemModelId);
		// if (num == 1) {
		// MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM, "失去物品{1}",
		// q_itemBean.getQ_name());
		// } else {
		// MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM,
		// "失去物品{1}({2})", q_itemBean.getQ_name(), num + "");
		// }
		return true;
	}
	 ...
	 * @param num
	 */
	public boolean removeItem(Player player, Item item, int num,
			Reasons reasons, long action) {
		if (item.getNum() < num) {
			// 数量不足
		...
		if (item.isTrade()) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("操作失败，物品正在交易中"));
			return false;
		}
		if (item.isLost()) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("操作失败，物品己过期"));
			return false;
		}
		long actionId = action != 0 ? action : Config.getId();
		if (item.getNum() == num) {
			...
			item.setNum(item.getNum() - num);
			// Q_itemBean q_itemBean =
			// DataManager.getInstance().q_itemContainer.getMap().get(item.getItemModelId());
			// if(num==1){
	 ...
	 * @param cellId
	 */
	public boolean removeItemByCellId(Player player, int cellId,
			Reasons reasons, long action) {
		Item item = getItemByCellId(player, cellId);
		if (item == null) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("指定位置的物品不存在"));
			return false;
		}
		if (reasons != Reasons.JIAOYIITEM && item.isTrade()) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("物品正在被交易"));
			return false;
		}
	...
	public Item getFirstItemByModelId(Player player, int itemModelId) {
		...
		while (iter.hasNext()) {
			Item item = (Item) iter.next();
			if (item.getItemModelId() == itemModelId && !item.isTrade()
					&& !item.isLost() && item.isBind()) {
				return item;
			}
		}
		while (iter.hasNext()) {
	 ...
	 *            玩家id
	 */
	private void sendSpiritInfo(Player player) 
	{
		ResSpiritChangeMessage msg = new ResSpiritChangeMessage();
		msg.setSpirit(player.getSpirit());
	...
	public void sendCanreceiveYuanbaoInfo(Player player) {
		...
		MessageUtil.tell_player_message(player, ybmsg);// 发送可领取钻石
	}
	 ...
	 * @return
	 */
	public int getBackpackAbleAddGridId(Player player, int itemModelId,
			int num, boolean isbind, long lostTime) {
		// 获取物品模型
		Q_itemBean model = ManagerPool.dataManager.q_itemContainer.getMap()
				.get(itemModelId);
		if (model.getQ_max() > 1) {
	...
	public int getBackpackAbleAddGridId(Player player, Item item) {
		return getBackpackAbleAddGridId(player, item.getItemModelId(),
				item.getNum(), item.isBind(), item.getLosttime());
	}
	...
	public void bagClearUp(Player player, boolean isgm) {
		if (!isgm) {
			if (player.getTransactionsinfo() != null) {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager
						.getInstance().getString("交易中不可以整理包裹"));
				return;
			}
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if(valuemodel == null) {
				log.error("整理背包时发现异常物品："+value.getItemModelId());
				return;
			}
			// 如果第一个物品是满的那么直接略过
				...
				for (int j = i + 1; j < list.size(); j++) {
					...
					if (isMergeAble(value, value2)) {
						...
						if (value.getNum() + value2.getNum() <= valuemodel
							...
							String logBefore2 = JSONserializable
									.toString(value2);
							...
							if (valuemodel.getQ_log() == 1) {
								logs.add(ItemChangeLog.createLog(
										player.getUserId(), player.getId(),
										value.getId(), value.getItemModelId(),
										logchangenum, logBefore,
										JSONserializable.toString(value),
										OtherReason.bagclearup,
										ItemAction.CLEARUP.toString(),
										actionId,
										ItemChangeAction.CHANGE.toString(),
										player.getCreateServerId(),
										player.getUserName(), player.getName(),
										player.getLevel(),value.getGridId()));
								logs.add(ItemChangeLog.createLog(
										player.getUserId(), player.getId(),
										value2.getId(),
										value2.getItemModelId(), logchangenum,
										logBefore2,
										JSONserializable.toString(value2),
										OtherReason.bagclearup,
										ItemAction.CLEARUP.toString(),
										actionId,
										ItemChangeAction.REMOVE.toString(),
										player.getCreateServerId(),
										player.getUserName(), player.getName(),
										player.getLevel(),value2.getGridId()));
							}
						} else {
							int all = value.getNum() + value2.getNum();
							int logchangenum = valuemodel.getQ_max()
									- value.getNum();
		...
		for (int i = 0; i < sortlist.size(); i++) {
			...
			if (valuemodel.getQ_log() == 1) {
				try {
					logs.add(ItemChangeLog.createLog(player.getUserId(),
							player.getId(), item.getId(),
							item.getItemModelId(), 0, before,
							JSONserializable.toString(item),
							OtherReason.bagclearup,
							ItemAction.CLEARUP.toString(), actionId,
							ItemChangeAction.MOVE.toString(),
							player.getCreateServerId(), player.getUserName(),
							player.getName(), player.getLevel(),item.getGridId()));
				} catch (Exception e) {
					log.error(e, e);
				}
			}
		...
		try {
			...
			log.setAfter(CodedUtil.encodeBase64(ZipUtil
					.compress(JSONserializable.toString(player
							.getBackpackItems()))));
			log.setRoleId(player.getId());
	...
	public boolean isMergeAble(Item item1, Item item2) {
		HashMap<Integer, Q_itemBean> goodmodels = DataManager.getInstance().q_itemContainer
				.getMap();
		...
		if (item1.getNum() >= sourcemodel.getQ_max()
				|| item2.getNum() >= sourcemodel.getQ_max()) {
			return false;
		}
		return true;
	}
	 ...
	 * @return
	 */
	public boolean isMerage(Item item1, Item item2) {
		HashMap<Integer, Q_itemBean> goodmodels = DataManager.getInstance().q_itemContainer
				.getMap();
		if (!goodmodels.containsKey(item1.getItemModelId())
				&& !goodmodels.containsKey(item2.getItemModelId())) {
...
//			return;
//		}
//		if(CooldownManager.getInstance().isCooldowning(player, CooldownTypes.BAG_OPEN_TIME, null)) {
//			return;
//		}
//		ManagerPool.cooldownManager.addCooldown(player,CooldownTypes.BAG_OPEN_TIME, null,10 * 1000);
		int bagCellTimeCount = player.getBagCellTimeCount();
		int bagCellsNum = player.getBagCellsNum();
		int time =0;
		while(true) {
			bagCellsNum++;
			Q_backpack_gridBean config = DataManager.getInstance().q_backpack_gridContainer
					.getMap().get(Global.BAG_TYPE + "_" + bagCellsNum);
			if(config == null) {
				//背包极限
				bagCellsNum = 0;
				break;
			}
			if(bagCellTimeCount < config.getQ_time()) {
				time = config.getQ_time() - bagCellTimeCount;
				break;
			}else {
				bagCellTimeCount = bagCellTimeCount - config.getQ_time();
			}
			
		}
//		if(cellId > bagCellsNum) {//前端开了加速器
//			return;
//		}
		ResCellTimeMessage resp = new ResCellTimeMessage();
		resp.setCellId(bagCellsNum);
		resp.setTimeRemaining(time);
		// if(log.isDebugEnabled()){
		// log.debug("包裹还剩下"+time);
		// }
		// System.out.println("包裹格子离开启还剩下"+time);
	 ...
	 *            1包裹 2仓库
	 */
	private void openCell(Player player, int cellId) {
		int rewardexp = 0;
		int bagCellTimeCount = player.getBagCellTimeCount();
		...
		msg.setIsSuccess(1);
		MessageUtil.tell_player_message(player, msg);
		MessageUtil.notify_player(player, Notifys.NORMAL, ResManager.getInstance().getString("成功扩容背包到{1}个!奖励经验{2}"),cellId + "", rewardexp + "");
	...
	public void openCellMsg(Player player, int cellId) {
		if (cellId <= 0 || cellId > Global.MAX_BAG_CELLS) {
			log.error("非法请求开格号 " + cellId + "roleid" + player.getId() + "当前格子ID"
					+ player.getBagCellsNum());
			return;
		}

		if (cellId - player.getBagCellsNum() > 7) {
			// 最大只能开七个格子 如果有大于此数的请求 视为非法请求
			log.error("最大只能开七个格子 " + cellId + "roleid" + player.getId() + "当前格子ID"
					+ player.getBagCellsNum());
			return;
		}
		...
		for (int i = player.getBagCellsNum() + 1; i <= cellId; i++) {
			Q_backpack_gridBean config = DataManager.getInstance().q_backpack_gridContainer
					.getMap().get(Global.BAG_TYPE + "_" + i);
			if(bagCellTimeCount < config.getQ_time()) {
		...
		if (!checkGold(player, needgold)) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，开启格子所需的钻石不足!"));
			return;
		}
		...
		if (needgold==0 || changeGold(player, -needgold, Reasons.YBBAGKAIGE, Config.getId())) {
			int bagCellsNum = player.getBagCellsNum();
	...
	// int bagCellsNum = player.getBagCellsNum();
	// if (bagCellsNum < Global.MAX_BAG_CELLS
	// && bagCellsNum >= Global.BAG_AUTOOPEN_CELL_ID) {
	...
	// .getMap().get(Global.BAG_TYPE + "_" + (bagCellsNum + 1));
	...
	// // bagCellsNum+1);
	// MessageUtil
	// .notify_player(player, Notifys.CHAT_SYSTEM, ResManager.getInstance().getString("成功扩容背包到{1}个!奖励经验{2}"), (bagCellsNum + 1) + "", bagConfig.getQ_exp() + "");
	// try {
	// CellOpenLog log = new CellOpenLog();
	...
	/**
	 * 返回正在倒计时的背包格子(格子id,倒计时秒)，如果已经是背包格子最大上限，则返回0,0
	 * @param player
	 * @return
	 */
	public Attribute getCoolDowningCellGrid(Player player) {
		...
		int bagCellTimeCount = player.getBagCellTimeCount();
		int coolDowningCellNum =bagCellsNum + 1; //格子id
		int lastTimeCount = 0;//剩余时间秒
		while(true) {
			Q_backpack_gridBean bagConfig = DataManager.getInstance().q_backpack_gridContainer
					.getMap().get(Global.BAG_TYPE + "_" + coolDowningCellNum);
			if(bagConfig == null) {
				//大于背包极限了
				return dto;
			}
			if(bagCellTimeCount < bagConfig.getQ_time()) {
				lastTimeCount = bagConfig.getQ_time() - bagCellTimeCount;
				break;
			}else {
				bagCellTimeCount = bagCellTimeCount - bagConfig.getQ_time();
				coolDowningCellNum++;
			}
		}
		dto.setType(coolDowningCellNum);
		dto.setValue(lastTimeCount);
		return dto;
	}
	// /**
	// * 绑钻
	 ...
	 * @param action
	 * @return
	 * @create	hongxiao.z      2014-2-12 下午7:26:06
	 */
	public boolean changeSpirit(Player player, int num, Reasons reasons, long action) 
	{
		if (num == 0) return true;
		
		if (num > 0 && PlayerManager.CHECK_NONAGE)	//反沉迷收益减少一半 
		{
			...
			{
				sendSpiritInfo(player);
				return true;
			}

		}
		
		int spirit = player.getSpirit() + num;
		if (spirit < 0 || spirit > Global.BAG_MAX_SPIRIT) 
		{
			if (num < 0) 
			{
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("操作失败，背包精魄数不足！"));
				return false;
			} 
			else 
			{
				
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("操作失败，背包精魄数己达上限！"));
				return false;
			}
		}
		
		int before = player.getSpirit();
		player.setSpirit(spirit);
		sendSpiritInfo(player);
		if (num > 0) 
		{ // 验证通过 记录累积得到精魄 和 累积消耗精魄
			MessageUtil.notify_player(player, Notifys.CHAT_PERSONAL, "获得{1}精魄", String.valueOf(Math.abs(num)));
			
			player.setTotalgetspirit(player.getTotalgetspirit() + num); // 玩家累积获得精魄
			WServer.sgeneratespirit.addAndGet(num); // 服务器累积产出精魄
		} 
		else 
		{
			MessageUtil.notify_player(player, Notifys.CHAT_PERSONAL, "失去{1}精魄", String.valueOf(Math.abs(num)));
			player.setTotalconsumespirit(player.getTotalconsumespirit()+ Math.abs(num)); // 玩家累积消耗精魄
			WServer.sconsumespirit.addAndGet(Math.abs(num)); // 服务器累积产出精魄
		}
		
		SpiritChangeLog log = new SpiritChangeLog();
		log.setActionId(action);
		log.setRoleId(player.getId());
		log.setBeforenum(before);
		log.setAfternum(spirit);
		log.setNum(num);
		log.setReason(reasons.getValue());
		log.setUserId(player.getUserId());
		log.setSid(player.getCreateServerId());
	 ...
	 * @return
	 */
	public boolean changeMoney(Player player, int num, Reasons reasons,
			long action) {
		if (num > 0 && PlayerManager.CHECK_NONAGE && Reasons.GETGOLD != reasons
				&& Reasons.BUYGOLD != reasons && Reasons.JIAOYIGOLD != reasons) {
		...
		if (money < 0 || money > Global.BAG_MAX_COPPER) {
			if (num < 0) {
				...
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager
						.getInstance().getString("操作失败，背包金币数己达上限"));
				return false;
			}
		}
		...
		sendMoneyInfo(player);
		if (num > 0) { // 验证通过 记录累积得到金币 和 累积消耗金币
			MessageUtil.notify_player(player, Notifys.CHAT_PERSONAL, ResManager.getInstance().getString("获得{1}金币"), String.valueOf(Math.abs(num)));
			player.setTotalgetmoney(player.getTotalgetmoney() + num); // 玩家累积获得金币
			WServer.sgeneratemoney.addAndGet(num); // 服务器累积产出金币
		} else {
			MessageUtil.notify_player(player, Notifys.CHAT_PERSONAL, ResManager.getInstance().getString("失去{1}金币"), String.valueOf(Math.abs(num)));
			player.setTotalconsumemoney(player.getTotalconsumemoney()
					+ Math.abs(num)); // 玩家累积消耗金币
			WServer.sconsumemoney.addAndGet(Math.abs(num)); // 服务器累积产出金币
			try {
				if (player.getTransactionsinfo() != null
						&& reasons != Reasons.JIAOYIGOLD) {
	...
	public boolean addGold(Player player, int num, Reasons reason, long action) {
		...
		if (goldnum < 0 || goldnum > Global.BAG_MAX_GOLD) {
			if (num >= 0) {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("操作失败，背包钻石数己达上限"));
			}
			return false;
		}
		player.setGold(gold);
		...
		if (num > 0) {
			...
			case GOLD_RECHARGE:
				...
				if (gold.getTotalGold() > 110){
					PlayerManager.getInstance().syncPlayerOrderInfo(player);
				}
				//! add by xuliang
				if(player.getRechargeGold() == 0) {
					//活动
					NewActivityManager.getInstance().trigger(player, NewActivityEnum.FirstPay, num);
					//任务
					TaskManager.getInstance().action(player, Task.ACTION_TYPE_RANK, TaskEnum.FIRST_PAY, num);
				}
				player.setRechargeGold(gold.getTotalGold());
				player.setRechargeTime(System.currentTimeMillis());
				//! 同步充值排行
				ReqSyncPlayerRechargeMessage msg = new ReqSyncPlayerRechargeMessage();
				msg.setPlayerId(player.getId());
				msg.setRechargeGold(gold.getTotalGold());
				msg.setRechargeTime(player.getRechargeTime());
				MessageUtil.send_to_world(msg);
				
				NewActivityManager.getInstance().trigger(player, NewActivityEnum.Pay4Gift, num);
				break;
			default:
				// 非交易类增加
				break;
		...
		sendGoldInfo(player);
		if (num > 0) {
			MessageUtil.notify_player(player, Notifys.NORMAL, ResManager.getInstance().getString("获得 {1} 钻石"), String.valueOf(Math.abs(num)));
			MessageUtil.notify_player(player, Notifys.CHAT_PERSONAL, ResManager.getInstance().getString("获得{1}钻石"), String.valueOf(Math.abs(num)));
			//! add by xuliang
			MessageUtil.notify_player(player, Notifys.CHAT_IMPORTANT, ResManager.getInstance().getString("获得{1}钻石"), String.valueOf(Math.abs(num)));
		} else {
			MessageUtil.notify_player(player, Notifys.NORMAL, ResManager.getInstance().getString("失去 {1} 钻石"), String.valueOf(Math.abs(num)));
			MessageUtil.notify_player(player, Notifys.CHAT_PERSONAL, ResManager.getInstance().getString("失去{1}钻石"), String.valueOf(Math.abs(num)));
			try {
				if (player.getTransactionsinfo() != null
						&& reason != Reasons.JIAOYIYB) {
					ManagerPool.transactionsManager.stChangeState(player); // 解锁交易
				}
			} catch (Exception e) {
				log.error(e, e);
	 ...
	 * @return
	 */
	public boolean changeGold(Player player, int num, Reasons reason,
			long action) {
		if (num >= 0) {
			return false;
		}
		...
		sendGoldInfo(player);
		if (num > 0) {
			MessageUtil.notify_player(player, Notifys.NORMAL, ResManager.getInstance().getString("获得{1}钻石"), String.valueOf(Math
					.abs(num)));
			MessageUtil.notify_player(player, Notifys.CHAT_PERSONAL, ResManager.getInstance().getString("获得{1}钻石"), String.valueOf(Math
					.abs(num)));
		} else {
//			MessageUtil.notify_player(player, Notifys.NORMAL, ResManager.getInstance().getString("失去{1}钻石"), String.valueOf(Math
//					.abs(num)));
			MessageUtil.notify_player(player, Notifys.CHAT_PERSONAL, ResManager.getInstance().getString("失去{1}钻石"), String.valueOf(Math
					.abs(num)));
			try {
				if (player.getTransactionsinfo() != null
						&& reason != Reasons.JIAOYIYB) {
			...
			try {
				TaskManager.getInstance().action(player, Task.ACTION_TYPE_RANK, TaskEnum.COSTGOLD, Math.abs(num));
			} catch (Exception e) {
				log.error("", e);
			}
			try {
				VipManager.getInstance().costGold(player);
			} catch (Exception e) {
				log.error("", e);
			}
		}
		...
		if (isscript) {// 非交易类消耗钻石，触发脚本
			IConsumeGoldScript script = (IConsumeGoldScript) ManagerPool.scriptManager
					.getScript(ScriptEnum.RETBINDGOLD);
			if (script != null) {
				try {
	...
	private void saveGoldExpend(Player player, int goldnum, Reasons reason) {
		...
		// try {
		// if
		// (WServer.getInstance().getSaveGoldExpendThread().checkReason(reason))
		// {
		// WServer.getInstance().getSaveGoldExpendThread().addSaveTask(player,
		// -goldnum, reason);
		// }
		// } catch (Exception e) {
		// log.error(e, e);
	...
	// * @return
	// */
	// public boolean changeGoldFirstBind(Player player, int num, Reasons
	// reason, long action) {
	// if (num == 0) {
	// return true;
	// }
	...
	public boolean changeTmpGold(Player player, int num) {
		...
		if (!isIsopentmpgold()) { // 是否开启临时钻石
			return ManagerPool.backpackManager.addGold(player, num, Reasons.QUCHUTMPYB, Config.getId());// 买家得到钻石
		}
	 ...
	 * @return
	 */
	public boolean changeBindGold(Player player, int num, Reasons reasons,
			long action) {
		...
		if (bindgold < 0 || bindgold > Global.BAG_MAX_GOLD) {
			if (num >= 0) {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager
						.getInstance().getString("操作失败，绑钻数己达上限"));
			}
			return false;
		}
		...
		sendBindGoldInfo(player);
		if (num > 0) { // 验证通过 记录 累积获得绑钻 与 累积消耗绑钻 排除社交原因
			MessageUtil.notify_player(player, Notifys.CHAT_PERSONAL, ResManager.getInstance().getString("获得{1}绑钻"), String.valueOf(Math.abs(num)));
			player.setTotalgetbindgold(player.getTotalgetbindgold() + num); // 玩家获得绑定钻石
			WServer.sgeneratebindgold.addAndGet(num); // 服务器产出绑定钻石
		} else {
			MessageUtil.notify_player(player, Notifys.CHAT_PERSONAL, ResManager.getInstance().getString("失去{1}绑钻"), String.valueOf(Math.abs(num)));
			player.setTotalconsumebindgold(player.getTotalconsumebindgold() + Math.abs(num)); // 玩家消耗绑定钻石
			WServer.sconsumebindgold.addAndGet(Math.abs(num)); // 服务器消耗绑定钻石
			TaskManager.getInstance().action(player, Task.ACTION_TYPE_RANK, TaskEnum.COSTBINDGOLD, Math.abs(num));
			LivenessManager.getInstance().expenseBindGold(player, num);
		}
	...
	public void clear(Player player) {
		ConcurrentHashMap<String, Item> backpackItems = player
				.getBackpackItems();
		long actionId = Config.getId();
	...
	public void addAbleReceieve(Player player, List<Item> spilthGoods) {
		...
		long actionId = Config.getId();
		String before = JSONserializable.toString(player
		...
		while (player.getTaskRewardsReceiveAble().size() > 14) {
			Item remove = player.getTaskRewardsReceiveAble().get(
					player.getTaskRewardsReceiveAble().size() - 1);
		...
		log.setActionId(actionId);
		log.setRoleId(player.getId());
		log.setAfterList(JSONserializable.toString(player
				.getTaskRewardsReceiveAble()));
		...
		addlog.setActionId(actionId);
		addlog.setRoleId(player.getId());
		addlog.setAfterList(JSONserializable.toString(player
				.getTaskRewardsReceiveAble()));
	...
	private String getKey(Item item) {
		...
		bagkeys.append(item.getItemModelId()).append("_")
				.append(item.getLosttime()).append("_").append(item.isBind());
	...
	public String getName(int itemmodel) {
		...
		if (!StringUtil.isBlank(q_name)
				&& q_name.contains(Symbol.XIAHUAXIAN_REG)) {
	...
	public int setItemBuff(Player player, String buffString, int num) {
		...
		for (String string : buffs) {
			...
			result = ManagerPool.buffManager.addBuff(player, player, buffid,
					num, 0, 0, 0, 0);
			if (result == 0) {
	 ...
	 * @return
	 */
	public Item getWarehouseItem(Player player,long getItemid){
		Iterator<Item> iter = player.getStoreItems().values().iterator();
		while (iter.hasNext()) {
			Item item = (Item) iter.next();
	 ...
	 * @return
	 */
	public Item getBackPackItem(Player player,long getItemid){
		Iterator<Item> iter = player.getBackpackItems().values().iterator();
		while (iter.hasNext()) {
			Item item = (Item) iter.next();
			if (item.getId() == getItemid) {
	 ...
	 *            玩家
	 * @param itemstr
	 *            物品字符串，格式(物品模板ID，物品数量，性别，是否绑定（0或1)，过期时间，升级级数，附加条数；...)
	 * @param itemlist
	 *            物品集合，一般为new ArrayList()
	 * @return 创建的物品占背包格子数
	 */
...
//					if (sex == 0 || sex == player.getSex()) {
						...
						// 战魂
						case -6: {
							Item fightspiritItem = Item
									.createFightSpirit(itemnum);
						...
						case -7: {
							...
							break;
						default: {
							List<Item> items = Item.createItems(itemid,
									itemnum, bind, losttime == 0 ? losttime
...
//					}
			...
			for (int i = 0; i < itemlist.size(); i++) {
				...
				if (item != null) {
					if (item.getItemModelId() == -1) {// Money
						if (!BackpackManager.getInstance().changeMoney(player,
								item.getNum(), Reasons.ACTIVITY_GIFT, id)) {
							MailServerManager.getInstance().sendSystemMail(
									player.getId(),
									player.getName(),
									ResManager.getInstance()
											.getString("活动系统邮件"),
									ResManager.getInstance().getString(
											"由于未知原因，您的活动中金币未领取成功，请点击附件领取。"),
									(byte) 1, item.getNum(), null);
							MessageUtil.notify_player(
									player,
									Notifys.ERROR,
									ResManager.getInstance().getString(
											"由于未知原因，您的活动中金币未领取成功，请点击邮件领取。"));
						}
						// } else if (item.getItemModelId() == -2) {//Gold
						// if
						// (!BackpackManager.getInstance().changeBindGold(player,
						// item.getNum(), Reasons.ACTIVITY_GIFT, id)) {
						// List<Item> items = new ArrayList<Item>();
						// items.add(Item.createBindGold(item.getNum()));
						// MailServerManager.getInstance().sendSystemMail(player.getId(),
						// player.getName(),
						// ResManager.getInstance().getString("活动系统邮件"),
						// ResManager.getInstance().getString("由于未知原因，您的活动中绑钻未领取成功，请点击附件领取。"),
						// (byte) 1, 0, items);
						// MessageUtil.notify_player(player, Notifys.ERROR,
						// ResManager.getInstance().getString("由于未知原因，您的活动中绑钻未领取成功，请点击邮件领取。"));
						// }
					} else if (item.getItemModelId() == -3) {// zhenqi
						/* xuliang
						if (PlayerManager.getInstance().addZhenqi(player,
								item.getNum(),
								AttributeChangeReason.ACTIVITIESADD) == 0
								&& item.getNum() != 0) {
							List<Item> items = new ArrayList<Item>();
							...
							MailServerManager.getInstance().sendSystemMail(
									...
									ResManager.getInstance()
											.getString("活动系统邮件"),
									ResManager.getInstance().getString(
											"由于未知原因，您的活动中真气未领取成功，请点击附件领取。"),
									(byte) 1, 0, items);
							MessageUtil.notify_player(
									player,
									Notifys.ERROR,
									ResManager.getInstance().getString(
											"由于未知原因，您的活动中真气未领取成功，请点击邮件领取。"));
						}
						*/
					} else if (item.getItemModelId() == -4) {// exp
						PlayerManager.getInstance().addExp(player,
								item.getNum(),
								AttributeChangeReason.ACTIVITIESADD);
					} else if (item.getItemModelId() == -5) {// bindgold
						if (!BackpackManager.getInstance().changeBindGold(
								player, item.getNum(), Reasons.ACTIVITY_GIFT,
								id)) {
							List<Item> items = new ArrayList<Item>();
							items.add(Item.createBindGold(item.getNum()));
							MailServerManager.getInstance().sendSystemMail(
									...
									ResManager.getInstance()
											.getString("活动系统邮件"),
									ResManager.getInstance().getString(
											"由于未知原因，您的活动中绑钻未领取成功，请点击附件领取。"),
									(byte) 1, 0, items);
							MessageUtil.notify_player(
									player,
									Notifys.ERROR,
									ResManager.getInstance().getString(
											"由于未知原因，您的活动中绑钻未领取成功，请点击邮件领取。"));
						}
					} else if (item.getItemModelId() == -6) {// fightspirit
						/* xuliang
						if (!ArrowManager.getInstance().addFightSpiritNum(
								player, ArrowManager.FightType_RI,
								item.getNum(), true,
								ArrowReasonsType.ACTIVITIES)) {
							List<Item> items = new ArrayList<Item>();
							...
							MailServerManager.getInstance().sendSystemMail(
									...
									ResManager.getInstance()
											.getString("活动系统邮件"),
									ResManager.getInstance().getString(
											"由于未知原因，您的活动中七曜战魂未领取成功，请点击附件领取。"),
									(byte) 1, 0, items);
							MessageUtil.notify_player(
									player,
									Notifys.ERROR,
									ResManager.getInstance().getString(
											"由于未知原因，您的活动中七曜战魂未领取成功，请点击邮件领取。"));
						}
						*/
					} else if (item.getItemModelId() == -7) {// rank
						RankManager.getInstance().addranknum(player,
								item.getNum(), RankType.Activities);
						// if (!RankManager.getInstance().addranknum(player,
						// item.getNum(), RankType.Activities)) {
						// List<Item> items = new ArrayList<Item>();
						// items.add(Item.createRank(item.getNum()));
						// MailServerManager.getInstance().sendSystemMail(player.getId(),
						// player.getName(), "活动系统邮件",
						// "由于未知原因，您的活动中军功未领取成功，请点击附件领取。", (byte) 1, 0, items);
						// MessageUtil.notify_player(player, Notifys.ERROR,
						// "由于未知原因，您的活动中军功未领取成功，请点击邮件领取。");
						// }
					} else {// 普通物品
						List<Item> items = new ArrayList<Item>();
						if (item instanceof Equip) {
							Equip equip = (Equip) item;
							items = Item.createItems(item.getItemModelId(),
									item.getNum(), item.isBind(), (long) equip
											.getLosttime() * 1000, equip
											.getGradeNum(), equip
											.getAttributes().size());
						} else {
							items = Item.createItems(item.getItemModelId(),
									item.getNum(), item.isBind(),
									(long) item.getLosttime() * 1000, 0, 0);
						}
						if (!BackpackManager.getInstance().addItems(player,
								items, Reasons.ACTIVITY_GIFT, id)) {
							MailServerManager.getInstance().sendSystemMail(
									player.getId(),
									player.getName(),
									ResManager.getInstance()
											.getString("活动系统邮件"),
									ResManager.getInstance().getString(
											"由于未知原因，该物品未领取成功，请点击附件领取。"),
									(byte) 1, 0, items);
							MessageUtil.notify_player(
									player,
									Notifys.ERROR,
									ResManager.getInstance().getString(
											"由于未知原因，您的活动中有物品未领取成功，请点击邮件领取。"));
						}
						// else {
						// ItemReasonsInfo itemReasonsInfo = new
						// ItemReasonsInfo();
						// if (!items.isEmpty()) {
	...
	public void notifyitemname(Player player, String itemstr) {
		...
		for (int i = 0; i < itemstrs.length; i++) {
			...
			if (itemdata.length >= 2) {
				Q_itemBean model = ManagerPool.dataManager.q_itemContainer
						.getMap().get(Integer.valueOf(itemdata[0]));
				if (model != null) {
					MessageUtil.notify_player(player, Notifys.CHAT_PERSONAL,
							ResManager.getInstance().getString("获得{1}（{2}）"),
							model.getQ_name(), itemdata[1]);
				}
			}
		}
	}
	 ...
	 * @param infos
	 * @param resStr
	 * @create	hongxiao.z      2014-3-12 下午5:28:14
	 */
	public void toBackpack(Player player, List<AnalysisItemInfo> infos, String resStr)
	{
		for (AnalysisItemInfo info : infos) 
		{
			toBackpack(player, info, resStr);
		}
	}
	 ...
	 * @param player
	 * @param info
	 * @create	hongxiao.z      2014-2-15 下午7:24:02
	 */
	private void toBackpack(Player player, AnalysisItemInfo info, String resStr)
	{
		switch(info.getItemId())
		{
			case -1:	//铜钱
				BackpackManager.getInstance().changeMoney(player, info.getNum(), Reasons.GRADE_GAIN_AWARD, Config.getId());
				break;
			case -2:	//钻石
				BackpackManager.getInstance().changeGold(player, info.getNum(), Reasons.GRADE_GAIN_AWARD, Config.getId());
				break;
			case -3:	//真气 暂无 
				break;
			case -4:	//经验
//				PlayerManager.getInstance().addExp(player, (long)entry.getValue(), AttributeChangeReason.PT_GAIN_AWARD);
				break;
			case -5:	//绑定钻石
				BackpackManager.getInstance().changeBindGold(player, info.getNum(), Reasons.GRADE_GAIN_AWARD, Config.getId());
				break;
			case -6:	//精魄
				BackpackManager.getInstance().changeSpirit(player, info.getNum(), Reasons.GRADE_GAIN_AWARD, Config.getId());
				break;
			default:	//道具
				Q_itemBean bean = DataManager.getInstance().q_itemContainer.get(info.getItemId());
				...
				if(!PlayerManager.checkJob(player.getJob(), bean.getQ_job_limit())) break;
				List<Item> items = Item.createItems(info.getItemId(), info.getNum(), true, info.getDuration(), info.getStrongLv(), info.getAddLv(), info.getZhuoyue());
				
				//获取背包空间数量
				int emptynum = BackpackManager.getInstance().getEmptyGridNum(player);
				if(items.size() > emptynum)	//空间不足发邮件
				{
					MailServerManager.getInstance().sendSystemMail(player.getId(), 
							player.getName(), ResManager.getInstance().getString(resStr), 
							ResManager.getInstance().getString("由于您的背包空间不足，道具已通过邮件发放！"), (byte) 1, 0, items);
					break;
				}
				
				//入包
				BackpackManager.getInstance().addItems(player, items, Reasons.GRADE_GAIN_AWARD, Config.getId());
		}
	}
	 ...
	 * @param resons
	 * @param item
	 * @create	hongxiao.z      2014-1-17 下午12:36:58
	 */
	public void showAs_1(Player player, Reasons resons, Item item, int type, int autoUse)
	{
		//! add by xuliang
		if (resons != Reasons.WEAREDORUNWEARED && autoUse != 1){
			...
			MessageUtil.notify_player(player, Notifys.CHAT_IMPORTANT, ResManager
					.getInstance().getString("恭喜您获得{$}"), goodsInfos, 0);
		}
		//! end
		
		if (resons.getValue() == Reasons.Pandora.getValue() &&
				(type == ItemTypeConst.EQUIP || type == ItemTypeConst.METRIAL_JEWEL))
		{
//			int itemreasons = 3;
//			if (resons.getValue() == Reasons.CHESTBOX_ITEM.getValue()) {
//				itemreasons = 4;
//			}
			...
			itemReasonsInfo.setItemNum(item.getNum());
//			itemReasonsInfo.setItemReasons(itemreasons);
			itemReasonsInfo.setItemReasons(Reasons.SHOW_ADD_ITEM_1.getValue());
			sendMessage.getItemReasonsInfoList().add(itemReasonsInfo);
			MessageUtil.tell_player_message(player, sendMessage);
		}
	}
	
	public boolean isIsopentmpgold() {
		return isopentmpgold;
	}

	public void setIsopentmpgold(boolean isopentmpgold) {
		this.isopentmpgold = isopentmpgold;
	}
}
