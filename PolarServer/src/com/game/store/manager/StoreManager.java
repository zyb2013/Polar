...
public class StoreManager {
	 ...
	 * @param player
	 */
	public boolean openStore(Player player) {
		int storageCost = VipManager.getInstance().getStorageCost(player);
		if (storageCost > 0) {
			if (!BackpackManager.getInstance().changeGold(player, -storageCost, Reasons.OPENSTORE, Config.getId())) {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，你的钻石不足，打开随身仓库需要花费{1}钻石"), String.valueOf(storageCost));
				return false;
			}
		}
		return true;
...
//		int rewardexp=0;
//		int storeCellTimeCount = player.getStoreCellTimeCount();
...
//			storeCellTimeCount = storeCellTimeCount - config.getQ_time();
		}
//		if(storeCellTimeCount <0) {
//			storeCellTimeCount = 0;
//		}
...
//		player.setSendBagOpenCellTime(false);
	...
	/**
	 * 返回正在倒计时的仓库格子(格子id,倒计时秒)，如果已经是仓库格子最大上限，则返回0,0
	 * @param player
	 * @return
	 */
	public Attribute getCoolDowningCellGrid(Player player) {
		...
		int storeCellsNum = player.getStoreCellsNum();
		if(storeCellsNum >= Global.MAX_STORE_CELLS) {
			return dto;
		}
		int storeCellTimeCount = player.getStoreCellTimeCount();
		int coolDowningCellNum =storeCellsNum + 1; //格子id
		int lastTimeCount = 0;//剩余时间秒
		while(true) {
			Q_backpack_gridBean bagConfig = DataManager.getInstance().q_backpack_gridContainer
					.getMap().get(Global.STORE_TYPE + "_" + coolDowningCellNum);
			if(bagConfig == null) {
				//大于背包极限了
				return dto;
			}
			if(storeCellTimeCount < bagConfig.getQ_time()) {
				lastTimeCount = bagConfig.getQ_time() - storeCellTimeCount;
				break;
			}else {
				storeCellTimeCount = storeCellTimeCount - bagConfig.getQ_time();
				coolDowningCellNum++;
			}
		}
	...
	private void sendItemInfos(Player player) {
		...
		Attribute coolDowningCellGrid = getCoolDowningCellGrid(player);
		msg.setCoolDowningGridId((short) coolDowningCellGrid.getType());
		msg.setCoolDowningGridTime((int) (coolDowningCellGrid.getValue()));
		MessageUtil.tell_player_message(player, msg);
	}
	...
	private boolean checkAble(Player player ,int npcid){
		if(player.getLevel()<Global.STORE_NEED_GRADE){
			if(npcid<=0){
				MessageUtil.notify_player(player,Notifys.NORMAL,ResManager.getInstance().getString("等级不足,需要{1}级才可以使用随身仓库"),Global.STORE_NEED_GRADE+"");
				return false;
			}else{
				if(!NpcManager.checkDistance(npcid, player.getPosition())){
					MessageUtil.notify_player(player,Notifys.NORMAL,ResManager.getInstance().getString("距离NPC过远"));
					return false;
				}
				if(!NpcManager.checkFunction(npcid, NpcManager.FUNCTION_STORE)){
					MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("该NPC没有仓库功能"));
					return false;
				}
			}
		}
...
//		}
		...
		// if(cellId-player.getStoreCellsNum()>7){
		// //最大只能开7个格子 如果有大于此数的请求 视为非法请求
		// log.error("最大只能开七个格子 " + cellId + "roleid" + player.getId() + "当前格子ID"
		// + player.getStoreCellsNum());
		// MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("最大只能开七个格子"));
		// return ;
		// }
		if(cellId<=0||cellId>Global.MAX_STORE_CELLS){
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("最多只能打开{1}个格子"),String.valueOf(Global.MAX_STORE_CELLS));
			return;
		}
...
//			if(storeCellTimeCount < config.getQ_time()) {
...
//		Gold gold = BackpackManager.getInstance().getGold(player);
		...
		long action = Config.getId();
		if(needgold == 0 || BackpackManager.getInstance().changeGold(player,-needgold, Reasons.YBSTOREKAIGE,action)){
//				.changeGoldFirstBind(player,needgold, Reasons.YBSTOREKAIGE,action)){
			...
			MessageUtil.notify_player(player, Notifys.NORMAL, ResManager.getInstance().getString("成功扩容仓库到{1}个!"), cellId + "");
			try{
...
//		}
		...
		while(true) {
			...
			Q_backpack_gridBean config = DataManager.getInstance().q_backpack_gridContainer
					.getMap().get(Global.STORE_TYPE + "_" + storeCellsNum);
			if(config == null) {
				//仓库极限
				storeCellsNum = 0;
				break;
			...
			if(storeCellTimeCount < config.getQ_time()) {
				time = config.getQ_time() - storeCellTimeCount;
				break;
			}else {
				storeCellTimeCount = storeCellTimeCount - config.getQ_time();
			}
		...
		if(cellId > storeCellsNum) {//前端开了加速器
			return;
		}
//		Q_backpack_gridBean config = DataManager.getInstance().q_backpack_gridContainer.getMap().get(Global.STORE_TYPE+"_"+cellId);
		...
		resp.setCellId(storeCellsNum);
		resp.setTimeRemaining(time);
//		if(log.isDebugEnabled()){
//			log.debug("仓库还剩下"+time);	
//		}
		log.error("仓库格子离开启还剩下"+time);
