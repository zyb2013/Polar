...
//	 	满足要求
//		进入下个逻辑
		 int resume_gold = model.getQ_got_resume_gold();//金币
		 if(player.getMoney()<resume_gold){
			 /* xuliang
				MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("激活{1}美人需要消耗{2}金币，您目前金币不足"),model.getQ_name(),resume_gold+"");
			*/
			 return;
		 }
...
//	 	 	若当前出战美人血量为满值则收起场景中，出战美人
	...
	public void heti(Player player,long petId){
		...
		if(model.getQ_ht_resume_copper()>player.getMoney()){
			/* xuliang
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("{1}美人合体需要消耗{2}金币，您的金币数量不足"), model.getQ_name(),model.getQ_ht_resume_copper()+"");
			*/
			return;
		}
...
//		按分钟计 不足一分钟按一分钟计
		...
		if (!BackpackManager.getInstance().checkGold(player, needgold)) {
			MessageUtil.notify_player(player, Notifys.ERROR, "钻石数量不足需要{1}", "" + needgold);
			return;
		}
		//检测2级密码
