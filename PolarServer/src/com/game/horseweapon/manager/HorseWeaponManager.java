...
public class HorseWeaponManager {
	...
	public boolean activateHorseWeapon(Player player, Item item, long actionId){
		...
		try{
			...
			parseUtil.setValue(String.format(ResManager.getInstance().getString("恭喜【%s】成功装备第二把武器【%s】战斗力提升%d！{@}"), player.getName(), weaponname, afterFightPower),
					new ParseUtil.VipParm(VipManager.getInstance().getVIPLevel(player), GuideType.HORSEWEAPON.getValue()));
