...
//			ManagerPool.fightManager.showGridInCircle(player, player.getPosition(), Global.BLOCK_DAMAGE_RADIUS, grids);
			//伤害周围敌对目标
			List<Fighter> fighters = ManagerPool.fightManager.selectFighters(map, player, player.getPosition(), Global.BLOCK_DAMAGE_RADIUS, type, playerAttackType, Global.BLOCK_DAMAGE_NUM, grids, 0);
			...
			while (iterator.hasNext()) {
				...
				if(fighter instanceof Player || fighter instanceof Pet){
					...
					if(FighterState.PKBAOHU.compare(player.getFightState())){
						...
						if(fighter instanceof Player){
							log.error("玩家(" + player.getId() + ")PK状态为(" + player.getPkState() + ")挂机状态为(" + PlayerState.AUTOFIGHT.compare(player.getState()) + ")对玩家(" + fighter.getId() + ")攻击(魔法值盾)导致和平保护buff消失");
						}else{
							log.error("玩家(" + player.getId() + ")PK状态为(" + player.getPkState() + ")挂机状态为(" + PlayerState.AUTOFIGHT.compare(player.getState()) + ")对宠物(" + fighter.getId() + ")攻击(魔法值盾)导致和平保护buff消失");
						}
				...
				ManagerPool.fightManager.damage(player, fighter, damage, 1, 0);
			}
		}
	}
}
