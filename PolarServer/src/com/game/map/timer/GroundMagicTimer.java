...
//				}
			...
			List<Fighter> targets = ManagerPool.fightManager.randomSelectFighters(getFighter(player, fighters, skill.getSkillModelId()), model.getQ_target_max());
			for (Fighter fighter : targets) {
				ManagerPool.fightManager.damage(player, fighter, countDamage(player, fighter, skill), model.getQ_enmity(), skill.getSkillModelId());
			}
	 ...
	 * @return
	 */
	private List<Fighter> getFighter(Fighter attacker, List<Fighter> fighters, int skillModelId) {
		//目标集合
		...
		while (iter.hasNext()) {
			...
			if (!ManagerPool.fightManager.checkCanAttack(attacker, fighter, skillModelId)) {
				continue;
			}
