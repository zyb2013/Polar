...
public class ResRoundObjectsMessage extends Message{
	...
	//周围召唤怪信息
	private List<SummonPetInfo> summonpets = new ArrayList<SummonPetInfo>();
	//消失召唤怪列表
	private List<Long> summonpetIds = new ArrayList<Long>();
	...
	public boolean write(IoBuffer buf){
		...
		//消失召唤怪列表
		writeShort(buf, summonpetIds.size());
		for (int i = 0; i < summonpetIds.size(); i++) {
			writeLong(buf, summonpetIds.get(i));
		}
		return true;
	}
	...
	public boolean read(IoBuffer buf){
		...
		//周围宠物信息
				int summonpets_length = readShort(buf);
				for (int i = 0; i < summonpets_length; i++) {
					summonpets.add((SummonPetInfo)readBean(buf, PetInfo.class));
				}
				//消失物品列表
				int summonpetIds_length = readShort(buf);
				for (int i = 0; i < summonpetIds_length; i++) {
					summonpetIds.add(readLong(buf));
				}
		return true;
	}
	
	public List<SummonPetInfo> getSummonpets() {
		return summonpets;
	}

	public void setSummonpets(List<SummonPetInfo> summonpets) {
		this.summonpets = summonpets;
	}

	public List<Long> getSummonpetIds() {
		return summonpetIds;
	}

	public void setSummonpetIds(List<Long> summonpetIds) {
		this.summonpetIds = summonpetIds;
	}
