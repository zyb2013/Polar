...
//		long openServerTime=dateByString.getTime();
	 ...
	 * @param time
	 * @return
	 * @create	hongxiao.z      2014-1-23 下午12:07:21
	 */
	...
	public static String GetTransformTime(long mss) {
		...
		long seconds = mss % 60;
		if (days > 0) {
			t = days + ResManager.getInstance().getString("天") + hours + ResManager.getInstance().getString("小时") + minutes + ("分") + seconds + ("秒");
		} else {
			t = hours + ResManager.getInstance().getString("小时") + minutes + ResManager.getInstance().getString("分") + seconds + ResManager.getInstance().getString("秒");
		}
		return t;
	}
	...
	public static String millisecondToStr(long millisecond) {
		if (millisecond < 0) {
			return millisecond + ResManager.getInstance().getString("毫秒");
		}
		if (millisecond < 1000) {
			return "1秒";
		}
		...
		if (day > 0) {
			result += (day + ResManager.getInstance().getString("天"));
		}
		second %= 3600 * 24;
		long hour = second / 3600;
		if (hour > 0) {
			result += (hour + ResManager.getInstance().getString("小时"));
		}
		second %= 3600;
		long minute = second / 60;
		if (minute > 0) {
			result += (minute + ResManager.getInstance().getString("分"));
		}
		second %= 60;
		if (second > 0) {
			result += (second + ResManager.getInstance().getString("秒"));
		}
		return result;
	}
...
//		String ss = "[[-1],[-1],[-1],[6,7],[1350,2000],[-2]]";   
//		checkRangeTime(ss);
		String str = "[[0],[0],[0],[0],[5],[-2]]";
		System.out.println(new Date(System.currentTimeMillis()));
		long time = getRangeTimeBeforeOrAfter(str, false, true);
...
//		}
	 ...
	 * @param [[2012],[1,10],[22],[-1],[101,2358],[10]]   [[-1],[-1],[-1],[-1],[0,2500],[1]]
	 * @param string
	 * @return
	 */
				...
				if (days[0] > 0) {	//日期和星期互斥
					if (days.length == 2) {	//区间
						if (day >= days[0] && day <= days[1]) {
							isdayweek = true;
						}else if (months.length == 2) {//如果是2个月
							if ((month == months[0] && day >= days[0]) || (month == months[1] && day <= days[1])) {
								isdayweek = true;
							}
						}
					} else {
...
//				Integer[] weeks = timelist.get(3);
				...
				if (boBeforeOrAfter) {
					...
					if (interval.length != 0 && interval[0] == -2) {
						...
						if (btime == 0) {
							...
							calendar.setTime(date);
							calendar.setTimeInMillis(calendar.getTimeInMillis() + (long) btime * 24 * 3600 * 1000);
							calendar.set(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), calendar.get(Calendar.DAY_OF_MONTH), 23, 59, 59);
						}
					} else {
	 ...
	 * @param date
	 * @return
	 * @create	hongxiao.z      2014-1-16 下午9:17:10
	 */
	public static boolean isMonth(Date date)
	{
		return isMonth(date.getTime());
	}
	 ...
	 * @param player
	 * @return
	 * @create	hongxiao.z      2014-1-16 下午9:39:02
	 */
	public static boolean isOpenMonth(Player player)
	{
		GameConfig config = new GameConfig();
		Date date = config.getServerTimeByPlayer(player);
		return isMonth(date);
	}
	 ...
	 * @param		指定时间
	 * @return
	 * @create	hongxiao.z      2014-1-23 上午9:31:34
	 */
	public static boolean isValidTime(Date date)
	{
		GameConfig config = WServer.getGameConfig();
		int country = config.getServerByCountry(1);
		
		//获得开F时间
		Date openDate = config.getServerTimeByServer(country);
		...
		int now_year  = _calendar.get(Calendar.YEAR);
		int now_month = _calendar.get(Calendar.MONTH);
		int now_day = _calendar.get(Calendar.DATE);
		
		return  (year > now_year) ? false : (year < now_year) ? true : 
				(month > now_month) ? false : (month < now_month) ? true : 
				(day > now_day);
	}
	 ...
	 * 获得开服时间
	 * @return
	 * @create	hongxiao.z      2014-1-23 上午10:06:48
	 */
	public static Calendar getOpenTime()
	{
		GameConfig config = WServer.getGameConfig();
		int country = config.getServerByCountry(1);
		Calendar cal = Calendar.getInstance();
		cal.setTime(config.getServerTimeByServer(country));
		return cal;
	}
	 ...
	 * 判断是否是本月开F
	 * @return
	 * @create	hongxiao.z      2014-1-16 下午9:45:26
	 */
	public static boolean isOpenMonth()
	{
		GameConfig config = WServer.getGameConfig();
		int country = config.getServerByCountry(1);
		Date date = config.getServerTimeByServer(country);
		return isMonth(date);
	}
	 ...
	 * @return
	 * @create	hongxiao.z      2014-1-16 下午9:33:11
	 */
	public static boolean isOpenMonth(int serverid)
	{
		GameConfig config = WServer.getGameConfig();
		Date date = config.getServerTimeByServer(serverid);
		return isMonth(date);
	}
	 ...
	 * @param time
	 * @return
	 * @create	hongxiao.z      2014-1-16 下午9:07:59
	 */
