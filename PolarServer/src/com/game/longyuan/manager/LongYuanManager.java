...
public class LongYuanManager {
	...
	public byte checkUpCondition(Player player,Q_longyuanBean xdb,boolean point) {
		...
		if (player.getMoney() < xdb.getQ_ordgold()) {
			if (point) {
				/* xuliang
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，激活{1}需要金币{2}。"),xdb.getQ_pointname(),String.valueOf(xdb.getQ_ordgold()));
				*/
			}
			return -3;
		}
...
//	
	...
	public String sayaddname(Player player ,Q_longyuanBean xdb){
		...
		if (xdb.getQ_maxmp() > 0) {
			bff.append(ResManager.getInstance().getString("[魔法值上限]+")+xdb.getQ_maxmp()+",");
		}
		if (xdb.getQ_maxsp() > 0) {
	...
	public HashMap<String, Integer> getTianyuanProperty(Player player) {
		...
		att.put(ResManager.getInstance().getString("魔法值"), 0);
					...
					if (sec >= lysection) {
						if (sec > lysection ||  (sec == lysection && slv >= lylv)) {
							try {
								...
								att.put(ResManager.getInstance().getString("魔法值"),att.get(ResManager.getInstance().getString("魔法值"))+data.getQ_maxmp());
