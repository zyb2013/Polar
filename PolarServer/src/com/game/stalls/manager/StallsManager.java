...
public class StallsManager {
	...
	public int getlookStallslv(){
		Q_globalBean global = ManagerPool.dataManager.q_globalContainer.getMap().get(Global.STALL_OPEN_LV);
	...
	public void stReqStallsOpenUpMessage(Player player,ReqStallsOpenUpMessage msg) {
		...
		if (player.getPkValue() > 0){
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("魔头不能使用摊位"));
			return;
		}
	 ...
	 * 商品上架
	 * @param player
	 * @param itemid	-1钻石，-2钻石
	 * @param goldnum	金币售价
	 * @param yuanbaonum   	钻石售价
	...
	public void  stReqStallsProductWasAddedMessage(Player player,ReqStallsProductWasAddedMessage msg ) {
		...
		if (goldnum == 0 && yuanbaonum == 0) {
			MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("请先输入对该物品的金币或钻石定价。"));
			return ;
		}
		...
		if (goldnum > 2100000000 || yuanbaonum > 2100000000) {
			MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("金币或钻石定价不能超过21亿。"));
			return;
		}
...
//				stStallsLog(player.getId(),-1,goodsitem,goldnum,yuanbaonum);	//数据库日志记录
				stStallsLog(player, null, -1L, goodsitem, goldnum, yuanbaonum); //数据库日志记录
			}else{
				MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您没{1}金币，无法上架。"),String.valueOf(itemnum));
				return;
			 }
		}else if (itemid == -2) {//上架钻石
//			Gold gold = ManagerPool.backpackManager.getGold(player);
			if(ManagerPool.backpackManager.checkGold(player, itemnum)) {
				...
				Item goodsitem = Item.createGold(itemnum,false); //产生一个钻石道具
				itemstr = JSONserializable.toString(goodsitem);
				issuccess = true;
//				stStallsLog(player.getId(),-1,goodsitem,goldnum,yuanbaonum);	//数据库日志记录
				stStallsLog(player, null, -1L, goodsitem, goldnum, yuanbaonum); //数据库日志记录
			}else {
				MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您没{1}钻石，无法上架。"),String.valueOf(itemnum));
				return;
			}
...
//				stStallsLog(player.getId(),-1,xitem, goldnum, yuanbaonum);	//数据库日志记录
				stStallsLog(player, null, -1L, xitem, goldnum, yuanbaonum); //数据库日志记录
	...
	public boolean setStallsGoods(Player player , Item item){
		...
		}else if (item.getId() == -2 || item.getItemModelId() == -2) {	//钻石
...
//			return true ;
		...
		if (item != null) {
			...
			if (issuccess == false) {
				...
				if (itemid > 0 && item.getItemModelId() > 0) {
					...
					ManagerPool.mailServerManager.sendSystemMail(msg.getStallsplayerid(),null,ResManager.getInstance().getString("系统邮件"),ResManager.getInstance().getString("摆摊金币上架失败返还"),(byte) 1,0,items);
				}else if (item.getItemModelId() == -2) {	//钻石
					ManagerPool.mailServerManager.sendSystemMail(msg.getStallsplayerid(),null,ResManager.getInstance().getString("系统邮件"),ResManager.getInstance().getString("摆摊钻石上架失败返还"),(byte) 1,0,items);
				}
	...
	public void  stReqStallsBuyMessage(Player player,ReqStallsBuyMessage msg) {
		...
		if(yuanbaonum > 0 && !ManagerPool.backpackManager.checkGold(player, yuanbaonum)){
			MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您的钻石数量不足，无法购买。"));
			return ;
		}
		
		//检测2级密码
		if (yuanbaonum > 0 && ManagerPool.protectManager.checkProtectStatus(player) ) {
			return;
		}
	...
	public void stResStallsBuycheckToGameMessage(ResStallsBuycheckToGameMessage msg){
		...
		if (msg.getStatus() == 0) {
			...
			if (msg.getPricegold() > 0 || msg.getPriceyuanbao() > 0) {
				...
				//检测2级密码
				if (msg.getPriceyuanbao() > 0 && ManagerPool.protectManager.checkProtectStatus(player) ) {
					return;
				}
	...
	public void stResStallsBuyDeductingToGameMessage( ResStallsBuyDeductingToGameMessage msg){
		...
		if (item != null) {
			...
			try{
				Player stallsplayer = PlayerManager.getInstance().getPlayer(msg.getStallsplayerid());
				Player buyplayer = PlayerManager.getInstance().getPlayer(msg.getPlayerid());
//				stStallsLog(msg.getStallsplayerid(),msg.getPlayerid(),item,goldnum,yuanbaonum);	//数据库日志记录
				stStallsLog(stallsplayer, buyplayer, buyplayer.getId(), item, goldnum, yuanbaonum);
			}catch (Exception e) {
				log.error(e, e);
			}
			...
			if (issuccess == false) {
				...
				if (itemid > 0 && item.getItemModelId() > 0) {
					...
					ManagerPool.mailServerManager.sendSystemMail(msg.getPlayerid(),null,ResManager.getInstance().getString("系统邮件"),ResManager.getInstance().getString("您从摊位购买的金币"),(byte)1,0,items);
				}else if (itemid == -2 || item.getItemModelId() == -2) {	//钻石
					ManagerPool.mailServerManager.sendSystemMail(msg.getPlayerid(),null,ResManager.getInstance().getString("系统邮件"),ResManager.getInstance().getString("您从摊位购买的钻石"),(byte)1,0,items);
				}
				
				if (player != null) {
					MessageUtil.notify_player(player,Notifys.SUCCESS,ResManager.getInstance().getString("由于您的包裹已满，或金币钻石达到上限，您从摊位购买的{1}*{2}已经从邮件发送给您。"),itemdata.getQ_name(),num+"");
				}
				...
				String txt = "";
				if (goldnum > 0) {
					txt = ResManager.getInstance().getString("金币(")+goldnum+") ";
				}
				if (yuanbaonum > 0) {
					txt = txt + ResManager.getInstance().getString("钻石(")+yuanbaonum+") ";
				}
...
//		Q_globalBean data =null;
//		
////		VipManager manager = new 
//		int vipid = VipManager.getInstance().getTransactionTax(player);
//			if(vipid>0){
//				 data =ManagerPool.dataManager.q_globalContainer.getMap().get(CommonConfig.STALL_RATE_VIP.getValue());
//			}else{
//				 data =ManagerPool.dataManager.q_globalContainer.getMap().get(CommonConfig.STALL_RATE.getValue());
//			}
		
		double stallRate = ((double)(10000-VipManager.getInstance().getTransactionTax(player)))/10000;
		
		if (msg.getPricegold() > 0 ) {
			int result = (int)(msg.getPricegold()*stallRate);
			if(result==0){
				result =1;
			}
			msg.setPricegold(result);
		}
		if (msg.getPriceyuanbao() > 0) {
			int result = (int)(msg.getPriceyuanbao()*stallRate);
			if(result==0){
				result =1;
			}
			msg.setPriceyuanbao(result);
		}
		...
		if (player != null) {
			...
			String str = "";
			if (msg.getPricegold() > 0 ) {
				str = ResManager.getInstance().getString("金币")+ msg.getPricegold();
			...
			if (msg.getPriceyuanbao() > 0) {
				str = str+ ResManager.getInstance().getString("钻石")+ msg.getPriceyuanbao();
//				Gold gold = ManagerPool.backpackManager.getGold(player);
				int playeryuanbao = 0;
				int recharge = 0;
...
//				if (playeryuanbao > 0 && recharge > 0) {
					if (ManagerPool.backpackManager.addGold(player,msg.getPriceyuanbao(),Reasons.STYBHUOKUAN,action)==false) {//摊主得到钻石
...
//				}else {
//					if (ManagerPool.backpackManager.changeTmpGold(player,msg.getPriceyuanbao())==false) {//摊主得到钻石
//						Item ybitem=Item.createGold(msg.getPriceyuanbao(), false);
//						List<Item> ybitems = new ArrayList<Item>();
//						ybitems.add(ybitem);
//						ManagerPool.mailServerManager.sendSystemMail(msg.getStallsplayerid(),null,ResManager.getInstance().getString("系统邮件"),ResManager.getInstance().getString("摆摊出售道具获得货款"),(byte)1,0,ybitems);
		...
		if (player != null) {
			...
			if (msg.getPricegold() > 0 ) {
				if (ManagerPool.backpackManager.changeMoney(player,msg.getPricegold(),Reasons.GOUMAISHIBAIGOLD,action) == false) {//买家得到金币
					...
					List<Item> items = new ArrayList<Item>();
					items.add(item);
					ManagerPool.mailServerManager.sendSystemMail(msg.getPlayerid(),null,ResManager.getInstance().getString("系统邮件"),ResManager.getInstance().getString("摊位商品购买失败，返还给您的金币"),(byte)1,0,items);
				}	
			}
...
//				Gold gold = ManagerPool.backpackManager.getGold(player);
				...
				if (playeryuanbao > 0 && recharge > 0) {
					if (ManagerPool.backpackManager.addGold(player,msg.getPriceyuanbao(),Reasons.STGOUMAISHIBAI,action) ==false) {//买家得到钻石
						type = 1;
					}
				}else {
					if (ManagerPool.backpackManager.changeTmpGold(player,msg.getPriceyuanbao())==false) {//买家得到钻石（放入临时钻石）
						type = 1;
					}
				}
				if (type == 1) {
					...
					List<Item> items = new ArrayList<Item>();
					items.add(item);
					ManagerPool.mailServerManager.sendSystemMail(msg.getPlayerid(),null,ResManager.getInstance().getString("系统邮件"),ResManager.getInstance().getString("摊位商品购买失败，返还给您的钻石"),(byte)1,0,items);
				}
	...
	public void stReqStallsOffShelfMessage(Player player,ReqStallsOffShelfMessage msg) {
		...
		if (msg.getGoodsid() > 0) {
			...
			if (num > Global.BAG_MAX_COPPER || num < 0) {
				MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，下架的金币加您身上携带金币已经达到上限，无法下架商品。"));
				return ;
			}
			...
			if (num > Global.BAG_MAX_GOLD || num < 0) {
				MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，下架的钻石加您身上携带钻石已经达到上限，无法下架商品。"));
				return ;
			}
		}
	...
	public void stResStallsOffShelfToGameMessage(ResStallsOffShelfToGameMessage msg ){
		...
		if (issuccess) {
			...
			if (item.getItemModelId() == -1 || item.getItemModelId() == -2) {
				name = ResManager.getInstance().getString("金币");
				if (item.getItemModelId() == -2) {
					name = ResManager.getInstance().getString("钻石");
				}
			}else{
...
//				stStallsLog(msg.getStallsplayerid(),-3,item,0,0);	//数据库日志记录
				stStallsLog(stallsplayer, null, -3L, item, 0, 0);
			}catch (Exception e) {
				log.error(e, e);
			}
		}else {
			...
			if (item.getId() > 0 && item.getItemModelId() > 0) {
				...
				ManagerPool.mailServerManager.sendSystemMail(msg.getStallsplayerid(),null,ResManager.getInstance().getString("系统邮件"),ResManager.getInstance().getString("从摊位下架的金币，由于下架后您不在线或者包裹已满，所以邮寄给您。"),(byte)1,0,items);
			}else if (item.getId() == -2 || item.getItemModelId() == -2) {	//钻石
				ManagerPool.mailServerManager.sendSystemMail(msg.getStallsplayerid(),null,ResManager.getInstance().getString("系统邮件"),ResManager.getInstance().getString("从摊位下架的钻石，由于下架后您不在线或者包裹已满，所以邮寄给您。"),(byte)2,0,items);
			}
			if (player !=null) {
...
//		}
		...
		wmsg.setPriceyuanbao(msg.getPriceyuanbao());
		wmsg.setStallsplayerid(player.getId());
		stStallsLog(player,-2,wmsg);	//数据库日志记录
		MessageUtil.send_to_world(wmsg);	//转发到世界服务器
	}
...
//	}
	...
	public void stReqStallsSearchMessage(Player player,ReqStallsSearchMessage msg) {
		...
		wmsg.setCan_use(msg.getCan_use());
		wmsg.setQ_job_limit(msg.getQ_job_limit());
		if(msg.getCan_use()==1){
			wmsg.setStrength(player.getAttibute_one()[0]);
			wmsg.setAgile(player.getAttibute_one()[1]);
			wmsg.setQ_job_limit((int)player.getJob());
		}
		wmsg.setAddAttribut(msg.getAddAttribut());
		wmsg.setIntensify(msg.getIntensify());
	
		
		wmsg.setQ_kind(msg.getQ_kind());
		wmsg.setQ_type(msg.getQ_type()+"");
		wmsg.setZhuoyue(msg.getZhuoyue());
		wmsg.setHidden(msg.getHidden());
	...
	/**
	 * 摆摊日志
	 * buyid -1商品上架，-2商品调整，-3商品下架 所以需要单独传入
	 */
	public void stStallsLog(Player stallsplayer, Player buyplayer, long buyId, Item item,int gold,int yuanbao) {
		...
		//
		log.setStallsuserid(stallsplayer.getUserId());
		log.setStallsusername(stallsplayer.getUserName());
		log.setStallsId(stallsplayer.getId());
		log.setStallsrolename(stallsplayer.getName());
		...
		if(buyId>0 && buyplayer!=null){
			log.setBuyuserid(buyplayer.getUserId());
			log.setBuyusername(buyplayer.getUserName());
			log.setBuyrolename(buyplayer.getName());
			log.setBuylevel(buyplayer.getLevel());
		}
	 ...
	 * @param msg
	 */
	public void stStallsLog(Player player,long buyId ,ReqStallsAdjustPricesToWorldMessage msg) {
//		if(!BackpackManager.getInstance().isLog(msg.getGoodsmodelid())){
//			return;
//		}
		StallsLog log = new StallsLog();
	
		log.setStallsId(player.getId());
		log.setStallsrolename(player.getName());
		log.setStallsusername(player.getUserName());
		log.setStallsuserid(player.getUserId());
