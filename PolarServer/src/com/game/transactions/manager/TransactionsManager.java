...
//			appearanceInfo.setHorseduangu((short) horse.getMixingbone());
		}
		...
		for (int i = 0; i < player.getEquips().length; i++) {
			...
			if (equip != null) {
				...
				appearanceInfo.getEquips().add(equipinfo);
			} else { // 如果该位置没有武器则添加空武器
				appearanceInfo.getEquips().add(new EquipInfo());
			}
		}
	...
	public void stTransactionsLaunch(Player aplayer ,long bplayerid) {
		...
		if (aplayer.getPkValue() > 0){
			MessageUtil.notify_player(aplayer, Notifys.ERROR, ResManager.getInstance().getString("魔头不能交易"));
			return;
		}
		
		if (bplayer.getPkValue() > 0){
			MessageUtil.notify_player(aplayer, Notifys.ERROR, ResManager.getInstance().getString("不能和魔头进行交易"));
			return;
		}
		...
		if (ts == null || (ts != null && ts.getAid() == 0 && ts.getBid() == 0  )) {
			...
			rolemode.setPlayername(aplayer.getName());
			rolemode.setJob(aplayer.getJob());
	...
	public void stTransactionsAccept(Player bplayer,long tsid) {
		...
		if (aplayer != null) {
			...
			arolemode.setJob(aplayer.getJob());
			...
			brolemode.setJob(bplayer.getJob());
	...
	public void  stTransactionsChangeGold(Player player ,int gold){
		...
		if (transactions != null) {
			...
			if (player.getMoney() < gold || gold < 0) {
				MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您输入的金币数量少于携带数量。"));
				ResTransactionsErrorMessage emsg = new ResTransactionsErrorMessage();
				emsg.setErrorid(2);
				MessageUtil.tell_player_message(player, emsg);
				return;
	...
	public void  stTransactionsChangeYuanbao(Player player ,int yuanbao){
		...
		if (transactions != null) {
			...
			if (playeryuanbao < yuanbao || yuanbao < 0) {
				MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您输入的钻石数量少于携带数量。"));
				ResTransactionsErrorMessage emsg = new ResTransactionsErrorMessage();
				emsg.setErrorid(3);
				MessageUtil.tell_player_message(player, emsg);
				return;
			...
			if (tsinfo.getYuanbao() != yuanbao ) {
				...
				if(!ManagerPool.backpackManager.checkGold(player, yuanbao) || ManagerPool.protectManager.checkProtectStatus(player)){//检测钻石锁
					ResTransactionsErrorMessage emsg = new ResTransactionsErrorMessage();
					emsg.setErrorid(3);
					MessageUtil.tell_player_message(player, emsg);
					return;
	...
	public void stTransactionsSetState(Player aplayer,byte state) {
		...
		if (transactions != null) {
			...
			if (atsinfo.getGold() < 0) {
				MessageUtil.notify_player(aplayer,Notifys.ERROR,ResManager.getInstance().getString("金币不能小于0。"));
				return;
			}
			if (atsinfo.getYuanbao() < 0) {
				MessageUtil.notify_player(aplayer,Notifys.ERROR,ResManager.getInstance().getString("钻石数量不能小于0。"));
				return;
			}
			...
			if (atsinfo.getStatus() == 2 && xtsinfo.getStatus() == 2) {
				...
				if (agold > 0 && aplayer.getMoney() < agold) {
					MessageUtil.notify_player(xplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，对方的携带的金币数少于交易的数量，交易失败。"));
					MessageUtil.notify_player(aplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您的背包携带的金币数少于交易的数量，交易失败。"));
					stTransactionsCanceled(aplayer);
					return;
				}
				if (ayuanbao > 0 && aplayeryuanbao < ayuanbao) {
					MessageUtil.notify_player(xplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，对方的携带的钻石数少于交易的数量，交易失败。"));
					MessageUtil.notify_player(aplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您的背包携带的钻石数少于交易的数量，交易失败。"));
					stTransactionsCanceled(aplayer);
					return;
				}

				if (xgold > 0 && xplayer.getMoney() < xgold) {
					MessageUtil.notify_player(aplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，对方的携带的金币数少于交易的数量，交易失败。"));
					MessageUtil.notify_player(xplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您的背包携带的金币数少于交易的数量，交易失败。"));
					stTransactionsCanceled(aplayer);
					return;
				}
				if (xyuanbao > 0 && xplayeryuanbao < xyuanbao) {
					MessageUtil.notify_player(aplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，对方的携带的钻石数少于交易的数量，交易失败。"));
					MessageUtil.notify_player(xplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您的背包携带的钻石数少于交易的数量，交易失败。"));
					stTransactionsCanceled(aplayer);
					return;
				}

				if ((agold > 0 && xplayer.getMoney() + agold > Global.BAG_MAX_COPPER ) || (xplayer.getMoney() + agold < 0)) {	
					MessageUtil.notify_player(aplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，交易的金币数超过了对方背包的负荷，本次交易失败了。"));
					MessageUtil.notify_player(xplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您的背包携带的金币数过多，本次交易失败了。"));
					stTransactionsCanceled(aplayer);
					return;
				}
				
				if ((ayuanbao > 0 && xplayeryuanbao + ayuanbao > Global.BAG_MAX_GOLD) || (xplayeryuanbao + ayuanbao < 0)){
					MessageUtil.notify_player(aplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，交易的钻石数超过了对方背包的负荷，本次交易失败了。"));
					MessageUtil.notify_player(xplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您的背包携带的钻石数过多，本次交易失败了。"));
					stTransactionsCanceled(aplayer);
					return;
				}
				
				if ((xgold > 0 && aplayer.getMoney() + xgold > Global.BAG_MAX_COPPER )  || (aplayer.getMoney() + xgold < 0)) {
					MessageUtil.notify_player(xplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，交易的金币数超过了对方背包的负荷，本次交易失败了。"));
					MessageUtil.notify_player(aplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您的背包携带的金币数过多，本次交易失败了。"));
					stTransactionsCanceled(aplayer);
					return;
				}
				
				if ((xyuanbao > 0 && aplayeryuanbao + xyuanbao > Global.BAG_MAX_GOLD) || ( aplayeryuanbao + xyuanbao < 0)) {
					MessageUtil.notify_player(xplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，交易的钻石数超过了对方背包的负荷，本次交易失败了。"));
					MessageUtil.notify_player(aplayer,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您的背包携带的钻石数过多，本次交易失败了。"));
					stTransactionsCanceled(aplayer);
					return;
				}
				...
				//先减后加(金币和钻石)
				 ...
				 if (agold > 0) {
					 if (ManagerPool.backpackManager.changeMoney(aplayer,-agold,Reasons.JIAOYIGOLD,logid)) {
						 ...
						 ists = true;
						 xstr.append( ResManager.getInstance().getString(" 金币")+agold);
					}
				 }
				 ...
				 if (xgold > 0) {
					 if (ManagerPool.backpackManager.changeMoney(xplayer,-xgold,Reasons.JIAOYIGOLD,logid)) {
						 ...
						 ists = true;
						 astr.append( ResManager.getInstance().getString(" 金币")+xgold);
					}	
				 }
				 ...
				 if (ayuanbao > 0) {
					 if (ManagerPool.backpackManager.changeGold(aplayer,-ayuanbao,Reasons.JIAOYIYB,logid)) {
						 if (xplayeryuanbao > 0 && xrecharge > 0 ) {
							...
							saveTmpyuanbaolog(xplayer,aplayer.getId(),aplayer.getName(),(byte)1,ayuanbao);//可领取钻石日志记录
							
						 }
						 sttransactionslog(aplayer,xplayer,-2,ayuanbao,logid);	 
						 ists = true;
						 xstr.append( ResManager.getInstance().getString(" 钻石")+ayuanbao);
					}
				 }
				 ...
				 if (xyuanbao > 0) {
					 if (ManagerPool.backpackManager.changeGold(xplayer,-xyuanbao,Reasons.JIAOYIYB,logid)) {
						 if (aplayeryuanbao > 0 && arecharge > 0) {
							...
							saveTmpyuanbaolog(aplayer,xplayer.getId(),xplayer.getName(),(byte)1,xyuanbao);//可领取钻石日志记录
							
						}	
						 sttransactionslog(xplayer,aplayer,-2,xyuanbao,logid);	
						 ists = true;
						 astr.append( ResManager.getInstance().getString(" 钻石")+xyuanbao);
					}
				 }
...
//				System.out.println("---------------------------------------------------------aplayer:"+aplayer.getId());
//				System.out.println("---------------------------------------------------------xplayer:"+xplayer.getId());
				MessageUtil.tell_player_message(aplayer, msg);
				msg.getRoleId().remove(aplayer.getId());
				MessageUtil.tell_player_message(xplayer, msg);
			}
		}
	}
	...
	public void sttransactionslog(Player outplayer,Player intoplayer ,Item item,long logid) {
		 ...
		 log.setIntouserid(intoplayer.getUserId());
		 log.setIntousername(intoplayer.getUserName());
		 log.setIntoId(intoplayer.getId());
		 log.setIntorolename(intoplayer.getName());
		 log.setIntolevel(intoplayer.getLevel());
		 log.setOutuserid(outplayer.getUserId());
		 log.setOutusername(outplayer.getUserName());
		 log.setOutId(outplayer.getId());
		 log.setOutrolename(outplayer.getName());
		 log.setOutlevel(outplayer.getLevel());
	...
	public void sttransactionslog(Player outplayer,Player intoplayer ,int type , int num, long logid) {
		 ...
		 if (type == -1) {
			 typename = ResManager.getInstance().getString("金币");
		}else if (type == -2) {
			typename = ResManager.getInstance().getString("钻石");
		}
	 ...
	 * @param rid
	 * @param name
	 * @param type    1交易，2摆摊货物卖出，3摆摊购买钻石
	 * @param yuanbao
	 */
		...
		if (tyb > 0) {
			...
			if (yuanbao > 0 &&  recharge > 0) {
				if (yuanbao + tyb >= Global.BAG_MAX_GOLD) {
					MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您的可携带钻石数量已经达到上限，无法领取。"));
					return;
				}
				if (ManagerPool.backpackManager.changeTmpGold(player,-tyb)){
					 ...
					 MessageUtil.tell_player_message(player, tymsg);
					 MessageUtil.notify_player(player,Notifys.NORMAL,ResManager.getInstance().getString("恭喜您成功领取{1}钻石。"),String.valueOf(tyb));
				}	
			 }else {
				 MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您的钻石账户未激活，请充值至少1钻石激活后再领取。"));
			}
		}else {
			MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("很抱歉，您没有可领取的钻石。"));
		}
	}
