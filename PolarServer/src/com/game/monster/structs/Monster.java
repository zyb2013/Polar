...
public class Monster extends Person implements IMapObject, Fighter, MemoryObject {
	...
	//被那个技能杀死
	private int beskillid;
	
	

	public int getBeskillid() {
		return beskillid;
	}

	public void setBeskillid(int beskillid) {
...
//			Grid monsterGrid = MapUtils.getGrid(this.getPosition(), blocks);
			...
			while (iter.hasNext()) {
				...
				//仇恨对象是召唤怪
				if(hatred.getTarget() instanceof SummonPet && hatred.getTarget().isDie()){
					continue;
				}
...
//				
	...
	public void reset(){
		...
		//变身清除
		this.getMorph().clear();
		//参数清除    恶魔广场 保留 参数 用于复活
		if(this.getMapModelId()<300001 || this.getMapModelId() > 300008){
		...
		try{
			//获得怪物模型
			Q_monsterBean model = ManagerPool.dataManager.q_monsterContainer.getMap().get(this.getModelId());
//			System.out.println("ID:"+model.getQ_id());
//			System.out.println("model.getQ_variation():"+model.getQ_variation());
		...
		try {
			if (model.getQ_monster_ai() != null  && model.getQ_monster_ai().length() > 0 ) {
				...
				for (int i = 0; i < skilllarray.size(); i++) {
					...
					Integer hpRangeMin = 0;
					Integer hpRangeMax = 0;
					if (skillHashMap.containsKey(ResManager.getInstance().getString("血量范围"))) {
						String hpRange = (String)skillHashMap.get(ResManager.getInstance().getString("血量范围"));							
						String[] ranges = hpRange.split(Symbol.XIAHUAXIAN_REG);
						
						hpRangeMin = Integer.valueOf(ranges[0]);
						hpRangeMax = Integer.valueOf(ranges[1]);
					}
					...
					//{类型:1,血量范围:0_5,间隔:11,几率:400,说话:"怪物头顶冒字内容",预警:10000,触发:1001,对象:1},
					if ((RandomUtils.random(10000) <= rnd  || (this.getNextskillid()!= null && this.getNextskillid().length() > 0)) &&
						(nowhp >= hpRangeMin && nowhp <= hpRangeMax)) {
						//获取目标
						...
						if (this.getNextskillid() != null && this.getNextskillid().length() > 0) {	//有预设技能，直接释放
							...
							if(warnid != null && warnid.length() > 2){
								if(this.getNextskillid() == null){
									this.setNextskillid(skillid);
									skillid = warnid;
									attack = true;
								}else if(this.getNextskillid().length()== 0){
									this.setNextskillid(skillid);
									skillid = warnid;
									attack = true;
								}
...
//								ManagerPool.cooldownManager.addCooldown(this, CooldownTypes.MONSTER_ACTION, null, (long)500);
		...
		Skill monsterAISkill = getMonsterAISkillType4();
		if (monsterAISkill != null){
			return monsterAISkill;
		}
...
//						}
	...
	/**
	 * 怪物掉血触发技能 (新加类型为4，进入这个血量范围，就把这个当作默认技能)
	 * hide by xuliang
	 */
	/*
	public void getmoreHpChangeSkill(){
		Skill monsterAISkill = getMonsterAISkillType4(true);
		
		if (monsterAISkill != null){
			ManagerPool.fightManager.monsterAttackPlayer(this, (Player)target, monsterAISkill);
		}
	}
	*/
	 ...
	 * add by xuliang
	 */
	private Skill getMonsterAISkillType4(){
		Q_monsterBean model = ManagerPool.dataManager.q_monsterContainer.getMap().get(this.getModelId());
		Skill monsterAiSkill = new Skill();
		...
		try {
			if (model.getQ_monster_ai() != null  && model.getQ_monster_ai().length() > 0 ) {
				...
				for (int i = 0; i < skilllarray.size(); i++) {
					net.sf.json.JSONObject skillHashMap = skilllarray.getJSONObject(i);
					Integer  type = (Integer) skillHashMap.get(ResManager.getInstance().getString("类型"));
					//{类型:4,血量范围:0_5,几率:333,说话:"内容",预警:10000,触发:1222,对象:1}
					if (type == 4) {//掉血触发
						int  nowhp =acqHppercentage();
						...
						if(nowhp >= hpRangeMin && nowhp <= hpRangeMax){
							if (RandomUtils.random(10000) <= rnd) {
								//获取目标
								Fighter target = getDefaultAttackTarget();
								if(isAattack(target,skillid)){
//									if (say != null && !say.equals("") && isSay){
//										ResMonsterSayMessage msg = new ResMonsterSayMessage();
//										msg.setSaycontent(say);
//										msg.setMonsterId(this.getId());
//										MessageUtil.tell_round_message(this, msg);
//									}
									...
									monsterAiSkill.setSkillLevel(skillModel.getQ_grade());
									monsterAiSkill.setSkillModelId(skillModel.getQ_skillID());
									return monsterAiSkill;
								}
							}
						}
