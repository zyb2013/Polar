...
//		long start = System.currentTimeMillis();
		...
		{
//			System.out.println("进入怪物列表检查。。。" + System.currentTimeMillis());
			//怪物死亡
			...
			if(monster.getDisappearTime()>0 && System.currentTimeMillis()>monster.getDisappearTime()){
				...
				for (Area area2 : areas) {
					...
					Iterator<SummonPet> iter2 = area2.getSummonpets().iterator();
					while (iter2.hasNext()) {
						SummonPet summonpet = (SummonPet) iter2.next();
						summonpet.getAttackTargets().remove(monster);
					}	
				}
...
//				}
				...
				//如果取不到beskillid ,填-1
				msg.setSkillid(monster.getBeskillid());
				MessageUtil.tell_round_message(monster, msg);
