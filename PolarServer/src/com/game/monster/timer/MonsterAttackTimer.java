...
//			for (Monster monster : monsters) {
		...
		while (iter.hasNext()) {
			...
			//! xuliang add, 防止多次使用默认技能而出现AI技能概率不一致的情况
			
			if (ManagerPool.cooldownManager.isCooldowning(monster, CooldownTypes.SKILL, skill.getSkillModelId() + "")){
				continue;
			}
			...
			if(distance <= skillModel.getQ_range_limit() - 1){
				...
				if(attSkill!=null){
					if(target instanceof Player){
						...
						ManagerPool.fightManager.monsterAttackPlayer(monster, (SummonPet)target, attSkill);
					}
				}
			}
		}
