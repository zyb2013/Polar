...
public class MonsterBossAiTimer extends TimerEvent {
	...
	public MonsterBossAiTimer(int serverId, int lineId, int mapId){
		super(-1, 500);//boss设置为0.5秒，小怪为2秒
...
//					monster.countHatreds();
			...
			//判断是不是BOSS
			if(model.getQ_monster_type() == MonsterManager.MONSTER_TYPE_COMMON){
				continue;
			}
			//木桩式怪物
			...
			//在巡逻范围以外 或 xuliang add 走到安全区

			
			if(distance > model.getQ_pursuit() || 
				ManagerPool.mapManager.isSafe(monster.getPosition(), monster.getMapModelId())){
				//停止走路
				monster.getRoads().clear();
				//寻路回到出生点
				roads = MapUtils.findRoads(blocks, monster.getPosition(), birth, -1, false);
