...
//		MailDetailInfoData mailDetailInfoData = loadmail(message.getMailid());
	...
	public void reqMailGetItemToServer(Player player, ReqMailGetItemToServerMessage message, boolean borefresh) {
		...
		//要取得的附件物品编号,为-1表示全部收取,为0表示收取金币，其他表示真正物品编号
		MailDetailInfoData mailDetailInfoData = player.getMaildataMap().get(message.getMailid());
//		MailDetailInfoData mailDetailInfoData = loadmail(message.getMailid());
		if (mailDetailInfoData != null) {
			mailDetailInfoData.setSaveType(SaveMailThread.MAIL_UPDATE);
			...
			if (message.getItemid() == -1) {
						...
						if (item.getItemModelId() > 0) {
							...
							if (item.getItemModelId() == -1) {
								...
								if (money < 0 || money > Global.BAG_MAX_COPPER) {
									MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("收取金币失败"));
									return;
								}
								...
								if (getgold > 0) {
									...
									if (gold < 0 || gold > Global.BAG_MAX_GOLD) {
										MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("收取钻石失败"));
										return;
									}
								} else {
								...
								if (bindgold < 0 || bindgold > Global.BAG_MAX_GOLD) {
									MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("收取绑钻失败"));
									return;
								}
				...
				if (mailDetailInfoData.getnGold() != 0) {
					...
					} else if (mailDetailInfoData.getBtGoldType() == 2) {//钻石
						if (getgold > 0) {
				...
				MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM, ResManager.getInstance().getString("从邮件获得：{1}"), str);
...
//		}
	...
	public void reqMailSendNewMailToServer(Player player, ReqMailSendNewMailToServerMessage message) {
		...
		} else if (message.getBtGoldType() == 2) {//钻石
	...
	public void reqMailDeleteMailToServer(Player player, ReqMailDeleteMailToServerMessage message) {
		if (message.getBtdeleteall() == 1) {
			...
			if (bDelete){
				...
				MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("邮件删除失败"));
			}
		}
		ResMailQueryListToClientMessage sendMessage = new ResMailQueryListToClientMessage();
		sendMessage.setBtErrorCode(Error_Fail);
...
//		}
	...
	*/
	 ...
	 * GM命令发送系统邮件
	 */
	public void gmSendSystemMail(Player player, String[] strs){
		List<Item> createItems = new ArrayList<Item>();
		
		long receiverid = Long.valueOf(strs[1]);
		String receivername = "";
		if (receiverid == 0){
			receiverid = player.getId();
			receivername = player.getName();
			
		}else{
			receivername = PlayerManager.getInstance().getPlayer(receiverid).getName();
		}
		
		String title = strs[2];
		String notice = strs[3];
		byte goldtype = Byte.valueOf(strs[4]);
		int num = Integer.parseInt(strs[5]);
		
		try{
			createItems = Item.createItems(Integer.parseInt(strs[6]), Integer.parseInt(strs[7]), false, 0);
		}catch(ArrayIndexOutOfBoundsException e){
		}finally{
			sendSystemMail(receiverid, receivername, title, notice, goldtype, num, createItems);
		}
	}
