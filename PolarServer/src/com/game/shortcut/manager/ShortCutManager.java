...
import com.game.shortcut.message.ResGetAssistantMessage;
...
public class ShortCutManager {
	...
	public void sendShortcutInfos(Player player) {
		...
		// 第20位是战斗小助手
		for (int i = 0; i < player.getShortCuts().length - 1; i++) {
	 ...
	 * @param shortcutType
	 *            快捷类型 1-物品 2-技能
	 * @param shortcutSource
	 *            指向目标
	 * @param shortcutSourceModel
	 *            指向模板
	 * @param shortcutGrid
	 *            放置格子
	 */
	public void addShortCut(Player player, int shortcutType,
			long shortcutSource, int shortcutSourceModel, int shortcutGrid) {
		// 前台发送格子位置从1开始
		shortcutGrid -= 1;
		// 检查位置
		if (shortcutGrid >= player.getShortCuts().length - 1 || shortcutGrid < 0)
			return;

		// 检查是否可以放置
		if (shortcutType == 1) {
			// 物品检查
			Q_itemBean item = ManagerPool.dataManager.q_itemContainer.getMap()
					.get(shortcutSourceModel);
			if (item == null)
				return;
			...
			Skill skill = ManagerPool.skillManager.getSkillByModelId(player,
					shortcutSourceModel);
			if (skill == null)
				return;
			// 技能检查

			// Q_skill_modelBean skillModel =
			// ManagerPool.dataManager.q_skill_modelContainer.getMap().get(SkillManager.skillKey(player,
		...
		// 检查技能是否已经存在于面板 如果存在则进行移动操作
		for (int i = 0; i < player.getShortCuts().length; i++) {
			ShortCut shortcut = player.getShortCuts()[i];
			if (shortcut != null
					&& shortcut.getShortcutSource() == shortcutSource) {
				moveShortCut(player, shortcut.getId(), shortcutGrid + 1);
				return;
			}
		}
		// end 检查技能是否存在于面板中
		...
		MessageUtil.tell_player_message(player,
				getShortCutAddMessage(shortcut, shortcutGrid + 1));
	...
	public void removeShortCut(Player player, long shortcutId) {
		...
		MessageUtil.tell_player_message(player,
				getShortCutRemoveMessage(grid + 1));
	}

	public void removeShortCut(Player player, int skillModelId) {
		// 获得快捷
		int grid = -1;
		for (int i = 0; i < player.getShortCuts().length - 1; i++) {
			ShortCut shortcut = player.getShortCuts()[i];
			if (shortcut == null)
				continue;
			if (shortcut.getShortcutType() == 2 && shortcut.getShortcutSourceModel() == skillModelId) {
				grid = i;
				break;
			}
		}
	...
	public void moveShortCut(Player player, long shortcutId, int toGrid) {
		...
		if (toGrid >= player.getShortCuts().length - 1 || toGrid < 0)
			return;
		...
		if (toShortCut == null) {
			...
			MessageUtil.tell_player_message(
					player,
					getShortCutAddMessage(player.getShortCuts()[toGrid],
							toGrid + 1));
			MessageUtil.tell_player_message(player,
					getShortCutRemoveMessage(grid + 1));
		} else {
			...
			MessageUtil
					.tell_player_message(
							player,
							getShortCutAddMessage(player.getShortCuts()[grid],
									grid + 1));
		}
	}
	...
	public void saveAssistantInfo(Player player, String saveString) {
		ShortCut shortcut = player.getShortCuts()[20];
		if (shortcut == null) {
			shortcut = new ShortCut();
			shortcut.setId(Config.getId());
		}
		shortcut.setAssistant(saveString);
		player.getShortCuts()[20] = shortcut;
		ManagerPool.playerManager.savePlayer(player);
	}

	public void getAssistantInfo(Player player) {
		ShortCut shortcut = player.getShortCuts()[20];
		ResGetAssistantMessage msg = new ResGetAssistantMessage();
		msg.setSaveString(shortcut == null ? "" : shortcut.getAssistant());
		MessageUtil.tell_player_message(player,msg);
	}
}
