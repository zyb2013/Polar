...
//	private static final int COMMONSEXTYPE=0;
	 ...
	 * 灵魂宝石 物品ID
	 */
	private static final int SOULD_IAMOND_ID = 600002;
	/**
	 * 1NPC商店
	 */
...
//	private static final int DROP_ABLE=1;
	...
	//是否给熔炼石
	private boolean isgivestone = true;
	 ...
	 * @param roleId 玩家Id
	 * @param npcId NPC Id
	 * @param selllId 贩卖Id 绑钻
	 * @param num 购买数量
	 */
	public void buyItem(Player player, int npcId, int sellId, int num, int costType, int coin, int bindgold, int gold,int sould) {
		//数据检查
		if (num <= 0) {
			return;
		}
		
		//! add by xuliang 增加玩家PK值检查
		if (player.getPkValue() > 0 && costType == 1){
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("魔头不能使用"));
			return;
		}
		...
		//&& player.getLevel() < 10
//		if (shopItem.getQ_shop_type() == SHOP_TYPE_PORTABLE ) {
//			//10级（不包含）以上开启随身商店
//			return;
//		}
		/** begin 这些特殊  的验证   在我们的项目 中   可能   不通用  暂时先 去掉       by :汪振瀚
...
//		}
		...
		List<Item> createItems = Item.createItems(model.getQ_id(), num, bind, losttime, shopItem.getQ_strengthen(), shopItem.getQ_extra_attribute(), shopItem.getQ_append());;
...
//			if ((shopItem.getQ_money_type() & 0x00000001) == 0) {
				if (!shopItem.getQ_money_type().contains("1")) {
				return;
			}
			//金币购买
			int price = 0;
			...
			if (player.getMoney() < money) {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("所需金币不足"));
				return;
			}
			...
			if (num > 1) {
				MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("购买[{1}]x{2}成功，使用{3}金币"), BackpackManager.getInstance().getName(shopItem.getQ_sell()), num + "", String.valueOf(money));
			} else {
				MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("购买[{1}]成功，使用{2}金币"), BackpackManager.getInstance().getName(shopItem.getQ_sell()), String.valueOf(money));
			}
		} else if (costType == 2) {
			//允许钻石购买
			if (!shopItem.getQ_money_type().contains("2")) {
				return;
			}
			...
			if (num > 1) {
				MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("购买[{1}]x{2}成功，使用{3}钻石"), BackpackManager.getInstance().getName(shopItem.getQ_sell()), num + "", String.valueOf(money));
			} else {
				MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("购买[{1}]成功，使用{2}钻石"), BackpackManager.getInstance().getName(shopItem.getQ_sell()), String.valueOf(money));
			}
			//购买成功后检测是否需要刷新商城列表
			...
			bind = true;
			//绑钻
			if (!shopItem.getQ_money_type().contains("4")) {
				return;
			}
			...
			if (player.getBindGold() < money) {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("所需绑钻不足"));
				return;
			}
...
//			int preCostGold = player.getGold()==null?0:player.getGold().getCostGold();
			...
			ManagerPool.backpackManager.addItems(player, createItems, Reasons.SHOPBUY, action);
			if (num > 1) {
				MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("购买[{1}]x{2}成功，使用{3}绑钻"), BackpackManager.getInstance().getName(shopItem.getQ_sell()), num + "", String.valueOf(money));
			} else {
				MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("购买[{1}]成功，使用{2}绑钻"), BackpackManager.getInstance().getName(shopItem.getQ_sell()), String.valueOf(money));
			}
//			//购买成功后检测是否需要刷新商城列表
//			if(needRefreshList(preCostGold,player.getGold().getCostGold())){
//				shopItemList(player, 2); // 这里的 2 是数据库中的 q_model_id 商店模型id
//			}
			...
			//是否允许用灵魂宝石购买
			if (!shopItem.getQ_money_type().contains("3")) {
				return;
			}
			bind = true;
			int  playerSouldCount =ManagerPool.backpackManager.getItemNum(player, SOULD_IAMOND_ID);
			
			int price = shopItem.getQ_souldiamond();
			if (isdaze) {
				price = getDaZhePrice(shopItem.getQ_show_bindgold(), shopItem.getQ_sale_rate());
			}
			if (price != sould) {
			...
			int money = price * num;
			resume = money;
			if (playerSouldCount < money) {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("所需灵魂宝石不足"));
				return;
			}
//			Gold gold = BackpackManager.getInstance().getGold(player);
//			int preCostGold = player.getGold()==null?0:player.getGold().getCostGold();
			...
			boolean changeSould =ManagerPool.backpackManager.removeItem(player, SOULD_IAMOND_ID,resume, Reasons.GOODUSE, action);
//			System.out.println("-----------------------扣除身上灵魂宝石结果:"+changeSould);
			if (!changeSould) {
				//扣除操作失败
				return;
			}
			...
			ManagerPool.backpackManager.addItems(player, createItems, Reasons.SHOPBUY, action);
			if (num > 1) {
				MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("购买[{1}]x{2}成功，使用{3}灵魂宝石"), BackpackManager.getInstance().getName(shopItem.getQ_sell()), num + "", String.valueOf(money));
			} else {
				MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("购买[{1}]成功，使用{2}灵魂宝石"), BackpackManager.getInstance().getName(shopItem.getQ_sell()), String.valueOf(money));
			}
//			//购买成功后检测是否需要刷新商城列表
//			if(needRefreshList(preCostGold,player.getGold().getCostGold())){
//				shopItemList(player, 2); // 这里的 2 是数据库中的 q_model_id 商店模型id
//			}
		...
		if (BackpackManager.getInstance().isLog(model.getQ_id())) {
			...
			shopBuyLog.setLoginip(player.getLoginIp());
	...
	public void sellItem(Player player, long itemId) {
		...
		if (ManagerPool.backpackManager.changeMoney(player, price, Reasons.SHOPSELL, action)) {
			...
			item.setGridId(-1);
			if (item instanceof Equip && isgivestone) {
				ManagerPool.meltingManager.sellEquipGetMelting(player, (Equip)item);
			}
		} else {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("出售失败，背包中金币数量已达上限。"));
			return;
		}

		if (item.getNum() > 1) {
			MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("出售[{1}]({2})成功，获得{3}金币"), BackpackManager.getInstance().getName(model.getQ_id()), item.getNum() + "", String.valueOf(price));
		} else {
			MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("出售[{1}]成功，获得{2}金币"), BackpackManager.getInstance().getName(model.getQ_id()), String.valueOf(price));
		}

		if (BackpackManager.getInstance().isLog(model.getQ_id())) {
			ShopSellLog selllog = new ShopSellLog();
			selllog.setUsername(player.getUserName());
			selllog.setUserid(Long.valueOf(player.getUserId()));
			selllog.setActionId(action);
			selllog.setRoleId(player.getId());
			selllog.setRolename(player.getName());
			selllog.setRolelevel(player.getLevel());
...
//		Gold gold = BackpackManager.getInstance().getGold(player);
	...
	public void buyBack(Player player, long ItemId) {
		...
		if (player.getMoney() < price) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，所需游戏金币不足"));
			return;
		}
		...
		if (item instanceof Equip && ManagerPool.meltingManager.checkEquipGetMelting((Equip) item)) {
			...
			if (!ManagerPool.backpackManager.checkGold(player, 100)) {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，回购装备等级60以上的紫装需要100钻石"));
				return;
			}
			//检测2级密码
			if (ManagerPool.protectManager.checkProtectStatus(player) ) {
			...
			if (ManagerPool.backpackManager.changeGold(player, -100, Reasons.MELTING_BUYITEM, action)) {
				...
				MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("回购【{1}】成功使用{2}钻石"), BackpackManager.getInstance().getName(q_itemBean.getQ_id()), String.valueOf(100));
				MessageUtil.tell_player_message(player, msg);
			...
			if (ManagerPool.backpackManager.changeMoney(player, -price, Reasons.SHOPBUY, action)) {
				...
				MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("回购【{1}】成功使用{2}金币"), BackpackManager.getInstance().getName(q_itemBean.getQ_id()), String.valueOf(price));
				MessageUtil.tell_player_message(player, msg);
				if (BackpackManager.getInstance().isLog(remove.getItemModelId())) {
...
//	}
	...
	public boolean autoRemove(Player player, int itemModelId, int num, Reasons reasons, long action) {
		...
		if (num == 0) {
			//检测2级密码
			if (costGold > 0 && ManagerPool.protectManager.checkProtectStatus(player) ) {
				return false;
			}
		...
		for (Q_shopBean q_shopBean : list) {
			if (2 == q_shopBean.getQ_model_id()) {  //这里的2是商店模型id  数据库中的q_model_id
				...
				if ((SHOP_SHANGJIA).equals(q_rack) || TimeUtil.isNowSatisfiedBy(q_rack)) {
					//上架商品 或 满足配置时间段的商品 检查是否满足消耗钻石的要求
					int needcost = q_shopBean.getQ_show_cost();
					if (needcost > 0) {
	...
	public void shopList(Player player, int shopModelId, byte page, boolean gridelimit) {
		...
		if (shopModelId == 10000 && player.getPkValue() > 0){
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("魔头不能使用随身商店"));
			return;
		}
...
//		int addcount = 0;
		...
		for (Q_shopBean shopModel : list) {
			try {
				...
				if(shopModel.getQ_shop_limit() > 0 ){
					String key = shopModel.getQ_id() +"_"+shopModel.getQ_rack();
					if(player.getShoplimitmap().containsKey(key)){
						buildShopItemInfo.setBuy_count(player.getShoplimitmap().get(key));
						buildShopItemInfo.setLimit_count(shopModel.getQ_shop_limit());
					}
				}
...
//				if(shopModel.getQ_shop_type()==0){
//					addcount =VipManager.getInstance().getShopCost(player) ;
//				}
			} catch (Exception e) {
				log.error(e, e);
			}
...
//		if(addcount!=0){
//			if (!BackpackManager.getInstance().changeGold(player, -addcount, Reasons.YBBUY, Config.getId())) {
//				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("没冲VIP，要 2钻石 才能看哦 "));
//				return;
//			}
//		}
...
//			info.setCoin(getDaZhePrice(model.getQ_show_coin(),model.getQ_sale_rate()));
		...
		info.setMoneyType(model.getQ_money_type());
		...
		info.setQ_zhuoyue(model.getQ_zhuoyue()); 
		info.setQ_extra_attribute(model.getQ_extra_attribute());
		info.setQ_lucky(model.getQ_lucky());
		info.setQ_suitable_level_max(model.getQ_suitable_level_max());
		info.setQ_suitable_level_min(model.getQ_suitable_level_min());
		info.setQ_show_level(model.getQ_show_level());
		info.setQ_souldiamond(model.getQ_souldiamond());
		info.setQ_show_souldiamond(model.getQ_show_souldiamond());
		return info;
	}
	 ...
	 * 打折价格 小于等于零为非法请求
	 *
	 * @param costtype 1 金币 2钻石 3绑定钻石 其它的为非法请求 返回负数
	 * @param modelId
	 * @return
	 */
	...
	public boolean isIsgivestone() {
		return isgivestone;
	}

	public void setIsgivestone(boolean isgivestone) {
		this.isgivestone = isgivestone;
	}
}
