...
public abstract class Task extends GameObject implements Comparator<Task>{
	...
	/***
	 *  扩展任务类型：0=主线（任务id从10000开始），5=支线（任务id从50000开始），6=转职（任务id从60000开始）
		其中转职任务新建表q_task_changejob，表结构和q_task_main相同
		主线、支线、转职、活动、福利这些活动的结构和主线MainTask一致,所以就不用区别对待，当成主线来处理就行，加载的时候要合并几个表
	 */
	/**
	 * 支线任务
	 */
	public static transient final byte BRANCH_TASK = 5;
	
	/**
	 * 转职任务
	 */
	public static transient final byte CHANGE_JOB = 6;
	/**
	 * 任务事件  杀死怪物
	 */
	...
	public static transient final short ACTION_TYPE_SPECIAL=5;//特殊事件,如主线强化任务，不需要统计，直接读取人物穿在身上的装备数据
...
//	protected HashMap<String,Integer> endNeedGoods=new HashMap<String, Integer>();
	...
	/***************需要存储的块END******************/
	public boolean action(short actionType,int model,int value,Object ... params){
		

		switch (actionType) {
		case ACTION_TYPE_GOODS:
			if (this instanceof RankTask) {
				if (!((RankTask) this).action(value <= 0 ? TaskEnum.USEITEM : TaskEnum.GETITEM, model, value)){
					return false;
				}
			}else if (this instanceof MainTask) {
				if (!((MainTask) this).action(value <= 0 ? TaskEnum.USEITEM : TaskEnum.GETITEM, model, value)){
					return false;
				}
			}else if(!endNeedGoods().keySet().contains(model)){
				return false;
			}
			break;
		case ACTION_TYPE_KILLMONSTER:	
			if(endNeedKillMonster().keySet().contains(model)){
				...
				MainTask task = (MainTask)this;
				if(task.endNeedKillMonster2().keySet().contains(model)){
					Integer need = task.endNeedKillMonster2().get(model);
					Integer integer = getKillmonsters().get(String.valueOf(model));
		...
		case ACTION_TYPE_RANK:
			if (this instanceof RankTask) {
				...
				if (!((MainTask) this).action(model, 0, value)){
					return false;
				}
			}else{
				return false;
		...
		case ACTION_TYPE_SPECIAL:
			if(model == TaskEnum.QIANGHUA ||model == TaskEnum.HORSE_STAGE_UP ||model == TaskEnum.WEAR_EQUIP||model == TaskEnum.UNWEAR_EQUIP) {
				if(this instanceof MainTask) {
					MainTask mainTask = (MainTask) this;
					if (!mainTask.action(model, 0, value,params)){
						return false;
					}
				}else {
					return false;
		...
		try{
			...
			if(endNeedKillMonster().size()>0){
				...
				for (Integer model : keySet) {
					...
					if(num==null || neednum==null ||num<neednum){
			...
			if(this instanceof MainTask) {
				MainTask task = (MainTask)this;
				if(task.endNeedKillMonster2().size()>0){
					Set<Integer> keySet = task.endNeedKillMonster2().keySet();
					boolean killOne = false;
					for (Integer model : keySet) {
						Integer neednum = task.endNeedKillMonster2().get(model);
						Integer num = getKillmonsters().get(String.valueOf(model));
						if(num != null && neednum != null && num.intValue() == neednum.intValue()) {
							killOne = true;
							break;
						}
					}
					if(!killOne){
						if(isPromp)MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("击杀")+ "怪物"+ResManager.getInstance().getString("数量不足"));
						return false;
					}
				}
			}
