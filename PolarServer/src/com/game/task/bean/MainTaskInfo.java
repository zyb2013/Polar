...
public class MainTaskInfo extends Bean {
	...
	//达成强化
	private List<TaskAttribute> permiseQiangHua = new ArrayList<TaskAttribute>();
	//其他类型任务条件
	private List<TaskAttribute> permiseOthers = new ArrayList<TaskAttribute>();
	//达成穿戴
	private List<TaskAttribute> permiseWear = new ArrayList<TaskAttribute>();
	...
	public boolean write(IoBuffer buf){
		...
		writeShort(buf, permiseQiangHua.size());
		for (int i = 0; i < permiseQiangHua.size(); i++) {
			writeBean(buf, permiseQiangHua.get(i));
		}
		// 其他任务条件
		writeShort(buf, permiseOthers.size());
		for (int i = 0; i < permiseOthers.size(); i++) {
			writeBean(buf, permiseOthers.get(i));
		}
		//达成穿戴
		writeShort(buf, permiseWear.size());
		for (int i = 0; i < permiseWear.size(); i++) {
			writeBean(buf, permiseWear.get(i));
		}
		return true;
	}
	...
	/**
	 * @return the permiseQiangHua
	 */
	public List<TaskAttribute> getPermiseQiangHua() {
		return permiseQiangHua;
	}

	/**
	 * @param permiseQiangHua the permiseQiangHua to set
	 */
	public void setPermiseQiangHua(List<TaskAttribute> permiseQiangHua) {
		this.permiseQiangHua = permiseQiangHua;
	}

	/**
	 * @return the permiseOthers
	 */
	public List<TaskAttribute> getPermiseOthers() {
		return permiseOthers;
	}

	/**
	 * @param permiseOthers the permiseOthers to set
	 */
	public void setPermiseOthers(List<TaskAttribute> permiseOthers) {
		this.permiseOthers = permiseOthers;
	}

	public List<TaskAttribute> getPermiseWear() {
		return permiseWear;
	}

	public void setPermiseWear(List<TaskAttribute> permiseWear) {
		this.permiseWear = permiseWear;
	}
