...
public class FighterInfo {
	...
	public void compareFighter(Fighter fighter){
		if(fighter instanceof Player)
			...
			compareSummonPet((SummonPet)fighter);
	}
...
//		}
	...
	private void compareSummonPet(SummonPet summonpet){
		//生命
		if(this.getHp() != summonpet.getHp()){
			ManagerPool.summonpetInfoManager.onHpChange(summonpet);
		}
		
		//魔法值
		if(this.getMp() != summonpet.getMp()){
			ManagerPool.summonpetInfoManager.onMpChange(summonpet);
		}
		
		//速度
		int speed = summonpet.getSpeed();
		if(this.getSpeed() != speed){
			ManagerPool.summonpetInfoManager.onSpeedChange(summonpet);
		}
	}
}
