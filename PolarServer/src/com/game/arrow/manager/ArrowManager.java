...
//	public static int FightType_TU = 7;				//7土
	...
	public void reqStarActivationToServer(Player player, ReqStarActivationMessage message) {
		...
		if (newBean.getQ_active_arrowid() != 0) {
			...
			if (player.getArrowData().getArrowlv() == 1) {
				...
				parseUtil.setValue(String.format("恭喜玩家【%s】成功激活第一把弓箭【%s】，获得超高属性加成及弓箭专属技能!{@}", player.getName(), q_arrowBean.getQ_arrow_name()), new ParseUtil.VipParm(VipManager
						.getInstance().getVIPLevel(player), GuideType.ARROW_A.getValue()));
				...
				/*xiaozhuoming: 暂时没有用到
				ParseUtil parseUtil = new ParseUtil();
				parseUtil.setValue(String.format("恭喜玩家【%s】成功将弓箭提升至顶级【%s】，真乃人中之龙必成就一代霸业!{@}", player.getName(), q_arrowBean.getQ_arrow_name()), new ParseUtil.VipParm(VipManager
						.getInstance().getVIPLevel(player), GuideType.ARROW_A.getValue()));
				MessageUtil.notify_All_player(Notifys.CHAT_SYSTEM, parseUtil.toString(),new ArrayList<GoodsInfoRes>(),GuideType.ARROW_A.getValue());
				*/
			} else {
				...
				if (oldarrowBean != null) {
					parseUtil.setValue(String.format("恭喜玩家【%s】成功将弓箭【%s】提升为【%s】，获得超高属性加成!{@}", player.getName(), oldarrowBean.getQ_arrow_name(), q_arrowBean.getQ_arrow_name()),
							new ParseUtil.VipParm(VipManager.getInstance().getVIPLevel(player), GuideType.ARROW_A.getValue()));
					...
					parseUtil.setValue(String.format("恭喜玩家【%s】成功将弓箭提升至顶级【%s】，获得超高属性加成！{@}", player.getName(), q_arrowBean.getQ_arrow_name()), new ParseUtil.VipParm(VipManager
							.getInstance().getVIPLevel(player), GuideType.ARROW_A.getValue()));
	...
	public void reqFightSpiritGetToServer(Player player, ReqFightSpiritGetMessage message) {
		...
		if (message.getGettype() == 0) {
			...
			//检测2级密码
			if (needyuanbao > 0 && ManagerPool.protectManager.checkProtectStatus(player) ) {
				return;
			}
			
			if (!BackpackManager.getInstance().checkGold(player, needyuanbao)) {
				/* xuliang 屏蔽
				MessageUtil.notify_player(player, Notifys.ERROR, String.format(ResManager.getInstance().getString("您的钻石不足%s，不能搜索七曜战魂【%s】。"), String.valueOf(needyuanbao), q_clone_activityBean.getQ_duplicate_name()));
				*/
				return;
			}
			if (count == 0) {
			...
			if (!BackpackManager.getInstance().checkGold(player, needyuanbao)) {
				/* xuliang 屏蔽提示
				MessageUtil.notify_player(player, Notifys.ERROR, String.format(ResManager.getInstance().getString("您的钻石不足%s，不能连续搜索七曜战魂【%s】。"), String.valueOf(needyuanbao), q_clone_activityBean.getQ_duplicate_name()));
				*/
				return;
			}
			if (count == 0) {
