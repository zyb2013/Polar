...
public class GemManager {
	...
	public int getHighGenPlayerlv(){
		return 0/*xuliang ManagerPool.dataManager.q_globalContainer.getMap().get(85).getQ_int_value()*/;
	}
	...
	public int getHighGenlv(){
		return 0/*xuliangManagerPool.dataManager.q_globalContainer.getMap().get(86).getQ_int_value()*/;
	}
	...
	public int getOpenGenlv(){
		return 0/*ManagerPool.dataManager.q_globalContainer.getMap().get(90).getQ_int_value()*/;
	}
	...
	public Gem[] getPosGems(Player player,int pos){
		...
		if(gems[pos] !=null){
			//panic god 暂时修改
			if(gems[pos][0] != null && gems[pos][1]!= null &&  gems[pos][2] != null){
					...
					if (result == 1) {
						...
						*/
						...
						if(gem.getGrid() >= 3){
							...
							parseUtil.setValue(String.format(ResManager.getInstance().getString("恭喜玩家【%s】激活【%s】战斗力提升%d!{@}"), player.getName(), gemname, fightpower),
									new ParseUtil.VipParm(VipManager.getInstance().getVIPLevel(player), GuideType.GEM_B.getValue()));
			...
			if (gem.getExp() >= gemupdata.getQ_need_up_exp()) {
				...
				if (gemactdata !=null) {
					if (gem.getLevel() >= 3) {
						...
						ParseUtil parseUtil = new ParseUtil();
						parseUtil.setValue(
								String.format(ResManager.getInstance().getString("恭喜玩家【%s】将%s上的【%s】成功升至【%d】品!{@}"), player.getName(), posname, dbname.getQ_gem_name(),
										gem.getLevel()), new ParseUtil.VipParm(VipManager.getInstance().getVIPLevel(player), GuideType.GEM_A.getValue()));
...
//					itemid = integers[0];
	...
	public void addGemBuff(Player player){
		...
		ManagerPool.buffManager.removeByBuffId( player, gembuffa,gembuffb,gembuffc);
