...
public class YbcardManager {
	...
	public void stReqYBCardToGameMessage(Player player, byte type, int num) {
		...
		wmsg.setType(type);
		wmsg.setNum(num);
		if (type== 1) {	//使用钻石卡type = 1;		//1封测期间（抽奖），2公测期间(领取钻石)
			...
			if (YB_MAX >= 999999999) {
				MessageUtil.notify_player(player,Notifys.CUTOUT,ResManager.getInstance().getString("今日钻石卡发放钻石已经达到上限，明天再用吧"));
				return;
			}
			if (ManagerPool.backpackManager.removeItem(player, 8060, num, Reasons.ACTIVITY_YBCARD_USE, Config.getId()) == false) {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，您包裹里没有公测钻石卡。"));
				return;
			}
		}
	...
	public void stResYBCardReceiveToGameMessage(ResYBCardReceiveToGameMessage msg) {
		...
		if (player != null) {
			...
			MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM, ResManager.getInstance().getString("恭喜您领取到了{1}绑钻"), msg.getYuanbaonum()+"");
		}
	}
	...
	public void stResYBCardAddYBPlayerToGameMessage(ResYBCardAddYBPlayerToGameMessage msg) {
		...
		if (player != null) {
			...
			//MessageUtil.notify_player(player, Notifys.SUCCESS, "恭喜您得到了{1}绑定钻石", msg.getNum()+"");
		}
