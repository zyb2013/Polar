...
public class LoginManager {
	...
	public void createCharacter(ReqJustCreateCharacterToGameMessage msg) {
		Player player = createCharacter(msg.getSession(), msg.getCreateServer(), msg.getName(), msg.getSex(),msg.getJob(), msg.getUserId()
	...
	private Player createCharacter(IoSession session, int createServer, String name, byte sex, byte job, String userId
		...
		if(name==null || "".equals(name)){
			...
			guestName = true;
		}else if(name==null || name.length() < 2 || name.length() > 7){
			//重名提示
		...
		//创建玩家
		Player player = ManagerPool.playerManager.createPlayer(userName, userId, name, icon, sex, job);
		//账号id
		...
		try{
			...
			//加载2级密码
			ManagerPool.protectManager.reloadProtect(player);
		}catch (Exception e) {
			log.error(e, e);
			//告诉网关退出,数据库错误
			ResCreateCharacterFailedMessage failedMessage = new ResCreateCharacterFailedMessage();
