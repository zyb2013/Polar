	...
	public void stReqLaunchProposeStartToGameMessage(Player player,ReqLaunchProposeStartToGameMessage msg) {
		...
		if (suitplayer != null) {
			...
			if (player.getMoney() < ringprice[1]) {
				/*xiaozhuoming: 暂时没有有到
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("购买婚戒所需金币不足，需要{1}金币"),ringprice[1]+"");
				*/
				return;
			}
			...
			if(ringprice[2] > 0 && !ManagerPool.backpackManager.checkGold(player, ringprice[2])){
				/*xiaozhuoming: 暂时没有有到
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("购买婚戒所需钻石不足，需要{1}钻石"),ringprice[2]+"");
				*/
				return;
			}
...
//		suitplayer.setMarriageid(0);
		if(!WServer.getInstance().getServerWeb().equals("hgpupugame")) {//韩国版屏蔽开区天数检测
				...
				if (is) {
					...
					if (ringprice[1] > 0) {
						if (!ManagerPool.backpackManager.changeMoney(player, -ringprice[1], Reasons.MARRIED_RING_MONEY, aid)) {
							/*xiaozhuoming: 暂时没有有到
							MessageUtil.notify_player(suitplayer, Notifys.ERROR, ResManager.getInstance().getString("对方购买婚戒所需金币不足"));
							MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("购买婚戒所需金币不足，需要{1}金币"),ringprice[1]+"");
							*/
							return;
						}
					...
					if (ringprice[2] > 0) {
						if (!ManagerPool.backpackManager.changeGold(player,  -ringprice[2],Reasons.MARRIED_RING_GOLD,aid)) {
							/*xiaozhuoming: 暂时没有有到
							MessageUtil.notify_player(suitplayer, Notifys.ERROR, ResManager.getInstance().getString("对方购买婚戒所需钻石不足"));
							MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("购买婚戒所需钻石不足，需要{1}钻石"),ringprice[2]+"");
							*/
							return;
						}
	...
	public void stReqDivorceToGameMessage(Player player,ReqDivorceToGameMessage msg) {
		...
		if (msg.getType() == 1) {	//1协议离婚，2强行离婚
			...
			if (ManagerPool.backpackManager.changeMoney(player, -2000000, Reasons.DIVORCE_GOLD, Config.getId())) {
				...
				/*xiaozhuoming: 暂时没有有到
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，您的金币不足200万。"));
				*/
			}
		}
	...
	public void divorceClearPlayerData(Player player  ){
		try {
			...
			ManagerPool.buffManager.removeByBuffId(player, 9166,9167,9168,9172);
...
//					List<Wedding> list = selectWedding(true);
	...
	public boolean ckWeddingmoney(Player player, int id){
		...
		if (tab != null && tab.length == 2) {
			...
			if(player.getMoney() < tab[0]){//检测金币
				/*xiaozhuoming: 暂时没有用到
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，您的金币不足{1}，无法举办该级别婚宴"),tab[0]+"");
				*/
				return false;
			}

			if (!ManagerPool.backpackManager.checkGold(player,tab[1])) {//检测钻石
				/*xiaozhuoming: 暂时没有用到
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，您的钻石不足{1}，无法举办该级别婚宴"),tab[1]+"");
				*/
				return false;
			}
			
			//检测2级密码
			if (tab[1] > 0 && ManagerPool.protectManager.checkProtectStatus(player) ) {
				return false;
			}
	...
	public synchronized void stReqReceiveredenvelopeToGameMessage(Player player) {
		...
		if (wedding != null) {
			...
			if (money + player.getMoney() > Global.BAG_MAX_COPPER || money + player.getMoney() < 0) {
				/*xiaozhuoming: 暂时没有用到
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("您要领取的金币和身上的金币合计数量超过了上限，暂时不能领取"));
				*/
				return;
			}
			...
			if(!wedding.getRedreceives().contains(player.getId())){
				...
				/*xiaozhuoming: 暂时没有用到
				MessageUtil.notify_player(player, Notifys.CHAT_ROLE, ResManager.getInstance().getString("恭喜您领取了婚宴红包{1}金币"),money+"");
				*/
...
//			String name = getWeddingname(wedding.getType());
			if (rednum > msg.getNum()) {
				/*xiaozhuoming: 暂时没有用到
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，参加{1}婚宴的红包底限需要{2}金币"),name,rednum+"");
				*/
				return;
			}
			...
			if(ManagerPool.backpackManager.changeMoney(player, -msg.getNum(), Reasons.WEDDING_RED_NUM, Config.getId())){
				...
				*/
				...
				/*xiaozhuoming: 暂时没有用到
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉，您没有{1}金币，无法参加婚宴"),msg.getNum()+"");
				*/
			}
		}
	}
	...
	public synchronized void stReqWeddingEatingToGameMessage(Player player,ReqWeddingEatingToGameMessage msg) {
		...
		if (npc != null) {
			...
			if (exp >= expmax && zhenqi >= zhenqimax ) {
				/*xiaozhuoming: 暂时没有用到
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("您已吃的过饱，今日婚宴所获经验和真气已达上限"));
				*/
				return;
			}
			num = num -1;
	...
	public long getSpouseid(Player player) {
		...
		if (marriage == null) {
			...
			Spouse spouse = marriage.getSpouse(player);
			if (spouse != null) {
				return spouse.getPlayerid();
			}else {
				return 0;
			}
		}
				...
				for (Integer[] integers : ringPricelist) {
					if((int)integers[0] == msg.getRingmodelid()){
						...
						if (player.getMoney() < integers[1]) {
							/*xiaozhuoming: 暂时没有用到
							MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("购买婚戒所需金币不足，需要{1}金币"),integers[1]+"");
							*/
							is = false;
						}
						...
						if(integers[2] > 0 && !ManagerPool.backpackManager.checkGold(player, integers[2])){
							/*xiaozhuoming: 暂时没有用到
							MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("购买婚戒所需钻石不足，需要{1}钻石"),integers[2]+"");
							*/
							is = false;
						}
						
						//检测2级密码
						if (integers[2] > 0 && ManagerPool.protectManager.checkProtectStatus(player) ) {
							return ;
						}
