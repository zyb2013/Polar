					...
					if (item != null) {
						if (item.getItemModelId() == -1) {//Money
							if (!BackpackManager.getInstance().changeMoney(player, item.getNum(), Reasons.CARD_GIFT_MONEY, Config.getId())) {
								MailServerManager.getInstance().sendSystemMail(player.getId(), player.getName(), ResManager.getInstance().getString("礼包系统邮件"), ResManager.getInstance().getString("由于未知原因，您的礼包中金币未领取成功，请点击附件领取。"), (byte) 1, item.getNum(), null);
								MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("由于未知原因，您的礼包中金币未领取成功，请点击邮件领取。"));
							}
							...
							if (!BackpackManager.getInstance().changeBindGold(player, item.getNum(), Reasons.CARD_GIFT_GOLD, Config.getId())) {
								...
								MailServerManager.getInstance().sendSystemMail(player.getId(), player.getName(), ResManager.getInstance().getString("礼包系统邮件"), ResManager.getInstance().getString("由于未知原因，您的礼包中绑钻未领取成功，请点击附件领取。"), (byte) 1, 0, items);
								MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("由于未知原因，您的礼包中绑钻未领取成功，请点击邮件领取。"));
							}
						} else {//普通物品
							List<Item> items = new ArrayList<Item>();
