...
public class ActivitiesManager {
	...
	// public static HashMap<String, Integer> activities = new HashMap<String,
	// Integer>();
	...
	{
		//开服活动
//		addActivity(1, 1, ScriptEnum.ACTIVITY_GRADE_GIFT);
		
		/*暂时 hongxiao.z 2014.2.10*/
		// 新手激活码
...
//		addActivity(3, 13, 2040); // 10+
	 ...
	 *            脚本编号
	 */
	void addActivity(int type, int id, int script) {
	...
	public void sendActivitiesInfo(Player player, boolean force) {
		IBaseActivityScript script = (IBaseActivityScript) ManagerPool.scriptManager
				.getScript(ScriptEnum.BASEACTIVITIES);
		if (script != null) {
	 ...
	 * 领取活动奖励
	 * @param player
	 * @param mainType	活动主分类标识
	 * @param type		在该活动类别中的标识
	 */
	public void getReward(Player player, int activityId, int type, int selected) 
	{
		IBaseActivityScript script = (IBaseActivityScript) ManagerPool.scriptManager
				.getScript(ScriptEnum.BASEACTIVITIES);
		if (script != null) {
			script.getReward(player, activityId, type, selected);
		}
	}
}
