...
public class BlackListDao {
	...
	public List<BlackListBean> select() {
		...
		try {
			...
			List<BlackListBean> list = (List<BlackListBean>) session
					.selectList("card_data.select");
	...
	public List<BlackListBean> selectbyTypeState(int type, int state) {
		...
		try {
			...
			List<BlackListBean> list = (List<BlackListBean>) session
					.selectList("blacklist.selectbytypestate", paramap);
			LoggerProvider.getDbconsuminglog().info(
					"blacklist.selectbytypestate "
							+ TimeUtil.getDurationToNow(currentTimeMillis)
							+ "ms");
			return list;
		} finally {
	...
	public Map<String, BlackListBean> selectBlackMapByTypeState(int type,
			int state) {
	...
	public int updateByUsernameType(BlackListBean blacklistbean) {
		...
		try {
			long currentTimeMillis = System.currentTimeMillis();
			int rows = session.insert("blacklist.updatebyusernametype",
					blacklistbean);
			LoggerProvider.getDbconsuminglog().info(
					"blacklist.updatebyusernametype "
							+ TimeUtil.getDurationToNow(currentTimeMillis)
							+ "ms");
			session.commit();
			return rows;
	...
	public static void main(String[] args) {
		...
		// bean.setUsername("tangc36");
		// bean.setEndtime(System.currentTimeMillis()+3600000);
		// bean.setEndcount(0); bean.setNowcount(0); bean.setType(1);
		// bean.setState(0);
