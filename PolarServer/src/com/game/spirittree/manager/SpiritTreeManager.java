	...
	/* xuliang
		...
		try {
			...
			if (msg.getPlayerid() != msg.getHostid()) {
				//军衔任务加次数
				ManagerPool.taskManager.action(player, Task.ACTION_TYPE_RANK, TaskEnum.HARVESTINGSPIRITTREEFRUIT, 1);
			}
...
//				}
			...
			if (player != null) {
				...
				if (pack_conBean.getQ_notice_type() >= 3) {
					...
					if (tiao == 6 && fruitReward.getStrenglevel() == 10) {
						...
						parseUtil.setValue(String.format(ResManager.getInstance().getString("玩家【%s】摘取【%s】获得顶级装备【%s】，战力瞬间爆棚！{@}"), player.getName(), fruitname, itemname),
								new ParseUtil.VipParm(VipManager.getInstance().getVIPLevel(player), GuideType.SPIRITTREE.getValue()));
	...
	public void stResFruitOperatingToGameMessage(ResFruitOperatingToGameMessage msg) {
		...
		if (player != null) {
			...
			if (msg.getType() == 1) {
				ManagerPool.taskManager.action(player, Task.ACTION_TYPE_RANK, TaskEnum.SPIRITTREEWATERING, 1);//军衔任务加次数
			}
	...
	public void stReqRipeningDecYBToGameMessage(ReqRipeningDecYBToGameMessage msg) {
		...
		if (player != null) {
			...
			if(msg.getYuanbao()>0 && ManagerPool.backpackManager.checkGold(player, msg.getYuanbao())) {
				...
				wmsg.setType((byte) 0);
				/* xuliang
				MessageUtil.notify_player(player,Notifys.ERROR,ResManager.getInstance().getString("催熟果实所需钻石不足，是否前往充值"));
				*/
			}
			wmsg.setFruitid(msg.getFruitid());
