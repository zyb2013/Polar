...
public class HiddenWeaponManager {
	...
	protected static Logger logger = Logger
			.getLogger(HiddenWeaponManager.class);
	...
	public boolean activateHiddenWeapon(Player player) {
		if (player.getLevel() < 70) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("您未达到70级，无法激活暗器系统"));
			return false;
		}
		...
		if (!BackpackManager.getInstance().checkGold(player, 300)) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("激活暗器失败,钻石不足"));
			return false;
		}
		...
		if (!BackpackManager.getInstance().changeGold(player, -300,
				Reasons.HIDDENWEAPON_GOLD, action)) {
			return false;
		}
		...
		weapon.setOvertime(System.currentTimeMillis() + (long) 7 * 24 * 60 * 60
				* 1000);
		...
		try {
			int afterFightPower = getHiddenWeaponAttrFightPower(player, weapon); // 激活后战斗力-
																					// 用于引导公告

			// 取得一阶暗器名称
			String weaponname = "";
			Q_hiddenweaponBean bean = DataManager.getInstance().q_hiddenweaponContainer
					.getMap().get(1);
			if (bean != null) {
			...
			parseUtil.setValue(
					String.format(
							ResManager.getInstance().getString(
									"恭喜【%s】成功激活暗器【%s】战斗力提升%d！{@}"),
							player.getName(), weaponname, afterFightPower),
					new ParseUtil.VipParm(VipManager.getInstance().getVIPLevel(player), GuideType.HIDDENWEAPON
							.getValue()));
			MessageUtil.notify_All_player(Notifys.CHAT_BULL,
					parseUtil.toString(), new ArrayList<GoodsInfoRes>(),
					GuideType.HIDDENWEAPON.getValue());
		} catch (Exception e) {
			logger.error(e, e);
		}
		return true;
	...
	public void levelupHiddenWeapon(Player player, byte auto) {
		...
		if (weapon.getLayer() >= LAYER_MAX) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("您的暗器已经达到最高阶。"));
			sendFailedError(player);
			return;
		}

		Q_hiddenweaponBean weaponBean = ManagerPool.dataManager.q_hiddenweaponContainer
				.getMap().get(weapon.getLayer());
		if (weaponBean == null) {
		...
		if (weapon.getOvertime() != -1
				&& System.currentTimeMillis() > weapon.getOvertime()) {
			overtime = true;
		}
		...
		if (costItemModelId != 0 && costNum != 0) {
			Q_itemBean itemBean = ManagerPool.dataManager.q_itemContainer
					.getMap().get(costItemModelId);
			if (itemBean == null) {
				sendFailedError(player);
				return;
			}
			// 判断材料是否足够 查看背包内容
			int num = BackpackManager.getInstance().getItemNum(player,
					costItemModelId);

			if (num < costNum) {
				if (auto == 0) {
					MessageUtil.notify_player(player, Notifys.ERROR, ResManager
							.getInstance().getString("需要{1}[{2}颗]，才可继续进阶."),
							itemBean.getQ_name(), String.valueOf(costNum));
					sendFailedError(player);
					return;
				} else {
					costGold = true;
			...
			// 扣除材料
			if (costGold) {
				if (!ManagerPool.shopManager.autoRemove(player,
						costItemModelId, costNum, Reasons.HIDDENWEAPON_GOLD,
						actionId)) {
					MessageUtil.notify_player(player, Notifys.ERROR, ResManager
							.getInstance().getString("需要{1}[{2}颗]，才可继续进阶."),
							itemBean.getQ_name(), String.valueOf(costNum));
				...
				if (!ManagerPool.backpackManager.removeItem(player,
						costItemModelId, costNum, Reasons.HIDDENWEAPON_ITEM,
						actionId)) {
					MessageUtil.notify_player(player, Notifys.ERROR, ResManager
							.getInstance().getString("需要{1}[{2}颗]，才可继续进阶."),
							itemBean.getQ_name(), String.valueOf(costNum));
		...
		ManagerPool.backpackManager.changeMoney(player,
				-weaponBean.getQ_need_money(), Reasons.HIDDENWEAPON_MONEY,
				actionId);
		...
		if (weapon.getDayblessvalue() >= weaponBean.getQ_blessnum_limit()) {
			...
			if (upnum <= weaponBean.getQ_up_num_min()) {
				...
				if (RandomUtils.isGenerate2(10000,
						weaponBean.getQ_up_probability())) { // 进入随机
					success = 1; // 成功
				} else {
					success = 0;
				}
		...
		if (success == 1) { // 成功处理
			...
			weaponBean = ManagerPool.dataManager.q_hiddenweaponContainer
					.getMap().get(weapon.getLayer());
			levelUp = true;
		} else { // 失败处理
			weapon.setDayupnum(upnum);
			blessnum = RandomUtils.randomValue(weaponBean.getQ_blessnum_max(),
					weaponBean.getQ_blessnum_min());
			String[] a = weaponBean.getQ_normal_rnd().split(Symbol.SHUXIAN_REG);
			String[] b = weaponBean.getQ_small_crit_rnd().split(
					Symbol.SHUXIAN_REG);
			String[] c = weaponBean.getQ_large_crit_rnd().split(
					Symbol.SHUXIAN_REG);
			...
			if (weapon.getDayexp() < weaponBean.getQ_up_fail_addexp()) {
				...
				ManagerPool.playerManager.addExp(player, exp,
						AttributeChangeReason.HIDDEN_WEAPON);
			} else {
				exp = 0;
			}
		}
		...
		if (levelUp) {
			...
			try {
				...
				Q_hiddenweaponBean tobean = DataManager.getInstance().q_hiddenweaponContainer
						.getMap().get(afterrank);
				if (tobean != null) {
					if (tobean.getQ_is_announce() == 1) { // 是否需要发公告
						...
						// 发送升级公告
						ParseUtil parseUtil = new ParseUtil();
						parseUtil.setValue(String
								.format(ResManager.getInstance().getString(
										"恭喜【%s】将【%s】升阶至【%s】战斗力提升%d！{@}"),
										player.getName(), fromweapon, toweapon,
										upvalue),
								new ParseUtil.VipParm(VipManager.getInstance().getVIPLevel(player),
										GuideType.HIDDENWEAPON_UP.getValue()));
						MessageUtil.notify_All_player(Notifys.CHAT_BULL,
								parseUtil.toString(),
	...
	public void wearHiddenWeapon(Player player, int layer) {
		...
		if (weapon.getOvertime() != -1
				&& System.currentTimeMillis() > weapon.getOvertime()) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("您的暗器已经过期。"));
			return;
		}

		if (weapon.getLayer() == layer && weapon.getState() == WEAR) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("您已经装备此暗器了。"));
			return;
		}
	...
	public Skill getHiddenWeaponSkillTriggerByAttack(Player player) {
		...
		HiddenWeapon hideWeapon = ManagerPool.hiddenWeaponManager
		...
		Q_hiddenweaponBean hiddeanwaponBean = ManagerPool.dataManager.q_hiddenweaponContainer
				.getMap().get(hideWeapon.getLayer());
		...
		while (it.hasNext()) {
			...
			Q_hiddenweapon_skillBean hiddenweaponSkillBean = ManagerPool.dataManager.q_hiddenweapon_skillContainer
					.getMap().get(
							tmpSkill.getSkillModelId() + "_"
									+ tmpSkill.getSkillLevel());
	...
	private Skill getIcoSkill(HiddenWeapon hideWeapon) {
		...
		if (skill == null) {
			...
			Q_hiddenweapon_skillBean tmpBean = ManagerPool.dataManager.q_hiddenweapon_skillContainer
					.getMap().get(
	...
	public void countHiddenWeaponAttribute(Player player) {
		ManagerPool.playerAttributeManager.countPlayerAttribute(player,
				PlayerAttributeType.HIDDEN_WEAPON);
	}
	...
	public List<HiddenWeaponSkillInfo> getSkillInfo(HiddenWeapon weapon,
			Player player) {
	 ...
	 * 计算暗器对角色加成的属性战斗力
	 */
	public int getHiddenWeaponAttrFightPower(Player player,
			HiddenWeapon hiddenweapon) {
		int fightpower = 0;
		Q_hiddenweapon_attrBean bean = DataManager.getInstance().q_hiddenweapon_attrContainer
				.getMap().get(hiddenweapon.getLayer());
	...
	public void addDaybless(Player player, int num) {
		...
		MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM, ResManager
				.getInstance().getString("祝福值增加了{1}"), num + "");
		*/
		sendHiddenWeaponInfo(player);
	}
	...
	public boolean studySkill(Player player, int skillId, int skillLevel) {
		Q_hiddenweapon_skillBean skillBean = ManagerPool.dataManager.q_hiddenweapon_skillContainer
				.getMap().get(skillId + "_" + skillLevel);
		if (skillBean == null) {
			return false;
		}
		...
		if (weapon == null) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("需要激活暗器才能使用"));
			return false;
		}

		if (skillBean.getQ_need_level() > weapon.getLayer()) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("很抱歉,您需要先将暗器进阶到{1}阶"),
					skillBean.getQ_need_level() + "");
			return false;
		}

		byte index = -1;
		IHiddenWeaponSkillScript script = (IHiddenWeaponSkillScript) ManagerPool.scriptManager
				.getScript(ScriptEnum.HIDDENWEAPON_SKILL);
		if (script != null) {
			try {
				index = (byte) script.onHiddenWeaponSkillStudy(player, skillId,
						skillLevel);
			} catch (Exception e) {
				logger.error(e, e);
				return false;
		...
		Q_skill_modelBean skillModel = ManagerPool.dataManager.q_skill_modelContainer
				.getMap().get(skillBean.getQ_skill());
		MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM, ResManager
				.getInstance().getString("恭喜您成功习得暗器技能{1}"), skillModel
				.getQ_skillName());
	...
	public void countNextIco(Player player) {
		...
		Q_hiddenweaponBean hiddenweaponBean = ManagerPool.dataManager.q_hiddenweaponContainer
				.getMap().get(hideWeapon.getLayer());
		...
		if (hiddenweaponBean.getQ_open_grid() != 0) {
			hideWeapon
					.setIcoIndex((byte) ((hideWeapon.getIcoIndex() + 1) % hiddenweaponBean
							.getQ_open_grid()));
		}
