...
public class AttributeBuff extends Buff implements IBuff {
	...
	//总命中加成
	protected transient int hitDodgePercent;
	//总闪避加成
	...
	//是否计入战斗力
	protected transient boolean fightFlag = false;
	//技能加成
	...
	//可以吸收有限伤害的
	protected transient int reduce_injure;
	//增加伤害的
	protected transient int add_injure;
...
//		TODO if (buffModel.getQ_fight_power() == 1) {
//			fightFlag = true;
//		} else {
//			fightFlag = false;
//		}
	...
	/**
	 * 获取对目标添加一个可以吸收有限伤害的buff
	 * @return
	 */
	public int getreduce_injure() {
	...
	public int getHit() {
		switch(this.getActionType()){
			case BuffType.HIT:
			case BuffType.HITPERCENT:
				Q_buffBean buff = ManagerPool.dataManager.q_buffContainer.getMap().get(this.getModelId());
				if(buff==null) return 0;
				return buff.getQ_effect_value() + this.getValue();
			case BuffType.POWERUP:
				if(powers!=null){
					if(powers.containsKey(CommonString.HIT)){
						return (Integer)powers.get(CommonString.HIT);
					}
				}
				return 0;
		}
	...
	public int getHitPercent() {
		switch(this.getActionType()){
			...
			case BuffType.POWERUP:
				if(powers!=null){
					if(powers.containsKey(CommonString.HITPERCENT)){
						return (Integer)powers.get(CommonString.HITPERCENT);
					}
				}
				return 0;
		}
	...
	public void setFightFlag(boolean fightFlag) {
	 ...
	 * @return
	 */
	public int getPerfectAttackAddition() {
		switch (this.getActionType()) {
	 ...
	 * @return
	 */
	public int getKnowingAttackAddition() {
		switch (this.getActionType()) {
