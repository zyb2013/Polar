...
//						((Player) target).setState(PlayerState.STAND);
			...
			case BuffType.LINGLONGBUFF:
				...
				while (iter.hasNext()) {
					...
					if(ManagerPool.buffManager.check(model, BuffConst.LINGLONG_BUFF)){
						if(model.getQ_effect_type() == 2){
							buff.setTotalRemainTime(0);
							buff.setTotalTime(0);
						}else if(model.getQ_effect_type() == 1 && !buff.getBackups().containsKey(String.valueOf(BuffConst.LINGLONG_BUFF))){
			...
			case BuffType.UNPOISON:
				//! 解毒Buff，判断玩家身上是否有毒Buff，若有，则解除，并且一定时间内不会中毒
				List<Buff> buffs = ManagerPool.buffManager.getBuffByType(source, BuffType.POISON);
				if (buffs != null && buffs.size() > 0){
					Iterator<Buff> iterBuff = buffs.iterator();
					while (iterBuff.hasNext()) {
						Buff poisonBuff = (Buff) iterBuff.next();
						ManagerPool.buffManager.removeBuff(source, poisonBuff);
					}
				}
				source.addFightState(FighterState.UNPOSION);
				return 1;	
		}
		return 0;
	}
	...
	public int remove(Fighter source) {
		switch (this.getActionType()) {
			...
			case BuffType.HORSEWEAPONPROTECT:
				...
				return 1;
			case BuffType.UNPOISON:
				source.removeFightState(FighterState.UNPOSION);
				return 1;
		}
		return 0;
	}
