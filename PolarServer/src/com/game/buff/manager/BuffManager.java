 ...
 * longyuanBuffs.add(10010);
	...
	public int addBuff(Fighter source, Fighter target, int buffId, int num, long total, int addtimer, int addtion, int addtionPercent) {
		...
		if (buffModel == null) {
			log.error("没有这个buff："+buffId);
			return 0;
		}
		//! 中毒Buff特殊处理
		if (buffModel.getQ_action_type() == BuffType.POISON && 
				FighterState.UNPOSION.compare(target.getFightState())){
			return 0;
		}
		...
		if(adder==null){
			log.error("Buff(" + buffId + ","+buffModel.getQ_target()+")->Source(" + source.getClass().getSimpleName() + ")->Target(" + target.getClass().getSimpleName() + "):not have adder");
			return 0;
		}
		//只对玩家有效
		...
		if (buffModel.getQ_effect_time() != -1) {
			 ...
			 * if(FighterState.LINGLONG.compare(adder.getFightState())){
				if(buffModel.getQ_effect_type()==1 && check(buffModel, BuffConst.LINGLONG_BUFF)){
			...
			if(FighterState.ZHEYING.compare(adder.getFightState())){
				if(buffModel.getQ_effect_type()==2 && check(buffModel, BuffConst.ZHEYING_BUFF)){
		...
		// parameter作为计数器
		if (buffModel.getQ_action_type() == BuffType.SECTION_ADD_HP || buffModel.getQ_action_type() == BuffType.SECTION_ADD_MP) {
			buff.setParameter(0);
		}
		//叠加层数
		buff.setOverlay(num);
		
		specialHandling(source, target, buff, buffModel);
		//参数数组
//		buff.getParameters().add(buffModel.getQ_Bonus_skill());
		...
		if (RandomUtils.random(Global.MAX_PROBABILITY) >= percent) {
			//添加失败
			log.info("加buff失败:未命中成功几率(玩家:" + target.getId() + ";模型:" + buffId + ")");
			/*
			 * 	luminghua hide
			 * if (buffId == BuffConst.KINGCITY_KING) {
...
//			List<Buff> buffs = getBuffByType(adder, buff.getActionType());
			...
			if (buffs.size() > 0) {
				...
				if (buffModel.getQ_overlay() == 1) {
					...
					if (buffs.size() < buffModel.getQ_overlay_maxcount() || buffModel.getQ_overlay_maxcount() == -1) {
						...
						sendAddBuffMessage(source, adder, buff);
						//DEBUFF自动移除判断
						if (buffModel.getQ_effect_type() == 1) {
					...
					return 1;
				} //! 叠加容量，并且将效果值替换为最高的buff的效果值 
				else if(buffModel.getQ_overlay() == 6){
					//原有buff加上属性值
					Buff old = null;
					for (int i = 0; i < buffs.size(); i++) {
						Buff tb = buffs.get(i);
					...
					if (old != null) {
						//获取BUFF模型
						Q_buffBean oldModel = ManagerPool.dataManager.q_buffContainer.getMap().get(old.getModelId());
						
						if (oldModel.getQ_effect_value() >= buffModel.getQ_effect_value()){
							old.setParameter(old.getParameter() + buff.getParameter());
							//添加buff消息
							sendChangeBuffMessage(source, adder, old);							
						}else{
							buff.setParameter(old.getParameter() + buff.getParameter());
							//! 移除效果较小的Buff
							remove(adder, old);
							//! 新增效果较大的Buff
							buff.add(source, adder);
				...
				else if (buffModel.getQ_overlay() == 7) {
					//原有相同ID的buff，则时间叠加，原有相同层级的buff则替换
					Buff old = null;
					for (int i = 0; i < buffs.size(); i++) {
						Buff tb = buffs.get(i);
						if (tb.getModelId() == buff.getModelId()) {
							old = tb;
							break;
						}else if (tb.getReplaceLevel() == buff.getReplaceLevel()) {
							remove(adder, tb);
							break;
						}
					}
				...
				if (buff.getTotalTime() != -1) {
					if (buff.getCount() == 1) {
						...
						if (buff.getStart() + buff.getTotalTime() < System.currentTimeMillis()) {
							BuffManager.getInstance().remove(fighter, buff);
							/* xuliang 移除buff调用 buffManager 里面的接口 
							buff.remove(fighter);
							fighter.getBuffs().remove(buff);
							*/
							//pk保护buff移除提示
							...
							/* xuliang 移除buff调用BuffManager里面的接口
							//通知移除buff
							sendRemoveBuffMessage(fighter, buff);
							*/
							recount = true;
	...
	private void remove(Fighter fighter, Buff buff) {
		if(fighter instanceof Player || !(buff instanceof AttributeBuff)){
			buff.remove(fighter);
			fighter.getBuffs().remove(buff);
		}else{
			//! xuliang 对于 怪物、宠物、召唤兽移除 属性类buff特殊处理，重新计算属性
			FighterInfo info = new FighterInfo();
				
			info.copyFighter(fighter);
			buff.remove(fighter);
			fighter.getBuffs().remove(buff);
			
			info.compareFighter(fighter);
		}
		//通知移除buff
		sendRemoveBuffMessage(fighter, buff);
	}
	 ...
	 * @param id
	 */
	public void removeByBuffId(Player player, int ... ids) {
	...
	public void removeOnDieOrQuit(Fighter fighter, boolean die) {
		...
		while (iter.hasNext()) {
			...
			int die_or_quit = buffModel.getQ_effect_dieordown_clear();
			//! 右数第一位为 死亡是否移除Buff，第二位为 退出是否移除Buff
			if (((die_or_quit & 1 << 0) != 0 && die) || ((die_or_quit & 1 << 1) != 0 && !die)){
				buff.remove(fighter);
				iter.remove();
				//通知移除buff
				sendRemoveBuffMessage(fighter, buff);
	 ...
	 * @return
	 */
	public List<Buff> getBuffShapeChange(Fighter fighter) {
		List<Buff> buffs = new ArrayList<Buff>();
		Iterator<Buff> iter = fighter.getBuffs().iterator();
		while (iter.hasNext()) {
			Buff buff = (Buff) iter.next();
			if (buff instanceof StateBuff && ((StateBuff) buff).getActionType() == BuffType.SHAPE_CHANGE) {
				buffs.add(buff);
			}
		}
		return buffs;
	...
	public BuffInfo getBuffInfo(Fighter adder, Buff buff) {
		...
		if (buff.getTotalTime() == -1) {//永久BUFF
			remain = -1;
		} else if (buff.getCount() == 1) {////下线是否计时 0-不计时 1-计时
			remain = (int) ((buff.getStart() + buff.getTotalTime() - System.currentTimeMillis()) / 1000); //非永久buff 计算剩余时间
			if (remain < 0) {
				remain = 0;
			}
		...
		if (buff.getParameter() == 0) {
			...
			info.setValue(buff.getParameter());//只会在大天使 生命，魔法池里面用
		}
...
//			}
	...
	private boolean specialHandling(Fighter source, Fighter adder, Buff buff, Q_buffBean buffModel){
		if (!(source instanceof Player)){
			return false;
		}
		...
		switch(buff.getActionType()){
		case BuffType.MAXHP_PLUS:
			//! 转换为固定值
		{	//! 加括号让其局部变量析构
			int percent = (player.getAttibute_one()[3] * 5);
			buff.setPercent(percent);
		}	
			return true;
		case BuffType.GUARDIANSPIRIT_PLUS:
			buff.setPercent(player.getAttibute_one()[2]);
			return true;
		case BuffType.ATTACK_PLUS:
			buff.setValue(player.getAttibute_one()[3] * 3000 / Global.MAX_PROBABILITY);
			return true;
		case BuffType.DEFENSE_PLUS:
			buff.setValue(player.getAttibute_one()[3] * 1000 / Global.MAX_PROBABILITY);
			return true;
		case BuffType.HP_PERCENT:
		{
			int percent = (player.getAttibute_one()[3] * 2 + 500);
			int value = adder.getMaxHp() * percent / Global.MAX_PROBABILITY;
			buff.setValue(value);
		}
			return true;
		}
	...
	private Buff getBuff(int type, int buffModelId) {
		switch (type) {
			...
			case BuffType.HORSEWEAPONPROTECT:
				return new StateBuff();
			//93商城购买指定道具时,降低价格(指定数量的钻石),可叠加
			case BuffType.MALLBUYREDUCE:
				return new AttributeBuff();
			case BuffType.GUARDIANSPIRIT:	
				return new AttributeBuff();
			//81 脚本BUFF
			...
			case BuffType.ADD_MP_BY_TIME:
				return new TimeBuff();
			//114 增加固定力量
			case BuffType.FIXED_STRENGTH:
			//115 增加固定敏捷
			case BuffType.FIXED_AGIL:
			//116 增加固定体力
			case BuffType.FIXED_VITALITY:
			//117 增加固定智力
			case BuffType.FIXED_INTELLIGENCE:
		case BuffType.PERFECT_ATTACK_PERCENT:
		case BuffType.KNOWING_ATTACK_PERCENT:
		case BuffType.IGNORE_ATTACK_PERCENT:
		case BuffType.PERFECT_ATTACK_ADDITION:
		case BuffType.KNOWING_ATTACK_ADDITION:
		case BuffType.MAXHP_PLUS:
		case BuffType.GUARDIANSPIRIT_PLUS:
		case BuffType.ATTACK_PLUS:
		case BuffType.DEFENSE_PLUS:
		case BuffType.ALL_ATTONE:
				return new AttributeBuff();
		case BuffType.SPECIAL_EFFECT:
		case BuffType.HP_PERCENT:
			return new TimeBuff(); 
		//124 变身类型
		case BuffType.SHAPE_CHANGE:
			return new StateBuff();
		//125 自爆buff
		case BuffType.ZIBAO:
			return new TimeBuff();
