...
//				Q_characterBean q_characterBean = DataManager.getInstance().q_characterContainer.getMap().get(player.getLevel());
				...
				*/
				...
				/*xiaozhuoming: 暂时没有用到
				MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM, String.format(ResManager.getInstance().getString("崇拜奖励：增加 %d 金币"), message.getMoney()));
				MessageUtil.notify_player(player, Notifys.MOUSEPOS, String.format(ResManager.getInstance().getString("崇拜奖励：增加 %d 经验, 增加 %d 金币"), exp, message.getMoney()));
				*/
			} else {
				//TODO 根据副本类型获得奖励
	...
	public void resGetTopTitleToServer(Player player, ResGetTopTitleToServerMessage message) {
		if (player != null) {
			...
			if(message.getTitleidlist().size()>0){
