...
public class CooldownManager {
	 ...
	 * @param delay 冷却时间
	 */
	public void addCooldown(SummonPet pet, CooldownTypes type, String key, long delay) {
		//获得玩家
		if (pet == null) {
			return;
		}
	 ...
	 * @param key 关键字
	 */
	public void removeCooldown(SummonPet pet, CooldownTypes type, String key) {
		//获得玩家
		if (pet == null) {
			return;
		}
	 ...
	 * @return
	 */
	public boolean isCooldowning(SummonPet summonpet, CooldownTypes type, String key) {
		//获得玩家
		if (summonpet == null) {
			return true;
		}
		...
		//查看冷却
		if (summonpet.getCooldowns().containsKey(cooldownKey)) {
			Cooldown cooldown = summonpet.getCooldowns().get(cooldownKey);
			if (System.currentTimeMillis() > cooldown.getStart() + cooldown.getDelay()) {
				//冷却时间已经结束
				return false;
			} else {
