...
import com.game.player.structs.UnGuideType;
...
public class PlayerDaZuoManager {
	...
	//真气数量
	public  static final String  ZHENQI_NUM= "ZHENQI_NUM";
...
//					}
	...
	public void sendShuangXiuApply(Player player, long target) {
		 ...
		 * if (other.getSex() == player.getSex()) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉,不可以同性结阵打坐"));
			return;
		}*/
		...
		if (isRoleShuangXiu(other)) {
			/* xuliang
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("很抱歉,对方正在与其他玩家结阵打坐中"));
			*/
			return;
		}
		...
		if (time != null) {
			if (System.currentTimeMillis() - time < 10 * 1000) {
				/* xuliang
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("您刚刚对该玩家发送过结阵打坐请求"));
				*/
				return;
			}
		}
...
//		MessageUtil.notify_player(player,Notifys.NORMAL,"您对玩家{1}发送了双修请求",other.getName());
	...
	public void refuseShuangXiu(Player player, Player sender) {
		if (sender != null) {
			...
			/* xuliang
			MessageUtil.notify_player(player, Notifys.NORMAL, ResManager.getInstance().getString("玩家{1}拒绝了您的结阵打坐请求"), player.getName());
			*/
		}
	}
	...
	public void startOrEndDaZuo(Player player) {
		if (isSit(player)) {
			...
			if (showPet != null&&!showPet.isDie()) {
				...
				UnGuideType.setGuideType(player, UnGuideType.dazuo_ningdan);
			} else {
				//单人打坐
				int addtimer = 0;
				if (player.checkKingCityBuffOfKing() || player.checkKingCityBuffOfNormal()) {
...
//			MessageUtil.notify_player(player, Notifys.NORMAL, "本地图禁止打坐");
		...
		if (isRoleShuangXiu(player)) {
			/* xuliang
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("您已经在跟其他玩家结阵打坐中了"));
			//己经处于双修状态
			 */
			return;
		}
...
//		HorseManager.getInstance().unride(sender);
		...
		UnGuideType.setGuideType(player, UnGuideType.dazuo_ningdan);
		UnGuideType.setGuideType(sender, UnGuideType.dazuo_ningdan);
	}
...
//		}
		...
		if (isSingleSit(player)) {
			...
			if (dazuoEruptExp > 0 || dazuoEruptZq > 0) {
				...
				if (dazuoEruptZq != 0 && VipManager.getInstance().getVIPLevel(player) == 0) {
					/* xuliang
					ParseUtil parseUtil = new ParseUtil();
					parseUtil.setValue(String.format(ResManager.getInstance().getString("您获得打坐暴击经验%d真气%d（{@}+真气0）"), dazuoEruptExp, dazuoEruptZq), new ParseUtil.VipParm(VipManager
							.getInstance().getVIPLevel(player), 2));
...
//			MessageUtil.notify_player(player, Notifys.NORMAL, "您退出了双修状态");
		...
		if (isRoleShuangXiu(player)) {
			...
			if (Exp > 0 || Zq > 0) {
				...
				if (Zq != 0 && VipManager.getInstance().getVIPLevel(other) == 0) {
					/* xuliang
					ParseUtil parseUtil = new ParseUtil();
					parseUtil.setValue(String.format(ResManager.getInstance().getString("您获得打坐暴击经验%d真气%d（{@}+真气0）"), Exp, Zq), new ParseUtil.VipParm(VipManager.getInstance()
							.getVIPLevel(other), 2));
			...
			/* xuliang
			MessageUtil.notify_player(other, Notifys.NORMAL, ResManager.getInstance().getString("由于对方中断了打坐，您退出了结阵打坐状态"));
			*/
			ResDazuoStateBroadCastMessage msg = new ResDazuoStateBroadCastMessage();
			msg.setRoleAId(player.getId());
			msg.setRoleAX(player.getPosition().getX());
...
//		Q_characterBean model = DataManager.getInstance().q_characterContainer.getMap().get(player.getLevel());
		...
		if (addZhenqi != 0 && VipManager.getInstance().getVIPLevel(player) == 0) {
			/* xuliang
			ParseUtil parseUtil = new ParseUtil();
			parseUtil.setValue(String.format(ResManager.getInstance().getString("您游泳获得真气%d（{@}+真气0）"), addZhenqi),
					new ParseUtil.VipParm(VipManager.getInstance().getVIPLevel(player), 2));
			MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM, parseUtil.toString());
			*/
		} else {
			/* XULIANG
		...
		if (!PlayerManager.getInstance().isFullHp(player)) {
			...
			if(addhp >= min){
				logger.debug("玩家：" + player.getName() + "(" + player.getId() + ")回复生命" + addhp + ",因为游泳");
			}
...
//		Q_characterBean model = DataManager.getInstance().q_characterContainer.getMap().get(player.getLevel());
		...
		if (addZhenqi != 0 && VipManager.getInstance().getVIPLevel(player) == 0) {
			ParseUtil parseUtil = new ParseUtil();
			parseUtil.setValue(String.format(ResManager.getInstance().getString("您打坐获得真气%d（{@}+真气0）"), addZhenqi),
					new ParseUtil.VipParm(VipManager.getInstance().getVIPLevel(player), 2));
			MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM, parseUtil.toString());
		} else {
		...
		if (!PlayerManager.getInstance().isFullHp(player)) {
			...
			int min = player.getMaxHp() * 2000/ Global.MAX_PROBABILITY;
			if(addhp >= min){
				logger.debug("玩家：" + player.getName() + "(" + player.getId() + ")回复生命" + addhp + ",因为打坐");
			}
			ManagerPool.playerManager.onHpChange(player);
		}
		if (!PlayerManager.getInstance().isFullMp(player)) {
...
//		Q_characterBean model = DataManager.getInstance().q_characterContainer.getMap().get(player.getLevel());
		...
		if (!PlayerManager.getInstance().isFullHp(player)) {
			...
			int min = player.getMaxHp() * 2000/ Global.MAX_PROBABILITY;
			if(addhp >= min){
				logger.debug("玩家：" + player.getName() + "(" + player.getId() + ")回复生命" + addhp + ",因为和宠物打坐");
			}
			ManagerPool.playerManager.onHpChange(player);
		}
		if (!PlayerManager.getInstance().isFullMp(player)) {
...
//		Q_characterBean model = DataManager.getInstance().q_characterContainer.getMap().get(player.getLevel());
		...
		Pet showPet = PetInfoManager.getInstance().getShowPet(player);
		if(showPet!=null){
			xishu = xishu+0.2;//自己如果有宠物，加0.2
		}

		Player other = PlayerManager.getInstance().getPlayer(player.getDazuotarget());
		Pet otherPet = PetInfoManager.getInstance().getShowPet(other);
		if(otherPet!=null){
			xishu = xishu+0.2;//对方玩家如果有宠物，加0.2
		}
		
		int zhenQi = (int)Math.ceil((model.getQ_role_sxzq() *xishu * ((double)player.getZhenQiMultiple() / 10000 + zhenqibeishu)));
		int exp=(int)Math.ceil( model.getQ_role_sxexp() * xishu *((double)player.getExpMultiple()/10000+expbeishu));
		...
		if (!PlayerManager.getInstance().isFullHp(player)) {
			...
			int min = player.getMaxHp() * 2000/ Global.MAX_PROBABILITY;
			if(addhp >= min){
				logger.debug("玩家：" + player.getName() + "(" + player.getId() + ")回复生命" + addhp + ",因为和玩家打坐");
			}
			ManagerPool.playerManager.onHpChange(player);
		}
		if (!PlayerManager.getInstance().isFullMp(player)) {
...
//		}
	 ...
	 * @param msg
	 */
	public void stReqCohesionZhenQiMessage(Player player,ReqCohesionZhenQiMessage msg) {
		if (msg.getNum() > 0) {
			int zq = msg.getNum()*50000;
			if (player.getZhenqi() >= zq ) {
				if (player.getLevel() < 30) {
					return;
				}
				ManagerPool.playerManager.addZhenqi(player, -zq, AttributeChangeReason.NINGDAN_SELF);
				List<Item>	items = Item.createItems(9224, msg.getNum(), false, 0);
				for (Item item : items) {
					item.getParameters().put(PLAYER_ID ,player.getId()+"");
					item.getParameters().put(PLAYER_NAME ,player.getName());
					item.getParameters().put(ZHENQI_NUM ,"50000");
				}
				String name = ManagerPool.backpackManager.getName(9224);
				if(ManagerPool.backpackManager.addItems(player, items, Reasons.ADD_ZHENQI_DAN,Config.getId() )){
					MessageUtil.notify_player(player, Notifys.MOUSEPOS, ResManager.getInstance().getString("使用{1}真气,获得{2}颗{3}"),zq+"",items.size()+"",name);
				}else{
					MailServerManager.getInstance().sendSystemMail(player.getId(), player.getName(), name,"", (byte)1, 0, items);
					MessageUtil.notify_player(player, Notifys.MOUSEPOS, ResManager.getInstance().getString("使用{1}真气,获得{2}颗{3}，已经邮件发送给您"),zq+"",items.size()+"",name);
				}
			}
		}
	}
