...
public class CardManager {
	...
	public void resInnerCardToServer(Player player, ResInnerCardToServerMessage message) {
		if (player != null) 
		{
			...
			{
				case Error_PhoneSuccess:	//手机验证成功
					sendMailByCard(player.getId(), player.getName(), message.getGiftid());
					/*xiaozhuoming: 暂时没有用到
					MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("您已经通过手机验证获得了礼包奖励，请通过邮件领取。"));
					*/
					return;
				case Error_Success: 	//卡号验证成功	这里会用到包含角色在平台领取激活卡奖励自动返回给角色背包，需要解锁
	...
	public void sendMailByCard(long receiverid, String receivername, int itemid) {
		if (itemid != 0) {
			...
			if (!items.isEmpty()) {
				MailServerManager.getInstance().sendSystemMail(receiverid, receivername, ResManager.getInstance().getString("系统邮件"), ResManager.getInstance().getString("恭喜您通过手机验证获得礼包奖励，请点击附件领取。"), (byte) 1, 0, items);
			}
		}
	}

	//------------------------手机验证----------------------------//
	public void testGainCard(Player player, String cardcode)
	{
		ReqCardToServerMessage message = new ReqCardToServerMessage();
		message.setCard(cardcode);
		reqCardToServer(player, message);
	}
	public void testPInfo(Player player, String account, String phone)
	{
		ReqCardPhoneToServerMessage msg = new ReqCardPhoneToServerMessage();
		msg.setAccount(account);
		msg.setPhone(phone);
		msg.setPlayerid(player.getId());
		reqCardPhoneToServer(player, msg);
	}
