...
public class CountManager {
	 ...
	 * @param key		副本ID
	 * @return
	 * @create	hongxiao.z      2014-1-2 下午3:12:12
	 */
	public Count getCountEntity(Player player, CountTypes type, String key)
	{
		// 初始化冷却关键字
		String countKey = null;
		...
		return player.getCounts().get(countKey);
	}
	
	//获得上一次  记数 时间  
	public long getLastTime(Player player, CountTypes type, String key) {
		// 初始化冷却关键字
		String countKey = null;
		if (key == null) {
			countKey = type.getValue();
	  ...
	  //记录 是否清除CD 
		public void setClearCd(Player player, CountTypes type, String key,byte value) {
			// 初始化冷却关键字
			String countKey = null;
			if (key == null) {
				countKey = type.getValue();
			...
			if (count != null) {
				reset(count);
				count.setIsClear(value);
			}
		}
		
		  //记录 是否清除CD 
		public byte getClearCd(Player player, CountTypes type, String key) {
			// 初始化冷却关键字
			String countKey = null;
			if (key == null) {
				countKey = type.getValue();
