...
public class NpcManager {
	...
	public static boolean checkShop(int npcid,int shopModelId){
		Q_npcBean npc = ManagerPool.dataManager.q_npcContainer.getMap().get(npcid);
		if(npc==null) return false;
		if (StringUtil.isBlank(npc.getQ_shop()) || !npc.getQ_shop().contains(String.valueOf(shopModelId))) {
			return false;
		}
		return true;
	}
...
//			NPC传送序列（文字描述|MAPid,X,Y,半径|等级|钻石=Y金币=G|价格;文字描述|MAPid,X,Y,半径|等级|钻石=Y金币=G|价格）
		...
		if(list!=null){
			for (Q_npcBean npcmodel : list) {
				...
				if(npcmodel.getQ_px()>0 && npcmodel.getQ_py()>0){
					...
					if(grids==null){
						log.error("map is "+map.getMapModelid()+" and npcId is "+npcmodel.getQ_id());
					}
				...
				if(npcmodel.getQ_map()==CsysManger.CSYS_MAPID){
					Q_csysBean 	csysBean =  DataManager.getInstance().q_csysContainer.getMap().get(npcmodel.getQ_id()+"");
					
					
					npc.setShow(false);
					npc.getParameters().put("retime", System.currentTimeMillis()+ (CsysManger.REFRESTIME + RandomUtils.random(csysBean.getQ_rad_min(), csysBean.getQ_rad_max()))*1000);
...
//				}
	...
	public void  hideNpc(NPC npc){
		...
		MessageUtil.tell_round_message(npc, msg);
//		PersonalNoticeMessage cmsg = new PersonalNoticeMessage();
//		cmsg.setType(Notifys.SUCCESS.getValue());
//		cmsg.setContent("NPC隐藏");
//		MessageUtil.tell_round_message(npc, cmsg);
		npc.setShow(false);
	}
	...
	public void playerGather(Player player, NPC npc) {
		...
		for(MainTask task:player.getCurrentMainTasks()) {
			Q_task_mainBean taskModel= DataManager.getInstance().q_task_mainContainer.getMap().get(task.getModelid());
			if(taskModel.getQ_task_type() == TaskType.GATHER_ITEM4) {
				canGather = true;
				break;
			}
		}
		if(canGather && BackpackManager.getInstance().getEmptyGridNum(player) < 1) {
			canGather = false;
			MessageUtil.notify_player(player, Notifys.NORMAL, "背包空间不足");
		}
		if(!canGather)
			return;
		//停止走路
	 ...
	 * @param roleId 玩家Id
	 */
	public void playerGatherNoTask(Player player, NPC npc) {
		Q_npcBean npcBean = ManagerPool.dataManager.q_npcContainer.getMap().get(npc.getModelId());
		if(npcBean==null){
			//npc模板不存在
			return;
			...
			if(stopOther){
				...
				while (iter.hasNext()) {
					...
					if(PlayerState.GATHER.compare(player2.getState()) && player2.getGatherId() == player.getGatherId()){
						...
						MessageUtil.notify_player(player2, Notifys.SUCCESS, "已经被其他玩家采走了");
