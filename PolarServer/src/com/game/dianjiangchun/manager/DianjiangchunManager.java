...
public class DianjiangchunManager {
	...
	private static byte s_btRmbCount = 10;			//点绛唇默认消耗钻石数
...
//	}
	...
	public void GetBeginDianjiangchun(Player vPlayer) {
		if (vPlayer.getLevel() < 45) {
			/*xiaozhuoming: 暂时没有用到
			MessageUtil.notify_player(vPlayer, Notifys.MOUSEPOS, ResManager.getInstance().getString("45级以后可以使用此功能"));
			*/
			return;
		}
...
//		if (!isMaxCount(vPlayer)) {
		if (!isAllChun(vPlayer)) {
			if (!isNoFreeChangeLuck(vPlayer)) {
				...
				//检测2级密码
				if (ManagerPool.protectManager.checkProtectStatus(vPlayer) ) {
					return ;
				}
				if (isRmbEnough(vPlayer)) {
					if (ExecuteChangeLuck(vPlayer)) {
						/* xuliang
						MessageUtil.notify_player(vPlayer, Notifys.SUCCESS, ResManager.getInstance().getString("花费{1}钻石，逆天改运成功"), String.valueOf(getS_btRmbCount()));
						*/
						SendDianjiangchunInfo(vPlayer, Status_Change);
					}
				} else {
					/* xuliang
					MessageUtil.notify_player(vPlayer, Notifys.ERROR, ResManager.getInstance().getString("对不起，您当前钻石不足，无法逆天改运！是否前往充值"));
					//TODO 客户端提示弹窗
					*/ 
...
//		}
	...
	public void GetReceiveintinfuriatingvalue(Player vPlayer) {
		...
		if (vPlayer.getStDianjiangchunSaveData().getnInfuriatingvalue() != 0) {
			...
			if (vPlayer.getStDianjiangchunSaveData().getnInfuriatingvalue() == 0) {
				TaskManager.getInstance().action(vPlayer, Task.ACTION_TYPE_RANK, TaskEnum.INCENSELIP, GetChunCount(vPlayer, getS_btChun()));
				vPlayer.getStDianjiangchunSaveData().getBosonList().clear();
			}
