...
public class GuildFlagManager {
	...
	//战盟争夺战状态 0 关闭，1开启，2结束（2只是用来区分下一次争夺战开始时间）
...
//				guildnamemap.clear();
//				attackjfmap.clear();
//				MessageUtil.notify_All_player(Notifys.SROLL, "战盟领地争夺战开始了!");
...
//				setFlagwarstatus(2);
//				setFlagwarendtime(0);
//				MessageUtil.notify_All_player(Notifys.SROLL, "今日的战盟领地争夺战结束了!");
//				ResGuildFlagStatusToClientMessage cmsg = new ResGuildFlagStatusToClientMessage();
//				MessageUtil.tell_world_message(cmsg);
...
//	
	...
	public void testflag(Player player,int type){
		if (type == 1) {
			...
			guildnamemap.clear();
			attackjfmap.clear();
			MessageUtil.notify_All_player(Notifys.SROLL, ResManager.getInstance().getString("战盟领地争夺战开始了!"));
			ResGuildFlagStatusToClientMessage cmsg = new ResGuildFlagStatusToClientMessage();
			cmsg.setTime(flagwarmaxtime);
			MessageUtil.tell_world_message(cmsg);
		}else if (type== 2) {
			...
			MessageUtil.notify_All_player(Notifys.SROLL, ResManager.getInstance().getString("今日的战盟领地争夺战结束了!"));
			ResGuildFlagStatusToClientMessage cmsg = new ResGuildFlagStatusToClientMessage();
			MessageUtil.tell_world_message(cmsg);
		}else if (type==3) {
	...
	public void stReqInsertGuildFlagToGameMessage(Player player) {
		try {
			...
			if(player.getGuildId() == 0){
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("只能盟主或者副盟主才能安插盟旗。"));
				return;
			}
			
			if ( player.getMemberInfo().getGuildPowerLevel() != 1 && player.getMemberInfo().getGuildPowerLevel() != 2) {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("只能盟主或者副盟主才能安插本盟盟旗。"));
				return;
			}
			...
			if (mapidlist.contains(map.getMapModelid()) == false) {
				/* xuliang
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("只有在【赵苑及以上地图】中才可安插盟旗。"));
				*/
				return;
			}
			...
			if (territorymap.containsKey(map.getMapModelid())) {
				...
				if (guildTerritoryFlag.getGuildid() == player.getGuildId()) {
					MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("当前地图已安插本盟旗帜。"));
				}else {
					MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("当前地图已存在盟旗，将其破坏后才能安插新旗帜。"));
				}
				return;
			}
			...
			if (attackjfmap.containsKey(map.getMapModelid())) {
				...
				if (guildFiagJF.getPriorityguildid() != player.getGuildId()  && guildFiagJF.getPriorityguildid() > 0) {
					if ( (int)(System.currentTimeMillis()/1000)- guildFiagJF.getPrioritytime() < 60* 3) {
						MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("【{1}】盟派在刚才的领地争夺战中砍旗最勇猛，拥有插旗优先权3分钟."),guildFiagJF.getPriorityguildname());
						return;
					}
				}
			}
			...
			if (player.getGuildInfo().getStockGold()  >= money) {
				...
				if (distance > 20*MapUtils.GRID_BORDER) {//20格距离内
					MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("您需要到当前地图{1}（盟旗安插的坐标）坐标附近才能安插盟旗。"),xyString);
					return;
				}
				...
				GuildTmpInfo guildTmpInfo = GuildServerManager.getInstance().getGuildTmpInfo(player.getGuildId());	//读取战盟临时数据
				...
				MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("恭喜您成功的安插了盟旗。"));
	
				MessageUtil.notify_guild_all(player.getGuildId() , String.format(ResManager.getInstance().getString("我盟在【%s】%s成功安插了一面新的盟旗"),mapdb.getQ_map_name(),xyString));
				saveguildfiag();
				MessageUtil.notify_All_player(Notifys.CUTOUT, ResManager.getInstance().getString("【{1}】盟派在【{2}】成功安插了一面新的盟旗"),gname , mapdb.getQ_map_name());
				...
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("盟贡仓库金币不足{1}，不能插旗。"),money+"");
			}
		} catch (Exception e) {
	...
	public long getjifensequence(int mapmodelid){
		if (attackjfmap.containsKey(mapmodelid)) {
			...
			guildFiagJF.getJifenmap().clear();//清除本地图进攻战盟记录积分
			guildFiagJF.setPriorityguildid(gid);//记录最高分数的战盟
	...
	public String getGuildName(long gid){
		...
		return ResManager.getInstance().getString("未知战盟");
	}
