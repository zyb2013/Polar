	...
	public void action() {
		...
		if (territorymap.containsKey(this.mapId)) {	//此地图已经被占领
			...
			for (int flagid : flagmonidlist) {
				...
				if (monsters.size() > 0) {
					long gid = 0;
					if (monsters.get(0).getParameters().containsKey("guildid")) {
						gid = (Long)monsters.get(0).getParameters().get("guildid");
					}
					if ((flagid == guildTerritoryFlag.getFlagid() ) && (gid == guildTerritoryFlag.getGuildid())) {
						is = true;	//已经存在
						if (ms%5 == 0) {	//5秒自动补血
			...
			if (is==false && guildTerritoryFlag.getFlagid() > 0) {	//刷新盟旗
				if (!flagmonidlist.contains(guildTerritoryFlag.getFlagid())) {
					log.error(guildTerritoryFlag.getFlagid()+"旗帜不在全局变量表盟旗数据列表中");
					return;
				}
				...
				mon.setName("【"+guildTerritoryFlag.getGuildname()+ResManager.getInstance().getString("】的盟旗"));
			...
			if (ms%5 == 0) {	//5秒执行一次,不符合条件的地图进行删除操作
				for (int flagid : flagmonidlist) {
					List<Monster> monsters = ManagerPool.monsterManager.getSameMonster(map,flagid);	//根据旗帜ID找怪物
					if (monsters.size() > 0) {
						for (Monster monster : monsters) {
			...
			if (attackjfmap.containsKey(map.getMapModelid())) {
				...
				if (guildFiagJF.getPriorityguildid() > 0 && guildFiagJF.getPrioritytime() > 0 ) {
					if (  (int)(System.currentTimeMillis()/1000) - guildFiagJF.getPrioritytime() >= 60* 3) {
						attackjfmap.remove(map.getMapModelid());	//3分钟后清除本地图插旗优先权的战盟记录
					}
				}
			}
		}
