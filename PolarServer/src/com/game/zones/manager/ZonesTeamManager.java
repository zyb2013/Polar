...
import com.game.zones.message.ResZoneTeamErroMessage;
...
public class ZonesTeamManager {
	...
	public static int EMGC_MIN =1;
	public static int EMGC_MAX =8;

	public static int XSCB_MIN =7001;
	public static int XSCB_MAX =7008;
	 ...
	 * @param parameter
	 */
	public void stReqZoneTeamOpenToGameMessage(Player player,int zoneId) {
		...
		for (Q_clone_activityBean q_clone_activityBean : actlist) {
			if (q_clone_activityBean.getQ_zone_type()==3 || q_clone_activityBean.getQ_zone_type()==1) {
				if(q_clone_activityBean.getQ_min_lv()<=player.getLevel()&&q_clone_activityBean.getQ_max_lv()>=player.getLevel()){
				
				ZoneTeamInfo info=new ZoneTeamInfo();
				int zoneModelid = q_clone_activityBean.getQ_id();
				
				
				int cdzone = zoneModelid;
				if(zoneModelid >= ZonesTeamManager.EMGC_MIN && zoneModelid <= ZonesTeamManager.EMGC_MAX){
					cdzone = ZonesTeamManager.EMGC_MIN;
				}
				if(zoneModelid >= ZonesTeamManager.XSCB_MIN && zoneModelid <= ZonesTeamManager.XSCB_MAX){
				...
				if (status == 0) {
					ManagerPool.countManager.addCount(player, CountTypes.ZONE_TEAM_ST, ""+cdzone,1, 0, 0);
				}
...
//				}
				...
				if(ManagerPool.countManager.getClearCd(player, CountTypes.ZONE_MANUAL, ""+cdzone)==0){
				
					if(q_clone_activityBean.getQ_wait_time()- (System.currentTimeMillis()-lastTime)>0){
						info.setIsopen((byte) 3);
						//MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("副本在冷却中。"));	
						//return;
					}
				}
				info.setLast_time(lastTime);
				info.setWait_time(	q_clone_activityBean.getQ_wait_time());
				
				if(q_clone_activityBean.getQ_id()>=3007&&q_clone_activityBean.getQ_id()<=3008){
					if(VipManager.getInstance().getVIPLevel(player)+3003>=q_clone_activityBean.getQ_id()){
						info.setEnterMaxnum(q_clone_activityBean.getQ_manual_num());
					}
				}else
				if(q_clone_activityBean.getQ_id()==3009){
					if(VipManager.getInstance().getVIPLevel(player)==9){
						info.setEnterMaxnum(q_clone_activityBean.getQ_manual_num());
					}
				}else{
					info.setEnterMaxnum(q_clone_activityBean.getQ_manual_num()+VipManager.getInstance().getDevilAddition(player));
				}
				...
				if(zoneId==0 ||zoneId ==info.getZoneid()){
...
//		}
		...
		if (map.isCopy() ==true) {
			MessageUtil.notify_player(player,Notifys.ERROR, ResManager.getInstance().getString("很抱歉，您正处于副本中，请先退出本副本再进行本操作"));
			return;
		}
		//判断 是否有  进入副本的 道具要求
		if(zonedata.getQ_need_item()!=0){
			int  ItemCount =ManagerPool.backpackManager.getItemNum(player, zonedata.getQ_need_item());
			
			if(zonedata.getQ_id() < 3007 &&zonedata.getQ_id() >3009){
				if(ItemCount<1){
		...
		if(zonedata.getQ_wait_time()!=0){
			...
			if(ManagerPool.countManager.getClearCd(player, CountTypes.ZONE_MANUAL, ""+cdzone)==0){
				...
				if(zonedata.getQ_wait_time()- (System.currentTimeMillis()-lastTime)>0){
...
//		}
		...
		if (msg.getEntertype() == 0 && (zonedata.getQ_type() == 1 || zonedata.getQ_type() == 3)) {//单人进入
			...
			if(zonedata.getQ_id()>=3007&&zonedata.getQ_id()<=3008){
				if(VipManager.getInstance().getVIPLevel(player)+3003<zonedata.getQ_id()){
					MessageUtil.notify_player(player,Notifys.ERROR, ResManager.getInstance().getString("VIP等级不足"));
					return ;
					
				}
			}
			if(zonedata.getQ_id()==3009){
				if(VipManager.getInstance().getVIPLevel(player)!=9){
					MessageUtil.notify_player(player,Notifys.ERROR, ResManager.getInstance().getString("VIP等级不足"));
					return ;
				}
			...
			if(zonedata.getQ_need_item()!=0){
				...
				if(zonedata.getQ_id() >= 3007 &&zonedata.getQ_id() <=3009){
					int needCount = zonedata.getQ_id()-3005;
					
					
					int usercount = ManagerPool.backpackManager.getItemNum(player, zonedata.getQ_need_item());
					
					
					if(usercount>needCount){
						
						if(!ManagerPool.backpackManager.removeItem(player, zonedata.getQ_need_item(),4, Reasons.BUYITEM, Config.getId())){
						...
						if(usercount!=0){
							//先扣钱 再扣道具
							if(!ManagerPool.backpackManager.changeGold(player, -(needCount-usercount)*10, Reasons.YBBUY, Config.getId())){
								MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("钻石不足"));
								return;
							}else{
								//扣道具 
								if(!ManagerPool.backpackManager.removeItem(player, zonedata.getQ_need_item(),usercount, Reasons.BUYITEM, Config.getId())){
									MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("道具扣除失败"));
									return;
								}
							}
					...
					if(!ManagerPool.backpackManager.removeItem(player, zonedata.getQ_need_item(),1, Reasons.BUYITEM, Config.getId())){
			...
			ICreateZoneScript script = (ICreateZoneScript) ManagerPool.scriptManager.getScript(ScriptEnum.ZONE_CREATE);
 			...
 			if (zone != null) {
 				...
 				zone.getOthers().put("playercount", 1);
...
//				cmsg.setZoenmonstrinfolist(getMapMonsterInfo(monnuMap));
				MessageUtil.tell_player_message(player, cmsg);
				if (manual == 0) {	//初始化计数器
					ManagerPool.countManager.addCount(player, CountTypes.ZONE_MANUAL, ""+cdzone,1, 1, 0);
				}else{
					ManagerPool.countManager.addCount(player, CountTypes.ZONE_MANUAL, cdzone+"", 1);	//现在改成手动和自动共用一个变量
				}
 			}
...
//			Player xplayer = ManagerPool.playerManager.getPlayer(member.getId());
			if(member==null){
				notify_team_player_erro(1,player, zonedata.getQ_id(),info,team,Notifys.ERROR, ResManager.getInstance().getString("很抱歉，队友【{1}】 不在线"),info.getMembername());
			...
			//判断 是否有  进入副本的 道具要求
			if(zonedata.getQ_need_item()!=0){
				int  ItemCount =ManagerPool.backpackManager.getItemNum(member, zonedata.getQ_need_item());
				
				if(ItemCount<1){
					notify_team_player_erro(2,player, zonedata.getQ_id(),info,team,Notifys.ERROR, ResManager.getInstance().getString("很抱歉，队友【{1}】 进入副本所需道具不足"),info.getMembername());
					return false;
				}
			}
			...
			//判断 此次 CD 是否购买过   0代表没买过
			if(ManagerPool.countManager.getClearCd(member, CountTypes.ZONE_MANUAL, ""+cdzone)==0){
			if(zonedata.getQ_wait_time()!=0){
				long lastTime = ManagerPool.countManager.getLastTime(member, CountTypes.ZONE_MANUAL, ""+cdzone);
				if(zonedata.getQ_wait_time()- (System.currentTimeMillis()-lastTime)>0){
					notify_team_player_erro(3,player, zonedata.getQ_id(),info,team,Notifys.ERROR, ResManager.getInstance().getString("很抱歉，队友【{1}】 副本在冷却中。"),info.getMembername());
					return false;
				}
...
//					notify_team_player_erro(tmpplayer, zonedata.getQ_id(),info,team,Notifys.ERROR, "{1}和您不在同个地图内，无法开启副本组队传送",info.getMembername());
//					return false;
//				}
				
				if (member.getCountry() != player.getCountry() ) {
					/* xuliang
					notify_team_player_erro(4,player, zonedata.getQ_id(),info,team,Notifys.ERROR, ResManager.getInstance().getString("很抱歉，队友【{1}】不在同个服务器或者国家内，无法进入副本"),info.getMembername(),zonedata.getQ_min_lv()+"");
					*/
					return false;
				}
				...
				if (membermap == null) {
					if (info.getMemberid() == player.getId()) {
						...
						notify_team_player_erro(5,player, zonedata.getQ_id(),info,team,Notifys.ERROR, ResManager.getInstance().getString("很抱歉，队友【{1}】不在地图中，因此无法组队进入副本"),info.getMembername());
					}
					return false;
				}
				...
				if ( membermap.isCopy()) {
					if (info.getMemberid() == player.getId()) {
						...
						notify_team_player_erro(6,player, zonedata.getQ_id(),info,team,Notifys.ERROR, ResManager.getInstance().getString("很抱歉，队友【{1}】正处于某个副本中，因此无法组队进入副本"),info.getMembername());
					}
					return false;
				}
				
				if (zonedata.getQ_min_lv() > member.getLevel()) {
					notify_team_player_erro(7,player, zonedata.getQ_id(),info,team,Notifys.ERROR, ResManager.getInstance().getString("很抱歉，队友【{1}】的等级低于{2}级，不能进入此副本"),info.getMembername(),zonedata.getQ_min_lv()+"");
					return false;
				}
			
				if (zonedata.getQ_max_lv() < member.getLevel()){
					notify_team_player_erro(8,player, zonedata.getQ_id(),info,team,Notifys.ERROR, ResManager.getInstance().getString("很抱歉，队友【{1}】的等级高于{2}级，不能进入此副本"),info.getMembername(),zonedata.getQ_max_lv()+"");
					return false;
				}
				
				if (!ManagerPool.mapManager.ischangMap(member)) {
					notify_team_player_erro(9,player, zonedata.getQ_id(),info,team,Notifys.ERROR, ResManager.getInstance().getString("很抱歉，队友【{1}】没有脱离战斗状态或者其他原因导致不能进入副本。"),info.getMembername(),zonedata.getQ_max_lv()+"");
					return false;
				}
				
				if (member.isDie()) {
					notify_team_player_erro(10,player, zonedata.getQ_id(),info,team,Notifys.ERROR, ResManager.getInstance().getString("很抱歉，队友【{1}】处于死亡状态，无法进入副本。"),info.getMembername());
					return false;
				}
...
//					}
//				}
				long manual = ManagerPool.countManager.getCount(member, CountTypes.ZONE_MANUAL, ""+cdzone);
				
				if ((manual >= zonedata.getQ_manual_num()) && zonedata.getQ_id()>=3007 && zonedata.getQ_id()<=3009 ) {
					notify_team_player_erro(11,player, zonedata.getQ_id(),info,team,Notifys.ERROR, ResManager.getInstance().getString("很抱歉，队友【{1}】今日参与次数已用完，因此无法组队进入副本"),info.getMembername());
					return false;
				}
				...
				/* xuliang
				notify_team_player_erro(12,player, zonedata.getQ_id(),info,team,Notifys.ERROR, ResManager.getInstance().getString("很抱歉，队友【{1}】可能不在本国，或者已经下线，因此无法组队进入副本"),info.getMembername());
				*/
				return false;
			}
		}
...
//		}
	...
	public void notify_team_player_erro(int errotype,Player tmpplayer,int zoneId,TeamMemberInfo erroinfo,TeamInfo team, Notifys type, String message, String... values) {
		
		ResZoneTeamErroMessage msg = new ResZoneTeamErroMessage();
		msg.setMemberid(erroinfo.getMemberid());
		msg.setName(erroinfo.getMembername());
		msg.setZoneid(zoneId);
		msg.setType((byte)errotype);
		MessageUtil.tell_player_message(tmpplayer,msg);
...
//		}
	 ...
	 * @param player
	 */
	public void longinEnd(Player player) {
			if (!player.isCross()) {
				// 盟战状态，上线通知
				...
				if(map!=null){
					...
					if(zone!=null){
						...
						if(zoneBean.getQ_id()==6001){
							if((int)zone.getOthers().get("zoneprocess")==1){
								timemsg.setSurplustime((Integer)zone.getOthers().get("time") + 10*60 - (int)(System.currentTimeMillis()/1000));
							}else{
								timemsg.setSurplustime((Integer)zone.getOthers().get("time") + 20*60 - (int)(System.currentTimeMillis()/1000));
							}
						}else{
						...
						int time = (int)(System.currentTimeMillis()/1000) - (int)zone.getOthers().get("time");
						boolean issend = true;
						if(time<=10){
							timemsg.setZoneprocess(0);
						}
						else{
							if(zone.getOthers().get("zoneprocess")!=null){
						...
						if(issend){
							...
							paramMap.put("zonemodelid", zone.getZonemodelid());
							...
							if(zone.getOthers().get("scriptId")!=null){
								sendMessage.setScriptid((Integer)zone.getOthers().get("scriptId"));
							}
