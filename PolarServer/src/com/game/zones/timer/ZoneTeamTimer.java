...
//												if (manual == 0) {	//初始化计数器
//													ManagerPool.countManager.addCount(member, CountTypes.ZONE_MANUAL, ""+zonedata.getQ_id(),1, 0, 0);
//												}
												
												if(zonedata.getQ_need_item()!=0){
													if(!ManagerPool.backpackManager.removeItem(member, zonedata.getQ_need_item(),1, Reasons.BUYITEM, Config.getId())){
														MessageUtil.notify_player(member, Notifys.ERROR, ResManager.getInstance().getString("道具扣除失败 无法进入副本"));
														continue;
													}
												}
										...
										zone.getOthers().put("levellist",levellist);	//往副本写入人物等级
						 				zone.getOthers().put("playercount", info.getMemberinfo().size());
