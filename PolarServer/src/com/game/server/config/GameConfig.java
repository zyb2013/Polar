...
public class GameConfig {
	...
	/**
	 * 服务器开服时间索引 key为服务器Id value为服务器开服时间
	 */
	...
	public void stGmServerTime(Player player, String str1, String str2) {
		...
		if (date != null) {
			...
			MessageUtil.notify_player(player, Notifys.SUCCESS,
					"{1}国开服时间已改为{2}",
					getCountryNameByConutry(player.getCountry()), timestr);
			*/
		} else {
	...
	public void stGmServerTime(String serverid, String str1, String str2,
			String httpresult) {
		try {
			String timestr = str1 + " " + str2;
			Pattern pattern = Pattern
			...
			Matcher time = pattern.matcher(timestr);
			if (time.find() == false) {
				HttpUtil.wget(httpresult
						+ "&result=0&tip="
						+ URLEncoder.encode("格式错误，必须为yyyy-MM-dd HH:mm:ss",
								"UTF-8") + "&location=server");
				return;
			}
			Date date = getConfigTimeToDate(timestr);
			if (date != null) {
				...
				HttpUtil.wget(httpresult
						...
						+ "&tip="
						+ URLEncoder.encode(
								getCountryNameByConutry(Integer
										.parseInt(serverid))
										+ "国开服时间已改为"
										+ timestr, "UTF-8"));
			} else {
				HttpUtil.wget(httpresult + "&result=0" + "&tip="
						+ URLEncoder.encode("时间解析出错", "UTF-8"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
