...
import com.game.buff.timer.SummonPetBuffTimer;
...
import com.game.map.timer.RobotPlayerAttackTimer;
...
import com.game.monster.timer.MonsterBossAiTimer;
...
import com.game.summonpet.timer.SummonPetAttackTimer;
import com.game.summonpet.timer.SummonPetHeartTimer;
import com.game.summonpet.timer.SummonPetStepTimer;
import com.game.summonpet.timer.SummonpetAiTimer;
...
public class MServer extends MapServer {
	...
	public void run() {
		...
		for (int i = 0; i < this.getMapConfigs().size(); i++) {
			...
			//初始化BOSS怪物AI定时
			((ServerThread)this.thread_pool.get(Server.DEFAULT_MAIN_THREAD)).addTimerEvent(new MonsterBossAiTimer(config.getServerId(), config.getLineId(), config.getMapId()));
			...
			*/
			...
			//定时计算召唤怪Buff
			((ServerThread)this.thread_pool.get(Server.DEFAULT_MAIN_THREAD)).addTimerEvent(new SummonPetBuffTimer(config.getServerId(), config.getLineId(), config.getMapId()));
			//定时格挡
...
//			
			...
			//召唤怪跟随AI
			((ServerThread)this.thread_pool.get(Server.DEFAULT_MAIN_THREAD)).addTimerEvent(new SummonPetHeartTimer(config.getServerId(), config.getLineId(), config.getMapId()));
			//召唤怪攻击AI
			((ServerThread)this.thread_pool.get(Server.DEFAULT_MAIN_THREAD)).addTimerEvent(new SummonPetAttackTimer(config.getServerId(), config.getLineId(), config.getMapId()));
			//召唤怪移动定时
			((ServerThread)this.thread_pool.get(Server.DEFAULT_MAIN_THREAD)).addTimerEvent(new SummonPetStepTimer(config.getServerId(), config.getLineId(), config.getMapId()));	
			//召唤怪血加成
			((ServerThread)this.thread_pool.get(Server.DEFAULT_MAIN_THREAD)).addTimerEvent(new SummonpetAiTimer(config.getServerId(), config.getLineId(), config.getMapId()));	
			//装备强化完成倒计时
			...
			*/
			...
			// 离线经验系统
			// ((ServerThread) this.thread_pool.get(Server.DEFAULT_MAIN_THREAD)).addTimerEvent(new OfflineTimer(config.getServerId(), config.getLineId(), config.getMapId()));
			//机器人模块
			((ServerThread) this.thread_pool.get(Server.DEFAULT_MAIN_THREAD)).addTimerEvent(new RobotPlayerAttackTimer(config.getServerId(), config.getLineId(), config.getMapId()));
