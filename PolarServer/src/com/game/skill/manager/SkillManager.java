...
public class SkillManager {
	 ...
	 * 通过技能书 学习技能
	 * 
	 * 此方法一般不用 所有技能自动学习 增加技能请调用addSkill方法
	 * 
	 * @param roldId
	 *            玩家Id
	 * @param skillModelId
	...
	public void study(Player player, int skillModelId, long bookId) {
		// 初学 默认取第一级
		Q_skill_modelBean skillModel = ManagerPool.dataManager.q_skill_modelContainer
				.getMap().get(skillModelId + "_" + 1);
		if (skillModel == null) {
		...
		if (player.getLevel() < skillModel.getQ_study_needgrade()) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("很抱歉，人物等级不足，无法学习"));
			return;
		}
		// 已拥有该技能
		if (isHaveSkill(player, skillModelId)) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("您已经学习过{1}技能，无需重复学习"), skillModel
					.getQ_skillName());
			return;
		}
		...
		if (bookId != 0) {
			...
			if (book == null) {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager
						.getInstance().getString("找不到技能书"));
				return;
			} else {
				if (book.isTrade()) {
					MessageUtil.notify_player(player, Notifys.ERROR, ResManager
							.getInstance().getString("很抱歉 ,物品正在被交易中无法使用"));
					return;
				}
				Q_itemBean bookmodel = ManagerPool.dataManager.q_itemContainer
						.getMap().get(book.getItemModelId());
				if (skillModelId != bookmodel.getQ_skill()) {
					log.error("传入的参数不一致 技能书学习传入学习的技能为" + skillModelId
							+ "物品上关联的技能为" + bookmodel.getQ_skill());
					return;
				}
				// 技能对应职业判断
				if (!PlayerManager.checkJob(player.getJob(), skillModel.getQ_job())) {
					MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("您不能学习 {1} 技能，职业不符"),skillModel.getQ_skillName());
					return;
				}
				...
				successMsg.setItemId(book.getItemModelId());
			...
			if (skillModel.getQ_study_needbook() != 0) {
				Q_itemBean bookmodel = ManagerPool.dataManager.q_itemContainer
						.getMap().get(skillModel.getQ_study_needbook());
				if (bookmodel == null) {
					log.error("找不到的技能书ID" + skillModel.getQ_study_needbook());
					return;
				}
				Item book = ManagerPool.backpackManager.getFirstItemByModelId(
						player, skillModel.getQ_study_needbook());
				if (book != null) {
					BackpackManager.getInstance().removeItemByCellId(player,
							book.getGridId(), Reasons.SKILLSTUDY, action);
				} else {
					MessageUtil.notify_player(
							player,
							Notifys.ERROR,
							ResManager.getInstance().getString(
									"很抱歉，您需要先拥有技能书《{1}》才能学习"),
	...
	private void startUpLevel(Player player, Skill skill) {
		...
		MessageUtil.tell_player_message(player,
				getSkillStartLevelUpMessage(skill));
	}
	...
	public void endUpLevel(Player player, Skill skill, int grade, boolean result) {
		...
		if (upLevel > 0) {
			...
			Q_skill_modelBean q_skill_modelBean = ManagerPool.dataManager.q_skill_modelContainer
					.getMap().get(skill.getSkillModelId() + "_" + skill.getSkillLevel());
			MessageUtil.notify_player(player, Notifys.CHAT_IMPORTANT, ResManager
					.getInstance().getString("恭喜您将技能{1}提升至{2}级！"), new ArrayList<GoodsInfoRes>(), 0, q_skill_modelBean.getQ_skillName(), skill.getSkillLevel()+"");
			MessageUtil.notify_player(player, Notifys.CHAT_PERSONAL, ResManager
					.getInstance().getString("恭喜您将技能{1}提升至{2}级！"), q_skill_modelBean.getQ_skillName(), skill.getSkillLevel()+"");
		}
	}
	...
	public void setLevel(Player player, Skill skill, int grade, boolean result) {
		...
		MessageUtil.tell_player_message(player,
				getLingWuResultMessage(skill, result));
	}
	 ...
	 * 
	 */
	public void skilllevelup(Player player, Skill skill, int grade,
			boolean result) {
	...
	public boolean addSkill(Player player, int skillModelId) {
		...
		/*//panic god 暂时屏蔽
		if (player.getJob() != skillModel.getQ_job()) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("您不能学习 {1} 技能，职业不符"), skillModel
					.getQ_skillName());
			return false;
		}*/
		...
		player.setSkillFightPower(player.getSkillFightPower() + skillModel.getQ_fight_bonus());
		// 告诉玩家添加技能成功
		MessageUtil.tell_player_message(player,
				getSkillAddMessage(player, skill));
		
		MessageUtil.notify_player(player, Notifys.CHAT_IMPORTANT, ResManager
				.getInstance().getString("恭喜您学会了强大的法术{1}"), new ArrayList<GoodsInfoRes>(), 0, skillModel.getQ_skillName());
		MessageUtil.notify_player(player, Notifys.CHAT_PERSONAL, ResManager
				.getInstance().getString("恭喜您学会了强大的法术{1}"), skillModel.getQ_skillName());

		// 被动技能增加buff
		if (skillModel.getQ_trigger_type() == 2
				&& skillModel.getQ_passive_action() == 0) {
			triggerSkill(player, player, skill, true);
		}

		// 自动添加技能栏
		// for (int i = 0; i < 5; i++) {
		// if (player.getShortCuts()[i] == null) {
		// ShortCutManager.getInstance().addShortCut(player, 2, skill.getId(), skillModel.getQ_skillID(), i + 1);
		// break;
		// }
		// }
		try {
	...
	public void goldEndUpLevel(Player player) {
		if (player.getNowLearnSkillId() <= 0 || player.getSkillLearnTime() < 0) {
			MessageUtil.notify_player(player, Notifys.NORMAL, ResManager
					.getInstance().getString("没有正在学习的技能"));
			return;
		}
		int nowLearnSkillModelId = player.getNowLearnSkillId();
		int skillLearnTime = player.getSkillLearnTime();
		Skill skillById = ManagerPool.skillManager.getSkillByModelId(player,
				nowLearnSkillModelId);
		if (skillById == null) {
			return;
		}
		Q_skill_modelBean q_skill_modelBean = DataManager.getInstance().q_skill_modelContainer
				.getMap().get(
						skillById.getSkillModelId() + "_"
								+ skillById.getSkillLevel());
		if (q_skill_modelBean == null) {
			return;
		}
		long action = Config.getId();
		if (DataManager.getInstance().q_skill_modelContainer.getMap().get(
				skillById.getSkillModelId() + "_"
						+ (skillById.getSkillLevel() + 1)) != null) {
			int q_up_need_time = DataManager.getInstance().q_skill_modelContainer
					.getMap()
					.get(skillById.getSkillModelId() + "_"
							+ (skillById.getSkillLevel() + 1))
					.getQ_up_need_time();
			int q_up_prob = DataManager.getInstance().q_skill_modelContainer
					.getMap()
					.get(skillById.getSkillModelId() + "_"
							+ (skillById.getSkillLevel() + 1)).getQ_up_prob();
			...
			if (skillById.getSkillLevel() < q_skill_modelBean.getQ_max_level()) {
				if (!BackpackManager.getInstance().checkGold(player, needgold)) {
					MessageUtil.notify_player(player, Notifys.ERROR, ResManager
							.getInstance().getString("很抱歉操作失败,钻石不够"));
					return;
				}
				// 检测2级密码
				...
				BackpackManager.getInstance().changeGold(player, -needgold,
						Reasons.YBSKILLSTUDY, action);
				if (RandomUtils.defaultIsGenerate(q_up_prob)) {
					result = true;
					endUpLevel(player, skillById,
							skillById.getSkillLevel() + 1, result);
				} else {
					endUpLevel(player, skillById, skillById.getSkillLevel(),
							result);
				}
			} else {
				MessageUtil.notify_player(player, Notifys.ERROR, ResManager
						.getInstance().getString("该技能已经达到最高等级"));
	 ...
	 * @return
	 */
	public SkillLingWuResultMessage getLingWuResultMessage(Skill skill,
			boolean result) {
	...
	public void levelUp(Player player, int skillModelId) {
		...
		// 组装当前技能模板ID字符串（当前技能ID_当前技能等级）
		String nowskillModelId = skill.getSkillModelId() + "_"
				+ (skill.getSkillLevel());
		// 组装目标技能模板ID字符串（当前技能ID_当前技能等级+1）
		String targetSkillModelId = skill.getSkillModelId() + "_"
				+ (skill.getSkillLevel() + 1);
		// 当前技能模板
		Q_skill_modelBean nowSkillModel = ManagerPool.dataManager.q_skill_modelContainer
				.getMap().get(nowskillModelId);
		// 目标技能模板
		Q_skill_modelBean targetSkillModel = ManagerPool.dataManager.q_skill_modelContainer
				.getMap().get(targetSkillModelId);
		// 获得当前技能最大等级
		int q_max_level = nowSkillModel.getQ_max_level();
		// 如果技能达到最大等级则通知玩家并返回
		if (skill.getSkillLevel() >= q_max_level) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("该技能达到了最高等级"));
			return;
		}
		// 如果目标技能为空，认为此技能已经达到最大等级，通知玩家并返回
		if (targetSkillModel == null) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("该技能达到了最高等级"));
			return;
		}
		// 如果玩家等级不够技能学习等级，通知玩家并返回
		...
		if (player.getNowLearnSkillId() > 0) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("很抱歉，同一时间只能研究一种技能"));
			return;
		}
		// 如果升级需要真气值不够
		if (targetSkillModel.getQ_up_need_zhengqi() > player.getZhenqi()) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("很抱歉，技能升级所需真气不足"));
			return;
		}
		// 如果技能所需金币不够
		if (targetSkillModel.getQ_up_need_copper() > player.getMoney()) {
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager
					.getInstance().getString("很抱歉，技能升级所需金币不足"));
			return;
		}
		// 生成唯一的ID作为action
		long action = Config.getId();

		// 查询升级所需材料编号_数量（格式：材料物品ID_数量;材料物品ID_数量;）
		...
		if (needGoods != null && needGoods.length() > 0) {
			...
			for (String string : split) {
				...
				int goodmodel = Integer.parseInt(split2[0]);
				int neednum = Integer.parseInt(split2[1]);
				ManagerPool.backpackManager.removeItem(player, goodmodel,
						neednum, Reasons.SKILLLEVELUP, action);
			}
		}
		// 扣除金币
		ManagerPool.backpackManager.changeMoney(player,
				-targetSkillModel.getQ_up_need_copper(), Reasons.SKILLLEVELUP,
				action);
		// 扣除真气
		ManagerPool.playerManager.addZhenqi(player,
				-targetSkillModel.getQ_up_need_zhengqi(),
				AttributeChangeReason.SKILLLEVELUP);
		// 获得当前技能的等级
	...
	public Skill getSkillByModelId(Player player, int nowLearnSkillId) {
		...
		// 暗器也需要处理下
		HiddenWeapon weapon = HiddenWeaponManager.getInstance()
			...
			Q_skill_modelBean skillModel = ManagerPool.dataManager.q_skill_modelContainer
					.getMap().get(
							skill.getSkillModelId() + "_"
									+ skill.getRealLevel(player));
			if (skillModel == null)
				continue;
			if (skillModel.getQ_trigger_type() != 2)
				continue;
			// 技能在攻击时触发
			if (skillModel.getQ_passive_action() == 1
					|| skillModel.getQ_passive_action() == 3) {
				skills.add(skill);
			}
		}

		skills.addAll(getHorseSkillTriggerByAttack(player));
		/*panic god 暂时没有这模块，屏蔽
	...
	public List<Skill> getSkillTriggerByDefense(Player player) {
		...
		while (iter.hasNext()) {
			...
			if (skillModel.getQ_trigger_type() != 2)
				continue;
			// 技能在防御时触发
			if (skillModel.getQ_passive_action() == 2
					|| skillModel.getQ_passive_action() == 3) {
				skills.add(skill);
			}
		}
	...
	public List<Skill> getHorseSkillTriggerByAttack(Player player) {
		List<Skill> skills = new ArrayList<Skill>();
		List<HorseSkill> hskills = ManagerPool.horseManager
				.selectActSkill(player);
	...
	public List<Skill> getPetSkillTriggerByAttack(Pet pet) {
		...
		for (int i = 0; i < pet.getSkills().length; i++) {
			...
			if (skillModel.getQ_trigger_type() != 2)
				continue;
			// 技能在攻击时触发
			if (skillModel.getQ_passive_action() == 6
					|| skillModel.getQ_passive_action() == 8) {
				skills.add(skill);
			}
		}
		return skills;
	...
	public List<Skill> getPetSkillTriggerByDefense(Pet pet) {
		...
		for (int i = 0; i < pet.getSkills().length; i++) {
			...
			if (skillModel.getQ_trigger_type() != 2)
				continue;
			// 技能在攻击时触发
			if (skillModel.getQ_passive_action() == 7
					|| skillModel.getQ_passive_action() == 8) {
				skills.add(skill);
			}
		}
	 ...
	 *            技能
	 * @param success
	 *            false-计算成功率 true-必定成功
	 * @return
	 */
	public int triggerSkill(Fighter source, Fighter target, Skill skill,
			boolean success) {
		...
		if (source instanceof Player) {
			skillModel = ManagerPool.dataManager.q_skill_modelContainer
					.getMap().get(
							skill.getSkillModelId() + "_"
									+ skill.getRealLevel((Player) source));
		} else {
			skillModel = ManagerPool.dataManager.q_skill_modelContainer
					.getMap().get(
							skill.getSkillModelId() + "_"
		...
		// 没有可触发buff
		if (skillModel.getQ_passive_buff() == null
				|| ("").equals(skillModel.getQ_passive_buff().trim()))
			return 0;
		...
		if (source instanceof Player) {
			// 技能冷却
			if (ManagerPool.cooldownManager.isCooldowning((Player) source,
					CooldownTypes.SKILL_TRIGGER,
					String.valueOf(skillModel.getQ_skillID()))) {
				return 0;
			}
			// 技能公共冷却
			if (ManagerPool.cooldownManager.isCooldowning((Player) source,
					CooldownTypes.SKILL_TRIGGER_PUBLIC,
					String.valueOf(skillModel.getQ_public_cd_level()))) {
				return 0;
			}
		} else if (source instanceof Pet) {
			Player player = ManagerPool.playerManager.getPlayer(((Pet) source)
					.getOwnerId());
			// 技能冷却
		...
		// 作用对象为主人
		if ((source instanceof Pet || source instanceof SummonPet ) && skillModel.getQ_target() == 6) {
			Player player = ManagerPool.playerManager.getPlayer(((Pet) source)
					.getOwnerId());
			target = player;
		}
		...
		if (skill.getSkillModelId() == Global.UNWEAR_HORSE_WEAPON_SKILL) {
			if (!(target instanceof Player)) {
				return 0;
			} else if (FighterState.HORSEWEAPONPROTECT.compare(target
					.getFightState())) {
				return 0;
			}
		}
        */
		...
		if (skillModel.getQ_restriction() != 0) {
			...
			Skill resist = null;
			if (target instanceof Player) {
				resist = getSkillByModelId((Player) target,
						skillModel.getQ_restriction());
				if (resist != null) {
					resistModel = ManagerPool.dataManager.q_skill_modelContainer
							.getMap()
							.get(resist.getSkillModelId() + "_"
									+ resist.getRealLevel((Player) target));
				...
				resist = getSkillByModelId((Monster) target,
						skillModel.getQ_restriction());
				if (resist != null) {
					resistModel = ManagerPool.dataManager.q_skill_modelContainer
							.getMap().get(
									resist.getSkillModelId() + "_"
											+ resist.getSkillLevel());
					int prob = RandomUtils.random(Global.MAX_PROBABILITY);
					if (prob < resistModel.getQ_passive_prob()) {
					} else {
						resistModel = null;
		...
		if (RandomUtils.random(Global.MAX_PROBABILITY) >= probability) {
			// BUFF添加未成功
			// 暗器技能被抵抗
			if (resistModel != null && skillModel.getQ_skill_user() == 5
		...
		// 火墙技能 走特殊流程
		if (skillModel != null && skillModel.getQ_skill_type() == 1
				&& source instanceof Player && target != null) {
			FightManager.getInstance().useGroundMagic((Player) source, skill,
					target.getMap(), target.getLine(), target.getPosition());
			return 0;
		}
		...
		// //添加技能冷却
		// ManagerPool.cooldownManager.addCooldown((Player)source,
		// CooldownTypes.SKILL_TRIGGER,
		// String.valueOf(skillModel.getQ_skillID()),
		// (long)(skillModel.getQ_cd() / speed));
		// //添加技能公共冷却
		// ManagerPool.cooldownManager.addCooldown((Player)source,
		// CooldownTypes.SKILL_TRIGGER_PUBLIC,
		// String.valueOf(skillModel.getQ_public_cd_level()),
		// (long)(skillModel.getQ_public_cd() / speed));
		// }else if(source instanceof Pet){
		// Player player = ManagerPool.playerManager.getPlayer(((Pet)
		// source).getOwnerId());
		// //开始冷却
		// double speed = 1 + ((double)source.getAttackSpeed()) / 1000;
		// //添加技能冷却
		...
		String[] buffs = skillModel.getQ_passive_buff()
				.split(Symbol.FENHAO_REG);
		
		//如果是自爆技能特殊处理
		if(skillModel.getQ_skill_type() == 5 && buffs.length >=2){
			if(target instanceof Player){
				ManagerPool.buffManager.addBuff(target, target,
						Integer.parseInt(buffs[0]), 1, value, percent);
			}else{
				ManagerPool.buffManager.addBuff(target, target,
						Integer.parseInt(buffs[1]), 1, value, percent);
			}
			return 1;
		}
		...
		for (int i = 0; i < buffs.length; i++) {
			// 获取BUFF模型
			Q_buffBean buffModel = ManagerPool.dataManager.q_buffContainer
					.getMap().get(Integer.parseInt(buffs[i]));
			if (buffModel == null)
				continue;
			...
			if (buffModel.getQ_target() == 4) {
				Player player = ManagerPool.playerManager.getPlayer(source
						.getId());
				if (player != null) {
					// 角色有组队 给自己和队员加buff
			...
			for (int j = 0; j < adders.size(); j++) {
				ManagerPool.buffManager.addBuff(adders.get(j), target,
						Integer.parseInt(buffs[i]), time, value, percent);
			}
		}
	...
	public boolean canTrigger(Fighter source, Fighter target, Skill skill) {
		...
		ITriggerSkillScript script = (ITriggerSkillScript) ManagerPool.scriptManager
				.getScript(ScriptEnum.SKILL_TRIGGER);
		if (script != null) {
			try {
	...
	public Q_skill_realmBean getSkillRealm(Player player) {
		...
		List<Q_skill_realmBean> realms = ManagerPool.dataManager.q_skill_realmContainer
				.getList();
		for (int i = realms.size() - 1; i > -1; i--) {
			Q_skill_realmBean realm = realms.get(i);
			if (level >= realm.getQ_arrive_levelsum()
					&& player.getLevel() >= realm.getQ_show_needgrade())
				return realm;
		}
	...
	public void loginCheck(Player player) {
		//发送所有技能信息
		sendSkillInfos(player);
		//重新计算所有技能战斗力总和
		int skillFightPower = 0;
		for(Skill skill:player.getSkills()) {
			...
			if(skillModel != null)
				skillFightPower += skillModel.getQ_fight_bonus();
		}
		player.setSkillFightPower(skillFightPower);
	}
	/**
	 * 发送玩家全部技能信息
	 * 
	 ...
	 * @param skill
	 */
	public void setSkillLevelInfo(Player player, List<SkillLevelInfo> list,
			Skill skill) {
		Iterator<Entry<Integer, PlayerAttribute>> iter = player.getAttributes()
				.entrySet().iterator();
		while (iter.hasNext()) {
			...
			if (playerAttribute.getSkillLevelUp().size() > 0) {
				int level = 0;
				if (playerAttribute.getSkillLevelUp().containsKey(-1))
					level = playerAttribute.getSkillLevelUp().get(-1);
				if (playerAttribute.getSkillLevelUp().containsKey(
						skill.getSkillModelId()))
					level += playerAttribute.getSkillLevelUp().get(
							skill.getSkillModelId());
				if (level != 0) {
	...
	public void autoStudySkill(Player role) {
		int key = ManagerPool.dataManager.q_characterContainer.getKey(role.getJob(), role.getLevel());
		...
		if (q_skill != 0) {
			Q_skill_modelBean skillModel = ManagerPool.dataManager.q_skill_modelContainer
					.getMap().get(q_skill + "_" + 1);
			if (skillModel != null
					&& !SkillManager.getInstance().isHaveSkill(role, q_skill)) {
	...
	public boolean removeSkill(Player player, int skillModelId) {
		...
		while (iter.hasNext()) {
			...
			if (skill.getSkillModelId() == skillModelId) {
				...
				ShortCutManager.getInstance().removeShortCut(player, skillModelId);
				Q_skill_modelBean skillModel = ManagerPool.dataManager.q_skill_modelContainer.getMap().get(skillModelId + "_" + skill.getRealLevel(player));
				player.setSkillFightPower(player.getSkillFightPower() - skillModel.getQ_fight_bonus());
				return true;
			}
		}
