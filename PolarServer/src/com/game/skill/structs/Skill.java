...
public class Skill extends GameObject {
	 ...
	 * 加成过的等级
	 */
	private transient int realLevel; //transient关键字指定其不被序列化
	...
	public int getRealLevel(Player player) {
		...
		Q_skill_modelBean model = DataManager.getInstance().q_skill_modelContainer
				.getMap().get(
						getSkillModelId() + "_" + (getSkillLevel() + addlevel));
		if (model != null) {
			...
			// 加成过的技能没有等级判断
			if (addlevel != 0) {
				logger.error("全部加成等级" + allAdd + "当前技能加成" + currentAdd
						+ "找不到等级模型 对应的技能:" + skillModelId);
			}
			return skillLevel;
		}
	}
