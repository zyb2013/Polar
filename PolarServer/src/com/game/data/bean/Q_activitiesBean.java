...
public class Q_activitiesBean {
	...
	//活动奖励类型（1为每天展示奖励(领完之后消失)，2为展示全部奖励，前端使用）
	private int q_show;
	...
	//需要充值钻石数([0(0 有没有充值 1 每日的充值 2 一段时间的充值 3 总充值 4 排行榜),1(充值 0 大于后面一个数字就可以 1 根据充值数字和后面数字取次数， 排行 排行类型), 3(充值 得到奖励的条件数字 ， 排行 名次)， 1212301059开始时间，1212311059结束时间])
	...
	/**
	 * get 需要充值钻石数([0(0 有没有充值 1 每日的充值 2 一段时间的充值 3 总充值 4 排行榜),1(充值 0 大于后面一个数字就可以 1 根据充值数字和后面数字取次数， 排行 排行类型), 3(充值 得到奖励的条件数字 ， 排行 名次)， 1212301059开始时间，1212311059结束时间])
	 * @return 
	 */
	...
	/**
	 * set 需要充值钻石数([0(0 有没有充值 1 每日的充值 2 一段时间的充值 3 总充值 4 排行榜),1(充值 0 大于后面一个数字就可以 1 根据充值数字和后面数字取次数， 排行 排行类型), 3(充值 得到奖励的条件数字 ， 排行 名次)， 1212301059开始时间，1212311059结束时间])
	 */
