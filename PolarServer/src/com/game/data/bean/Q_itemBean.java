...
public class Q_itemBean {
	...
	// 佩戴部位// Q_kind 1是主手武器，2是副手武器盾，盾固定放在副手栏，其他单手武器(二级类型填1)没限制，双手武器(二级类型填2)占两栏
	private int q_kind;
	...
	// 23分配点增加 24全部点数清洗 ，101=钻石，102=绑钻，103=金币 格式示例[1,2,4]
...
//	private int q_drop_animation;//物品掉落动画资源编号
	...
	private String q_job_limit;// 使用职业需求,三位数 0通用，100是只有战士能用 110是战士法师都能用
							// 111是战士法师弓箭手都能用 011（11）法师弓箭手能用。100,100,000
	...
	private int q_third_type;// 三级分类：装备：1法杖，2盾牌，3弓，4弩
	...
	private int q_equip_steplv;// 品阶
	
	private int q_use_price;//使用时要扣除的钻石
	
	

	public int getQ_use_price() {
		return q_use_price;
	}

	public void setQ_use_price(int q_use_price) {
		this.q_use_price = q_use_price;
	}
	...
	public String parseEffict_value() {
		if(StringUtil.isBlank(q_effict_value)) {
			return q_effict_value;
		}
		JSONArray array = JSONArray.fromObject(q_effict_value);
		for(int i=0; i<array.size(); i++) {
			...
			if(str.contains("-")) {
				...
				int random = RandomUtils.random(Integer.parseInt(split[0]), Integer.parseInt(split[1]));
				array.set(i, random);
			}
		}
	...
	/**
	 * get 怪物爆出件数上限阀值（填0为不限制）（格式：件数/小时）例如：2/3 是指：每3小时最多爆出2件（不能超过24）
	 * @return 
	 */
	...
	/**
	 * set 怪物爆出件数上限阀值（填0为不限制）（格式：件数/小时）例如：2/3 是指：每3小时最多爆出2件(不能超过24）
	 */
	...
	public int getQ_equip_steplv() {
		return q_equip_steplv;
	}

	public void setQ_equip_steplv(int q_equip_steplv) {
		this.q_equip_steplv = q_equip_steplv;
	}
}