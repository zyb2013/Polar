...
public class Q_buffBean {
	...
	//替换层级，当层级为0的时候，效果叠加与效果替换只对同ID的BUFF有效，当层级大于0的时候，效果叠加与效果替换对相同层级的BUFF有效
	...
	//BUFF参数，技能ID、技能ID，【攻击：X,防御:X,闪避:X,暴击:X,幸运:X,生命上限:X,魔法值上限:X,体力上限:X,攻击速度:X,移动速度:X,攻击比例：万分比,经验比例：X，真气比例：X。】
	...
	//变身技能(id 为对应变身的monster模板ID),没有变身默认为0
	private int q_shapechange_monsterid;
	
	//变身后默认携带的技能，格式为1001|1002
	private String q_shapechange_skill;
	...
	/**
	 * get 损益类型（0无所谓，2正面BUFF，1负面BUFF）注意
	 * @return 
	 */
	...
	/**
	 * get BUFF参数，技能ID、技能ID，【攻击：X,防御:X,闪避:X,暴击:X,幸运:X,生命上限:X,魔法值上限:X,体力上限:X,攻击速度:X,移动速度:X,攻击比例：万分比,经验比例：X，真气比例：X。】
	 * @return 
	 */
	...
	/**
	 * set BUFF参数，技能ID、技能ID，【攻击：X,防御:X,闪避:X,暴击:X,幸运:X,生命上限:X,魔法值上限:X,体力上限:X,攻击速度:X,移动速度:X,攻击比例：万分比,经验比例：X，真气比例：X。】
	 */
	...
	public int getQ_shapechange_monsterid() {
		return q_shapechange_monsterid;
	}

	public void setQ_shapechange_monsterid(int q_shapechange_monsterid) {
		this.q_shapechange_monsterid = q_shapechange_monsterid;
	}

	public String getQ_shapechange_skill() {
		return q_shapechange_skill;
	}

	public void setQ_shapechange_skill(String q_shapechange_skill) {
		this.q_shapechange_skill = q_shapechange_skill;
	}
