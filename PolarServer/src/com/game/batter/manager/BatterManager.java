...
public class BatterManager {
	...
	public void setBatter(List<Hatred> hatreds, Monster monster, Fighter attacter){
		...
		}else if(attacter != null && attacter instanceof SummonPet) {	//击杀者如果是召唤怪，那就获取主人
			killer=PlayerManager.getInstance().getPlayer(((SummonPet)attacter).getOwnerId());
		}else if(attacter != null && attacter instanceof Player) {
...
//		}
		if (killer != null ) {
			if(Math.abs(attacter.getLevel() - monster.getLevel()) < 150){
	...
	public void addBatter(int montype, Player killer, Monster monster){
		if (montype == 1) {
			...
			if (olddb != null) {
				int s = olddb.getQ_countdown();
				if (killer.getEvencutnum() > 10000) {
					s =  200;	//大于10000后设置为200毫秒
				}

				if(killer.getEvencuttime() + s > System.currentTimeMillis()){//累加
					int num = killer.getEvencutnum()+1;
					if(num > killer.getEvencutnum()){
...
//					else {
					...
					killer.setEvencutnum(num);
					//最大连斩数超过历史最大连斩数
					if(killer.getEvencutnum() > killer.getMaxEventcut() && killer.getLevel() >= Global.SYNC_PLAYER_LEVEL && killer.getEvencutnum() >= Global.SYNC_EVENT_CUT){
						killer.setMaxEventcut(killer.getEvencutnum());
...
//					}
	...
	public Q_evencutBean  getEvencutDB(Player player) {
		...
		Q_evencutBean q_evencutBeanLast = dblist.get(0);
		for (Q_evencutBean q_evencutBean : dblist) {
			if (player.getEvencutnum() < q_evencutBean.getQ_evencut_num()) {
				return q_evencutBeanLast;
			}
			q_evencutBeanLast = q_evencutBean;
		}
		return q_evencutBeanLast;
	}
	
	public int getMonsterExpAddition(Player player){
		Q_evencutBean db = getEvencutDB(player);
		
		int s = db.getQ_countdown();
		if (player.getEvencutnum() > 10000) {
			s =  200;	//大于10000后设置为200毫秒
		}
		
		if(player.getEvencuttime() + s > System.currentTimeMillis()){
			return db.getQ_exp();
		}
	...
	public void bossBatter(Player player ){
		...
		int atk = ManagerPool.dataManager.q_characterContainer.getMap().get(key).getQ_batter_atk();
		cmsg.setId(player.getBossbatternum()*atk);
