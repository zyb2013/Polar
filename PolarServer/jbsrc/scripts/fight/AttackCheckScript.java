...
//			}
		...
		/*panic god 屏蔽
			//---------------------------战盟领地旗帜------和（MonsterCanBeAttackScript脚本一致）------------
			if (ManagerPool.guildFlagManager.getflagmonidlist().contains(monster.getModelId())) {//检测是否对应怪物
				...
				if (player != null) {
					if (player.getLevel() < 30) {
						MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("您的等级低于30级，不能攻击盟旗。"));
						return false;
					}
					...
					if (monster.getParameters().containsKey("guildid")) {
						...
						if (player.getGuildId() == guildid) {
							MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("不能攻击本盟盟旗。"));
							return false;
						}
					}
					
					if(ManagerPool.guildFlagManager.getFlagwarstatus() != 1 ){
						MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("现在不是领地争夺战时间，不能攻击盟旗。"));
						return false;
					}
					
					if (player.getLine() != 1) {
						MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("请到1线去攻击盟旗真身。"));
						return false;
					}
				}
			}
