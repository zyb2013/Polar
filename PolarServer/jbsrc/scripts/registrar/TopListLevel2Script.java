...
public class TopListLevel2Script implements IRegistrar {
	...
	public void giveCommonReward(Player player) { 
		if(!BackpackManager.getInstance().changeBindGold(player, 5000, Reasons.ACTIVITY_GIFT, Config.getId())){
			String title="等级排行榜第二名活动奖励";
			String content="等级排行榜第二名活动奖励";
			List<Item> items = Item.createItems(-5, 5000, true, 0);
			if(!MailServerManager.getInstance().sendSystemMail(Long.valueOf(player.getId()), player.getName(), title, content, (byte)0, 0, items)){
				faillog.info("[Player"+player.getName()+"|"+player.getId()+"]"+"等级排行榜第二名发奖励发邮件失败"+JSONserializable.toString(items));
			}
		}
	}
