...
public class BiWuDaoFlagPluck implements INpcDefaultActionScript,INpcGatherActionScript{
	...
	public void defaultAction(Player player, NPC npc){
		...
		if (player.getGuildId() > 0) {
			...
			MessageUtil.notify_player(player, Notifys.MOUSEPOS, ResManager.getInstance().getString("没有加入战盟，不能夺旗"));
		}
	}
	...
	public void gather(Player player, NPC npc){
		...
		if (player.getGuildId() >0) {
			...
			parseUtil.setValue(String.format(ResManager.getInstance().getString("【%s】的【%s】在比武岛占领战旗获得10点军功奖励，该战盟成员获得真气、经验收益翻倍！{@}"), gname, player.getName()),
					new ParseUtil.VipParm(VipManager.getInstance().getVIPLevel(player), GuideType.BIWUDAO_MOVE.getValue()));
			...
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("只有战盟成员才能夺旗"));
		}
		//player停止采集
		ManagerPool.npcManager.playerStopGather(player, true);
	}
