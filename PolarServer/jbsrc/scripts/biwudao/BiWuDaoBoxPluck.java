...
public class BiWuDaoBoxPluck implements INpcDefaultActionScript,INpcGatherActionScript{
	...
	public BiWuDaoBoxPluck(){
		...
		boxrewardmap.put(40002, "-4_60");	//经验
		boxrewardmap.put(40003, "-5_100");	//绑钻
		boxrewardmap.put(40004, "-3_60");	//真气
		boxrewardmap.put(40005, "30205_1;1011_2;1007_30;3019_2;30301_2;1001_5;1009_1");	//勇者
	}
	...
	public void defaultAction(Player player, NPC npc){
		...
		if (npc.getModelId() == YONGZHEBAOXIANGID) {
			if (player.getGuildId() > 0 && ManagerPool.biWuDaoManager.getBiwudaoguildid() == player.getGuildId()) {
				...
				/* xuliang
				MessageUtil.notify_player(player, Notifys.MOUSEPOS, ResManager.getInstance().getString("只有比武岛占领者战盟成员才能开启{1}"),npcdata.getQ_name());
				*/
				return;
			}
		}else {
	...
	public void gather(Player player, NPC npc){
		...
		if (npcid == YONGZHEBAOXIANGID) {
			if (player.getGuildId() > 0 && ManagerPool.biWuDaoManager.getBiwudaoguildid() == player.getGuildId()) {
				...
				/* xuliang
				MessageUtil.notify_player(player, Notifys.MOUSEPOS, ResManager.getInstance().getString("只有比武岛占领者战盟成员才能开启本宝箱"));
				*/
				ManagerPool.npcManager.playerStopGather(player, true);
				return;
			}
	...
	public void giveboxreward(Player player,int npcid ){
		...
		//-1金币，-2钻石，-3真气，-4经验，-5绑钻，-6战魂，-7军功
		if (rewdb[0] == -1) {
			ManagerPool.backpackManager.changeMoney(player, rewdb[1], Reasons.BIWUDAO, action);
			str = str + ResManager.getInstance().getString(" 金币:") + rewdb[1];
		} else if (rewdb[0] == -2) {
			//钻石为特殊，暂不加
		} else if (rewdb[0] == -3) {
			...
			str = str + ResManager.getInstance().getString(" 绑钻:") + rewdb[1];
		} else if (rewdb[0] == -6) {
		...
		parseUtil.setValue(String.format(ResManager.getInstance().getString("【%s】在决战比武岛活动中开启【%s】获得%s {@}"), player.getName(), npcdata.getQ_name(), str), new ParseUtil.VipParm(
		...
		*/
		MessageUtil.notify_player(player, Notifys.SUCCESS, ResManager.getInstance().getString("开启{1}获得了{2}"),npcdata.getQ_name(),str );
	}
