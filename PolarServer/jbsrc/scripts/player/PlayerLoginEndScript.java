...
=======
	...
	//   奖励内容（数值 ）-1金币，-2钻石，-3真气，-4经验，-5绑钻，-6战魂，-7军功，大于0的ID是物品，现在不能支持复杂物品配置	
	public String[][] BUCHANG_TAB = {//补偿 区ID是计算的服务器区id
				{"hgpupugame_1","2013-3-26 00:01:00","2013-3-28 23:59:59","1022_1"},
	};
								...
								if (!createItems.isEmpty()) {
									if (ManagerPool.backpackManager.getEmptyGridNum(player) >= createItems.size()) {
										if (!BackpackManager.getInstance().addItems(player, createItems, Reasons.def7, action)) {
											MailServerManager.getInstance().sendSystemMail(player.getId(), player.getName(), ResManager.getInstance().getString("上线补偿"), data.length > 4 ? data[4] :  ResManager.getInstance().getString("补偿物品"), (byte) 0, 0, createItems);
										} else {
						...
						for (int[] rewdb : rewtab) {
							...
							if (rewdb[0] == -1) {
								...
								if (!createItems.isEmpty()) {
									if (ManagerPool.backpackManager.getEmptyGridNum(player) >= createItems.size()) {
										if (!BackpackManager.getInstance().addItems(player, createItems, Reasons.def7, action)) {
											MailServerManager.getInstance().sendSystemMail(player.getId(), player.getName(), ResManager.getInstance().getString("合区补偿"), data.length > 4 ? data[4] : ResManager.getInstance().getString("补偿物品"), (byte) 0, 0, createItems);
										} else {
											str = str + String.format(" %s:%d", BackpackManager.getInstance().getName(rewdb[0]), rewdb[1]);
										}
									} else {
