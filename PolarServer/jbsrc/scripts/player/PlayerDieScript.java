...
public class PlayerDieScript implements IPlayerDieScript {
	...
	public void onPlayerDie(Player player, Fighter killer) {
		...
		//赤色要塞 玩家死亡
		IPlayerDieScript script = (IPlayerDieScript) ManagerPool.scriptManager.getScript(ScriptEnum.CSYS);
		if (script != null) {
			try {
		...
		//4V4
		 script = (IPlayerDieScript) ManagerPool.scriptManager.getScript(ScriptEnum.BF4V4);
		if (script != null) {
			try {
				script.onPlayerDie(player, killer);
			} catch (Exception e) {
