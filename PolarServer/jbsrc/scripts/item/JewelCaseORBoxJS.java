 ...
 * @2012-9-24 下午3:09:13
 */
public class JewelCaseORBoxJS implements IItemScript {
...
//	数值配置需求：数值配置需要支持热加载（逻辑最好用脚本实现，不排除经常更改）
	...
	public boolean use(Item item, Player player, String... parameters) {
		...
		ScriptsUtils.callWorld(35, "queryDayRecharge", ""+1009110, "queryCallBack", ""+player.getId(),""+num,""+item.getId());
		return true;
	}
...
//		（1）在线人数控制的8888钻石奖励开出条件：服务器在线人数超过1000人时，第一个使用美人百宝袋时直接开出8888钻石奖励，在线控制的8888钻石仅此一次
//		（2）开启数量条件控制的8888钻石奖励开出规则：服务器需要记录美人百宝袋开启数量达到200个时，会放出2个8888钻石奖励。开启数量达到400个时，会放出第3个8888钻石奖励，以此类推。
//		（3）玩家获得8888钻石奖励应优先计算，若玩家获得了8888钻石奖励，则不会获得其他奖励
		//服务器级计数
		
//		美人百宝袋新增开启机制：
//		1、当日充值金额>999RMB的玩家开启美人百宝袋时，排除8888钻石奖励。
//		2、已抽取到1次8888钻石的玩家，给该玩家记录一次获奖标识，当玩家拥有该标识时，不能抽取8888钻石。15天后该标识消失。
...
//			recharge<=999&&player.getGold().getIsinner()==0&&ago>15*24*60*60*1000
...
//					&&player.getGold().getIsinner()==0
				...
				//MessageUtil.notify_All_player(Notifys.SROLL, "{1}玩家开启江山百宝袋获得8888钻石大奖",player.getName());
				//MessageUtil.notify_All_Activity(Notifys.CHAT_SYSTEM, 21, "{1}玩家开启江山百宝袋获得8888钻石大奖",player.getName());
				player.getActivitiesReward().put("mrbbdlasttime", System.currentTimeMillis()+"");
				/*xiaozhuoming: 暂时没有用到
				MessageUtil.notify_player(player, Notifys.CHAT_BULL, ResManager.getInstance().getString("恭喜您开启江山百宝袋获得8888钻石大奖"));
				ParseUtil parseUtil = new ParseUtil();
				parseUtil.setValue(String.format(ResManager.getInstance().getString("玩家【%s】开启【江山百宝袋】获得8888钻石大奖，真是羡煞旁人！{@}"), player.getName()), new ParseUtil.VipParm(VipManager
						.getInstance().getVIPLevel(player), GuideType.BAIBAODAI.getValue()));
