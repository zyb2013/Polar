...
//				}
		...
		if (id == -1) {
			...
			MessageUtil.notify_player(player, Notifys.SUCCESS,ResManager.getInstance().getString("恭喜您打开宝盒获得了：金币{1}"),num+"");
			MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM,ResManager.getInstance().getString("恭喜您打开宝盒获得了：金币{1}"),num+"");
			return true;
		}else if ( id == -3) {
