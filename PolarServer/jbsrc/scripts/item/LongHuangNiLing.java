	...
	public boolean use(Item item, Player player, String... parameters) {
		if (item.getItemModelId() == 9123) {
			...
			if (ManagerPool.backpackManager.removeItem(player, item, 1,Reasons.LONGHUANGNILING, Config.getId())) {
				...
				parseUtil.setValue(String.format(ResManager.getInstance().getString("玩家【%s】使用了【%s】获得坐骑升阶成功率提升的效果！{@}"), player.getName(), name), new ParseUtil.VipParm(VipManager
						.getInstance().getVIPLevel(player), GuideType.LONGHUANG.getValue()));
