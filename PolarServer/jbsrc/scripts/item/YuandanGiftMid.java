...
public class YuandanGiftMid implements IItemScript {
	...
	public boolean use(Item item, Player player, String... parameters) {
		...
		list.add("btn2#" + this.getId() + "#open#"+item.getId());
	...
	public void open(List<Object> para){
		...
		//花费500钻石开启可获得：5倍*3  战魂2000  当前等级紫装强化5-7随便一件  坐骑进阶丹10个 元旦礼包（大）
		String itemstr = "1024,3,0,1;-6,2000;1011,10,0,1;9151,1,0,1";
		//创建奖励物品
		List<Item> items = new ArrayList<Item>();
		int num = ManagerPool.backpackManager.createItems(player, itemstr, items);
		Item equipItem = getRandEquip(player, (int) (Math.random() * 2 + 5));
	...
	public Item getRandEquip(Player player, int strengthenLevel) {
		...
		while (i.hasNext()) {
			...
			if (item.getQ_kind() > 10 || item.getQ_kind() < 1) continue;
