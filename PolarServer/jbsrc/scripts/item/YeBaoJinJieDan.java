 ...
 * @2012-9-24 下午1:05:02
 */
public class YeBaoJinJieDan implements IItemScript {
	...
	public boolean use(Item item, Player player, String... parameters) {
		...
		if(horse.getLayer() == horse_layer -1){
			...
			MessageUtil.notify_player(player, Notifys.SROLL, "恭喜玩家{1}使用瑞角夜豹幻化丹将坐骑直接提升至瑞角夜豹", player.getName());
			
			BackpackManager.getInstance().removeItem(player, item, 1, Reasons.GOODUSE, id);
			return true;
		}else{
			MessageUtil.notify_player(player, Notifys.ERROR,"您的座骑没有达到洪荒剑齿，不能使用该物品");
			return false;
		}
		*/
	}
