		...
		if (type > 0) {//对宝石进行提升
			...
			if (newname.length() > 1) {
				...
				*/
				...
				/* xuliang
				parseUtil.setValue(String.format(ResManager.getInstance().getString("玩家【%s】使用【%s】使得%d颗%s属性加倍,战斗力提升%d!{@}"), player.getName(), itemname, num, oldname, fightpower),
						new ParseUtil.VipParm(VipManager.getInstance().getVIPLevel(player), GuideType.GEM_QH.getValue()));
