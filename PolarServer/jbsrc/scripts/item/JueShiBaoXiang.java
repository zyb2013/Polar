	...
	public boolean use(Item item, Player player, String... parameters) {
		...
		if (ManagerPool.backpackManager.removeItem(player, item, 1, Reasons.def18, Config.getId())) {
			...
			if (itemdata[4] > 0) {
				...
				parseUtil.setValue(String.format(ResManager.getInstance().getString("【%s】开启【%s】，获得:%s(%d)!{@}"), player.getName(), itemname, rewname, itemdata[1]),
						new ParseUtil.VipParm(VipManager.getInstance().getVIPLevel(player), 35));
