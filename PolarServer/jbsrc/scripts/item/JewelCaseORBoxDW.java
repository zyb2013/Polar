...
//					&&player.getGold().getIsinner()==0
				...
				//MessageUtil.notify_All_player(Notifys.SROLL, "{1}玩家开启帝王百宝袋获得8888钻石大奖",player.getName());
				//MessageUtil.notify_All_Activity(Notifys.CHAT_SYSTEM, 21, "{1}玩家开启帝王百宝袋获得8888钻石大奖",player.getName());
				player.getActivitiesReward().put("mrbbdlasttime", System.currentTimeMillis()+"");
				/*xiaozhuoming: 暂时没有用到
				MessageUtil.notify_player(player, Notifys.CHAT_BULL, ResManager.getInstance().getString("恭喜您开启帝王百宝袋获得8888钻石大奖"));
				ParseUtil parseUtil = new ParseUtil();
				parseUtil.setValue(String.format(ResManager.getInstance().getString("玩家【%s】开启【帝王百宝袋】获得8888钻石大奖，真是羡煞旁人！{@}"), player.getName()), new ParseUtil.VipParm(VipManager
