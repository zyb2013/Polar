		...
		for (int i = 0; i < num; i++) {
			if(ManagerPool.backpackManager.removeItem(player, item, 1, Reasons.def20,aid)){
				...
				if (opennum > 150 && RandomUtils.random(10000) <= 500) {
					...
					parseUtil.setValue(String.format(ResManager.getInstance().getString("【%s】开启【%s】，获得:%s!{@}"), player.getName(), name, addname), new ParseUtil.VipParm(VipManager
							.getInstance().getVIPLevel(player), 37));
				...
				//玩家使用该宝箱数量大于15个后，有20%几率开出。当开出一个后，将不会再开出。		
					...
					if (RandomUtils.random(10000) <= 2000) {
						...
						ParseUtil parseUtil = new ParseUtil();
						parseUtil.setValue(String.format(ResManager.getInstance().getString("【%s】开启【%s】，获得:%s!{@}"), player.getName(), name, addname), new ParseUtil.VipParm(
								VipManager.getInstance().getVIPLevel(player), 37));
						MessageUtil.notify_All_player(Notifys.CHAT_BULL, parseUtil.toString(),new ArrayList<GoodsInfoRes>(),37);
					}
				}
