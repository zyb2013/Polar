 ...
 * @2012-9-24 下午3:09:13
 */
public class JewelCaseORBox implements IItemScript {
...
//	数值配置需求：数值配置需要支持热加载（逻辑最好用脚本实现，不排除经常更改）
	...
	public boolean use(Item item, Player player, String... parameters) {
		...
		ScriptsUtils.callWorld(35, "queryDayRecharge", ""+1009103, "queryCallBack", ""+player.getId(),""+num,""+item.getId());
		return true;
	}
...
//					player.getGold().getIsinner()==0&&
				...
				//MessageUtil.notify_All_player(Notifys.SROLL, "{1}玩家开启美人百宝袋获得8888钻石大奖",player.getName());
				//MessageUtil.notify_All_Activity(Notifys.CHAT_SYSTEM, 21, "{1}玩家开启美人百宝袋获得8888钻石大奖",player.getName());
				player.getActivitiesReward().put("mrbbdlasttime", System.currentTimeMillis()+"");
				/*xiaozhuoming: 暂时没有用到
				MessageUtil.notify_player(player, Notifys.CHAT_BULL, ResManager.getInstance().getString("恭喜您开启美人百宝袋获得8888钻石大奖"));
				ParseUtil parseUtil = new ParseUtil();
				parseUtil.setValue(String.format(ResManager.getInstance().getString("玩家【%s】开启【美人百宝袋】获得8888钻石大奖，真是羡煞旁人！{@}"), player.getName()), new ParseUtil.VipParm(VipManager
