...
//		//多人副本使用钻石一键扫荡，判断等级
//		if (handler instanceof ReqZoneImmediatelyByCostHandler) {
//			ReqZoneImmediatelyByCostMessage smsg = (ReqZoneImmediatelyByCostMessage)handler.getMessage();
//			Q_clone_activityBean zoneBean = ManagerPool.zonesManager.getContainer(smsg.getZoneid());
...
//		}
	...
	public void levelupHorseWeapon(Player player, byte auto){
		...
		if(player.getMoney() < weaponBean.getQ_need_money()){
			MessageUtil.notify_player(player, Notifys.ERROR, ResManager.getInstance().getString("您金币不足。"));
			sendFailedError(player);
			return;
		}
			...
			try{
				...
				if(tobean!=null){
					if( tobean.getQ_is_announce()==1 ){ //是否需要发公告
						...
						parseUtil.setValue(String.format(ResManager.getInstance().getString("恭喜【%s】将【%s】升阶至【%s】战斗力提升%d！{@}"), player.getName(), fromweapon, toweapon, upvalue),
								new ParseUtil.VipParm(VipManager.getInstance().getVIPLevel(player), GuideType.HORSEWEAPON_UP.getValue()));
...
//				}
				...
				if (player.getLevel() >= breakdata.getQ_break_level() ) {
					if (ManagerPool.realmManager.checkTakeMaterial(player,breakdata.getQ_break_item(),msg.getType())) {
						...
						ParseUtil parseUtil = new ParseUtil();
						if (newfightpower > 0) {
							parseUtil.setValue(String.format(ResManager.getInstance().getString("【%s】成功激活境界，提升%d战斗力{@}"), player.getName(), newfightpower), new ParseUtil.VipParm(
									VipManager.getInstance().getVIPLevel(player), GuideType.REALM.getValue()));
						}else {
							parseUtil.setValue(String.format(ResManager.getInstance().getString("【%s】成功激活境界！{@}"), player.getName(), newfightpower), new ParseUtil.VipParm(
									VipManager.getInstance().getVIPLevel(player), GuideType.REALM.getValue()));
						}
				...
				if (player.getMoney() < breakdata.getQ_break_money()) {
					/*xiaozhuoming: 暂时没有用到
					MessageUtil.notify_player(player, Notifys.MOUSEPOS, ResManager.getInstance().getString("需要{1}金币,才可继续突破境界"),breakdata.getQ_break_money()+"");
					*/
						...
						if (result == 1) {
							...
							if (breakdata.getQ_isbulletin() == 1) {
								...
								if (newfightpower > 0) {
									...
									parseUtil.setValue(String.format(ResManager.getInstance().getString("恭喜玩家【%s】成功突破境界至：【%s】,战斗力提升%d!{@}"), player.getName(), basicup.getQ_name(),
											newfightpower), new ParseUtil.VipParm(VipManager.getInstance().getVIPLevel(player), GuideType.REALM_TUPO.getValue()));
								}else {
									...
									parseUtil.setValue(String.format(ResManager.getInstance().getString("恭喜玩家【%s】成功突破境界至：【%s】{@}"), player.getName(), basicup.getQ_name()),
											new ParseUtil.VipParm(VipManager.getInstance().getVIPLevel(player), GuideType.REALM_TUPO.getValue()));
								}
...
//							String[] items = breakdata.getQ_break_item().split(Symbol.XIAHUAXIAN_REG);
							...
							if (wmitemnum > 0) {
								...
								MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM, ResManager.getInstance().getString("突破境界成功，使用：[{1}]{2}个，[金币]{3}。"),name,"1",breakdata.getQ_break_money()+"");
								*/
							}else {
								/*xiaozhuoming: 暂时没有用到
								String name = ManagerPool.backpackManager.getName(Integer.parseInt(items[0]));
								MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM, ResManager.getInstance().getString("突破境界成功，使用：[{1}]{2}个，[金币]{3}。"),name,items[1],breakdata.getQ_break_money()+"");
								*/
							}
							...
							//MessageUtil.notify_player(player, Notifys.MOUSEPOS, ResManager.getInstance().getString("突破境界失败了，使用：[{1}]{2}个，[金币]{3}。"),name,items[1],breakdata.getQ_break_money()+"");
							if (wmitemnum > 0) {
								String name = ManagerPool.backpackManager.getName(wmitemid);
								cmsg.setPrompt(String.format(ResManager.getInstance().getString("突破境界失败了，使用：[%s]%s个，[金币]%d。"),name,"1",breakdata.getQ_break_money()));
							}else {
								String[] items = breakdata.getQ_break_item().split(Symbol.XIAHUAXIAN_REG);
								String name = ManagerPool.backpackManager.getName(Integer.parseInt(items[0]));
								cmsg.setPrompt(String.format(ResManager.getInstance().getString("突破境界失败了，使用：[%s]%s个，[金币]%d。"),name,items[1],breakdata.getQ_break_money()));
							}
						}
