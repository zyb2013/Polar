...
public class BETASumRecharge2000 implements IActivityScript {
	...
	public void getReward(Player player) {
		...
		if(mailitems.size()>0){
			String title="累积充值2000钻石奖励";
			String content="累积充值2000钻石奖励";
			if(!MailServerManager.getInstance().sendSystemMail(player.getId(), player.getName(), title, content, (byte)0, 0, mailitems)){
				faillog.info("[Player"+player.getName()+"|"+player.getId()+"]"+"累积充值2000钻石发奖励失败发邮件失败"+JSONserializable.toString(mailitems));
			}
		}
	}
