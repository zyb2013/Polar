...
//国庆 坐骑升阶
public class GQHORSE10 implements IActivityScript {
	private int itemid1=1024;
	private int itemnum1=2;
	
	private int itemid2=1001;
	private int itemnum2=50;

	private int itemid3=1007;
	private int itemnum3=100;
	
	private int itemid4=30201;
	private int itemnum4=50;
	
	
	private static String KEY = "GQHORSE10";
	@Override
	public int getId() {
		return ScriptEnum.GQHORSE10;
	}
	...
	public void getReward(Player player) {
		...
		//获得奖励
		if(ManagerPool.backpackManager.getEmptyGridNum(player) < 4){
			...
			items = Item.createItems(itemid2, itemnum2, true, 0);
			BackpackManager.getInstance().addItems(player, items, Reasons.ACTIVITY_GIFT, actionId);
			items = Item.createItems(itemid3, itemnum3, true, 0);
			BackpackManager.getInstance().addItems(player, items, Reasons.ACTIVITY_GIFT, actionId);
			items = Item.createItems(itemid4, itemnum4, true,time);
			BackpackManager.getInstance().addItems(player, items, Reasons.ACTIVITY_GIFT, actionId);
			/*xiaozhouming: 暂时没有用到
			...
			String name4=ManagerPool.dataManager.q_itemContainer.getMap().get(itemid4).getQ_name();
			MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM, "恭喜您获得奖励：{1}({2}),{3}({4}),{5}({6}),{7}({8})", name1,itemnum1+"", name2,itemnum2+"", name3,itemnum3+"",name4,itemnum4+"");
			*/
		}
	}
	...
	public String getRewardInfo(Player player) {
		//五倍收益丹*2、装备强化石*50、宝石精华*100、真气丹（绑定）*50
	...
	public int getState(Player player) {
		if(player.getActivitiesReward().containsKey(KEY)){ //未领过
			...
			if(playerlayer>=10){ //没有活动期间升阶的标记 并且当前坐骑等级大于等于领取条件 表示玩家在活动之前达到目标等级，直接跳过
				return 0;
			}
		}
		return 2;
