...
public class BETASumRecharge1000 implements IActivityScript {
	...
	public void getReward(Player player) {
		...
		if(mailitems.size()>0){
			String title = "累积充值1000钻石活动奖励";
			String content = "累积充值1000钻石活动奖励";
			if(!MailServerManager.getInstance().sendSystemMail(player.getId(), player.getName(), title, content, (byte)0, 0, mailitems)){
				faillog.info("[Player"+player.getId()+"]"+"累积充值1000钻石奖励发送失败发送邮件失败"+JSONserializable.toString(mailitems));
			}
		}
	...
	public String getDescribe(Player player) {
		return "累积充值1000钻石可领取以下奖励";
	}

	@Override
	public String getRewardInfo(Player player) {  //绑钻3000  连斩延时丹2 红玫瑰10 双倍收益丹5
