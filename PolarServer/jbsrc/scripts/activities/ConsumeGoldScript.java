...
public class ConsumeGoldScript implements IConsumeGoldScript{
	...
	public boolean consumegold(Player player, int num, Reasons reason,long action) {
		if (num < 0 ) {
			...
			if ( System.currentTimeMillis() >= startdate.getTime() && System.currentTimeMillis() < enddate.getTime()) {
				...
				/*xiaozhuoming: 暂时没有用到
				MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM, ResManager.getInstance().getString("聚宝盆活动：消费{1}钻石，获得返还{2}绑钻"),gold+"", (int)(lijing) +"");
				*/
			}
		}
		return false;
	...
	public void loginRetbindgold(List<Object> plist){
		...
		}else {//如果活动已经结束，而且可领取绑钻大于1，展示  可领取按钮
	...
	public void receiveretbindgold(List<Object> list){
		...
		if ( System.currentTimeMillis() >= startdate.getTime() && System.currentTimeMillis() < enddate.getTime()) {
			...
			if (player.getRetcurrbindgold() >= 1) {
				...
				if (day != player.getRetbindgoldday()) {
					if(player.getRetcurrbindgold() >= ling ){
						...
						MessageUtil.notify_player(player, Notifys.CHAT_ROLE, ResManager.getInstance().getString("获得{1}绑钻"),ling+"");
					}else {
						ling = (int) player.getRetcurrbindgold();
						player.setRetcurrbindgold(0);
						ManagerPool.backpackManager.changeBindGold(player, ling, Reasons.def21,  Config.getId());
						MessageUtil.notify_player(player, Notifys.CHAT_ROLE, ResManager.getInstance().getString("获得{1}绑钻，(已全部领取)"),ling+"");
					...
					/*xiaozhuoming: 暂时没有用到
					MessageUtil.notify_player(player, Notifys.MOUSEPOS, ResManager.getInstance().getString("每天只能领取一次，绑钻总额的10%，明天再来吧"),ling+"");
					*/
				}
			...
			/*xiaozhuoming: 暂时没有用到
			MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM, ResManager.getInstance().getString("没有可领取的绑钻了"));
			*/
		}
	}
