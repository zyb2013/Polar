...
//国庆 坐骑升阶
public class GQHORSE8 implements IActivityScript {
	...
	private int itemnum3=30;
	
	private static String KEY = "GQHORSE8";
	@Override
	public int getId() {
		return ScriptEnum.GQHORSE8;
	}
	...
	public String getRewardInfo(Player player) {
		//坐骑祝福丹（绑定）*1、坐骑进阶丹（绑定）*50、真气丹（绑定）*30
	...
	public int getState(Player player) {
		if(player.getActivitiesReward().containsKey(KEY)){ //未领过
			...
			if(playerlayer>=8){ //没有活动期间升阶的标记 并且当前坐骑等级大于等于领取条件 表示玩家在活动之前达到目标等级，直接跳过
				return 0;
			}
		}
		return 2;
