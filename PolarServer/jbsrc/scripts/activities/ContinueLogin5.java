...
public class ContinueLogin5 implements IActivityScript{

	private static String KEY = "CONTINUELOGIN5";
	...
	public void getReward(Player player) {
		...
		if(BackpackManager.getInstance().getEmptyGridNum(player)>=3){
			...
			BackpackManager.getInstance().changeBindGold(player, 50, Reasons.ACTIVITY_GIFT, actionId);
	...
	@Override //红玫瑰（绑）*3、双倍收益丹（绑）*1、绑钻*50、回城卷（绑）*6、   真气丹(绑）*1
	public String getDescribe(Player player) {
		return "连续5天登录游戏可领取二测绑钻奖励：10000,（若中断，登录天数将重置为1）";
	}

	@Override
	public String getRewardInfo(Player player) {
		return "[{\"id\":-5, \"num\":" + 50 + "}, {\"id\":1015, \"num\":" + 1 + "},{\"id\":1100, \"num\":"+3+"},{\"id\":9001, \"num\":"+5+"},{\"id\":30201, \"num\":"+1+"}]";
	}
