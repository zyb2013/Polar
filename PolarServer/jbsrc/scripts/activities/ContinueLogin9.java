...
public class ContinueLogin9 implements IActivityScript{

	private static String KEY = "CONTINUELOGIN9";
	...
	public void getReward(Player player) {
		...
		if(BackpackManager.getInstance().getEmptyGridNum(player)>=3){
			...
			items.addAll(Item.createItems(9001, 10, true, 0));
			...
			BackpackManager.getInstance().changeBindGold(player, 90, Reasons.ACTIVITY_GIFT, actionId);
	...
	public String getDescribe(Player player) {
		return "连续9天登录游戏可领取二测绑钻奖励：18000,（若中断，登录天数将重置为1）";
	}

	@Override //红玫瑰（绑）*3、双倍收益丹（绑）*1、绑钻*90、回城卷（绑）*10、  真气丹(绑）*3
	public String getRewardInfo(Player player) {
		return "[{\"id\":-5, \"num\":" + 90 + "}, {\"id\":1015, \"num\":" + 1 + "},{\"id\":1100, \"num\":"+3+"},{\"id\":9001, \"num\":"+10+"},{\"id\":30201, \"num\":"+3+"}]";
	}
