...
//国庆 坐骑升阶
public class GQHORSE7 implements IActivityScript {
	private int itemid1=1023;
	private int itemnum1=1;
	
	private int itemid2=1011;
	private int itemnum2=30;

	private int itemid3=30201;
	private int itemnum3=20;
	
	private static String KEY = "GQHORSE7";
	@Override
	public int getId() {
		return ScriptEnum.GQHORSE7;
	}
	...
	public String getRewardInfo(Player player) {
		//坐骑祝福丹（绑定）*1、坐骑进阶丹（绑定）*30、真气丹（绑定）*20
	...
	public int getState(Player player) {
		if(player.getActivitiesReward().containsKey(KEY)){ //未领过
			...
			if(playerlayer>=7){ //没有活动期间升阶的标记 并且当前坐骑等级大于等于领取条件 表示玩家在活动之前达到目标等级，直接跳过
				return 0;
			}
		}
		return 2;
