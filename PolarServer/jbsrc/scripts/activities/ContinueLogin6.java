...
public class ContinueLogin6 implements IActivityScript{

	private static String KEY = "CONTINUELOGIN6";
	...
	public void getReward(Player player) {
		...
		if(BackpackManager.getInstance().getEmptyGridNum(player)>=3){
			...
			items.addAll(Item.createItems(9001, 7, true, 0));
			items.addAll(Item.createItems(30201, 1, true, 0));
			BackpackManager.getInstance().addItems(player, items, Reasons.ACTIVITY_GIFT, actionId);
			BackpackManager.getInstance().changeBindGold(player, 60, Reasons.ACTIVITY_GIFT, actionId);
	...
	public String getDescribe(Player player) {
		return "连续6天登录游戏可领取二测绑钻奖励：12000,（若中断，登录天数将重置为1）";
	}

	@Override //红玫瑰（绑）*3、双倍收益丹（绑）*1、绑钻*60、回城卷（绑）*7 、  真气丹(绑）*1
	public String getRewardInfo(Player player) {
		return "[{\"id\":-5, \"num\":" + 60 + "}, {\"id\":1015, \"num\":" + 1 + "},{\"id\":1100, \"num\":"+3+"},{\"id\":9001, \"num\":"+7+"},{\"id\":30201, \"num\":"+1+"}]";
	}
