			...
			if (jiaoChangInfo.getCurjifen() > 0) {
				...
				if (q_characterBean != null) {
					...
					if (lijin + player.getBindGold() > Global.BAG_MAX_GOLD) {
						...
						items.add(item);
						/* xuliang
						MailServerManager.getInstance().sendSystemMail(player.getId(), player.getName(), ResManager.getInstance().getString("系统邮件"), String.format(ResManager.getInstance().getString("校场奖励：获得%d绑钻。"), lijin), (byte) 0, 0, items);
						MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM, String.format(ResManager.getInstance().getString("校场奖励绑钻由于您的绑钻已达到上限，转发为系统邮件领取。"), lijin));
						*/
					} else {
						/* xuliang
						BackpackManager.getInstance().changeBindGold(player, lijin, Reasons.JIAOCHANGE_GETBINDGOLD, Config.getId());
						MessageUtil.notify_player(player, Notifys.CHAT_SYSTEM, String.format(ResManager.getInstance().getString("校场奖励：获得%d绑钻。"), lijin));
						*/
					}
				}
				jiaoChangInfo.setCurjifen(0);
