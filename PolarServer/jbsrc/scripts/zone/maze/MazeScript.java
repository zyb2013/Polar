...
public class MazeScript  implements IServerEventTimerScript,ICreateZoneScript ,IEnterTeleporterScript , INpcServiceScript, INpcApplyServicesScript, IEnterMapScript, IZoneEventTimerScript{
	...
	*/
	...
	//优胜奖经验
	private static int YOUSHENG_JINGYAN = 90;
	...
	//参与奖经验
	private static int CANYU_JINGYAN = 30;
...
//		}
		...
		Q_characterBean bean = ManagerPool.dataManager.q_characterContainer.getMap().get(key);
			...
			if (floornum == 19) {	//在20层里，计算玩家完成时间，是否为前3名
				...
				if(sort!=0){
					...
					if(bean!=null){
						...
						/* xuliang
						if(sort==1 || sort==2){
							MessageUtil.notify_All_player(Notifys.SROLL, java.text.MessageFormat.format(ResManager.getInstance().getString("恭喜{0}获得了迷宫活动的第{1}个优胜奖，可获得{2}绑钻，{3}经验，{4}真气大奖"), new Object[] {player.getName(), sort, bindgold, exp, zhenqi}));
						}else if(sort==3){
							MessageUtil.notify_All_player(Notifys.SROLL, java.text.MessageFormat.format(ResManager.getInstance().getString("恭喜{0}获得了迷宫活动的最后1个优胜奖，可获得{1}绑钻，{2}经验，{3}真气大奖"), new Object[] {player.getName(), bindgold, exp, zhenqi}));
						}
						*/
					}
