 ...
 */
@Deprecated
public class BowBossRoomSixScript  implements ICreateZoneScript, IEnterMapScript, IZoneEventTimerScript, IMonsterDieScript{
	//副本结束
	...
	//boss
	private int BOSS = 14006;

	//buff
	private int BUFF = 24006;
	...
	public void action(long zoneId, int zoneModelId, List<Object> parameters) {
		...
		if(hpPercent < 30){			
			...
			while (iter.hasNext()) {
				...
				//提示
				/* xuliang
				MessageUtil.notify_player(player, Notifys.CHAT_BULL, ResManager.getInstance().getString("BOSS施放了歃血补虚，五秒后BOSS进入虚弱状态！"));
				*/
			}
