...
/**
 * 大天使七曜副本一层层脚本，废弃，仅供参考
 * @author hongxiao.z
 * @date   2014-1-11  下午4:29:20
 */
@Deprecated
public class BowBossRoomOneScript  implements ICreateZoneScript, IEnterMapScript, IZoneEventTimerScript, IMonsterDieScript{
	//副本结束
	...
	public ZoneContext onCreate(Player player, int zoneId) {
		if (zoneId == ZONEID) {
			...
			//拉人进副本
			ManagerPool.mapManager.changeMap(player, zone.getConfigs().get(0).getMapId(), zone.getConfigs().get(0).getMapModelId(), 1, getPosition(enter_x, enter_y, config.getMapModelId()), this.getClass().getName() + ".onCreate");
