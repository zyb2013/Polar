 ...
 */
@Deprecated
public class BowBossRoomFourScript  implements ICreateZoneScript, IEnterMapScript, IZoneEventTimerScript, IMonsterDieScript{
	//副本结束
	...
	public void action(long zoneId, int zoneModelId, List<Object> parameters) {
		...
		if(hpPercent < 30){			
			...
			while (iter.hasNext()) {
				...
				//提示
				/* xuliang
				MessageUtil.notify_player(player, Notifys.CHAT_BULL, ResManager.getInstance().getString("BOSS对你施放了半壁红霞，令你血量上限大幅减少！"));
				*/
			}
