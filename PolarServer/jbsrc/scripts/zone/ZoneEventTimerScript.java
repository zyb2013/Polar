...
public class ZoneEventTimerScript implements IZoneEventTimerScript {
	...
	public void action(long zoneId, int zoneModelId, List<Object> parameters) {
		IZoneEventTimerScript script = (IZoneEventTimerScript) ManagerPool.scriptManager.getScript(ZonesConstantConfig.PT_SCRIPT_ID);
		if (script != null) 
		{
			try
			{
		...
		script = (IZoneEventTimerScript) ManagerPool.scriptManager.getScript(jiaochang_scriptId);
		if (script != null) 
		{
			try
			{
		...
		script = (IZoneEventTimerScript) ManagerPool.scriptManager.getScript(ScriptEnum.BF4V4);
		if (script != null) {
			try {
				script.action(zoneId, zoneModelId, parameters);
			} catch (Exception e) {
