...
public class CreateZoneScript implements ICreateZoneScript {
	...
	public static int emgc_scriptId = 55001; 
	
	public static int xscb_scriptId = 55002;
	
	public static int csys_scriptId = 55012;
	
	public static int vip_scriptId = 51001;
	...
	public ZoneContext onCreate(Player player, int zoneId) 
	{
		...
		ICreateZoneScript script = (ICreateZoneScript) ManagerPool.scriptManager.getScript(emgc_scriptId);
		if (script != null) 
		{
			try {
		 ...
		 script = (ICreateZoneScript) ManagerPool.scriptManager.getScript(xscb_scriptId);
		if (script != null) {
			try {
				context = script.onCreate(player, zoneId);
				if(context!=null) return context;
		...
		script = (ICreateZoneScript) ManagerPool.scriptManager.getScript(ZonesConstantConfig.PT_SCRIPT_ID);
		if (script != null) {
			try 
			{
				context = script.onCreate(player, zoneId);
		...
		script = (ICreateZoneScript) ManagerPool.scriptManager.getScript(csys_scriptId);
		if (script != null) {
			try {
				context = script.onCreate(player, zoneId);
				if(context!=null) return context;
		...
		script = (ICreateZoneScript) ManagerPool.scriptManager.getScript(vip_scriptId);
		if (script != null) {
			try {
				context = script.onCreate(player, zoneId);
				if(context!=null) return context;
