				...
				if (hp%10 == 0) {
					...
					if (territory != null) {
						...
						if (oldhp != hp) {
							monster.getParameters().put("hp", hp);
							if (hp > 50) {
								MessageUtil.notify_guild_all(territory.getGuildid() , String.format("本盟安插在【%s】地图的盟旗遭受攻击，剩余%s％血量",name,hp+""));
							}else if (hp > 1 ) {
								MessageUtil.notify_guild_all(territory.getGuildid() , String.format("本盟安插在【%s】地图的盟旗仅剩余%s％血量，帮内弟兄赶紧前往讨伐敌人，守护盟旗！",name,hp+""));
							}
						}
					}
				}
