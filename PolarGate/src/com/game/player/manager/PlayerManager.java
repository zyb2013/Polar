...
public class PlayerManager {
	...
	//panic god public static int MAX_PLAYER = 4000;  //1月20日测试的时候改为5000人
	public static int MAX_PLAYER = 5000;
	...
	public void login(IoSession session, int createServer, String userId, String password){
		...
		try{
			...
			//非局域网环境需要验证登录密码  IP未取到 或者 IP取到了但不是内网IP 暂时不做ip 限制
			/*
			 *  '119.131.244.178', '119.131.244.202', '14.23.168.10', '14.23.168.18'
			 */
			if(ip==null || (!ip.startsWith("10.2.") && !ip.startsWith("127.0.0.1") 
					&& !ip.equals("119.131.244.178") && !ip.equals("119.131.244.202") && !ip.equals("14.23.168.10") && !ip.equals("14.23.168.18"))){
...
//				oldSession.close(true);
			...
			//注册当前玩家
			GateServer.getInstance().registerUser(session, createServer, userId, 1);
			...
			log.error(session + "发送登陆消息,返回角色列表,时间:" + System.currentTimeMillis());
	...
	public void login(IoSession session, ReqLoginForPlatformMessage msg){
		...
		try{
            ...
            // guowenjie: 如果在封禁ip列表中，登录失败
            if(ManagerPool.blacklistManager.contains(ip)) {
                ResLoginFailedMessage fmsg  = new ResLoginFailedMessage();
                fmsg.setReason((byte)7); session.write(fmsg);
                return;
            }
			...
			synchronized (user_states) {
				int state = getUserState(createServer, userId);
				if(state!=0){
					log.error("Login User " + userId + session + " " + getUserStateInfo(state));
					return;
				}
				setUserState(createServer, userId, UserState.LOGINING.getValue());
				log.error("User " + userId + session + " login set state " + getUserStateInfo(UserState.LOGINING.getValue()));
...
//				oldSession.close(false);
	 ...
	 * @param msg 登录消息
	 */
	public void loginPublic(IoSession session, ReqLoginForPulbicMessage msg){
		int createServer = Integer.parseInt(msg.getServerId());
		//判断服务器是否存在
		try{
...
//				oldSession.close(false);
			...
			synchronized (user_states) {
				int state = removeUserState(createServer, userId);
				log.error("User " + userId + session + " remove " + getUserStateInfo(state) + " for select online " + playerId);
			}
...
//	}
	 ...
	 * @param sex
	 */
	public void createCharacter(IoSession session, String name, String icon, byte sex, byte job, byte guest, byte auto){
...
//		}
	...
	public void createFailed(int createServer, String userId, byte reason){
		log.error("create " + userId + " failed reason " + reason);
		synchronized (user_states) {
			int state = removeUserState(createServer, userId);
			log.error("User " + userId + " remove " + getUserStateInfo(state) + " for create failed");
		}
