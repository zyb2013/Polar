...
//	private static boolean connected = false;
	//panic test robot
	//机器人开关
	public static int ROBOT_OPEN = 0;
	public static int Old_MAX_robot = 10000005;
	public static int MAX_robot = 40000001; 
	public static int MAX_robot2 = 40002005;
	public static int name_robot = 1;
	public static int name_robot2 = 2001;
	private boolean old_robotSuccess = true;
	...
	public void run() {
		...
		if (ROBOT_OPEN == 1) {
			// robot测试使用
			new Timer("Server-robot-Timer").schedule(new TimerTask() {
				@Override
				public void run() {
					RoleDao roledao = new RoleDao();
					Role character = null;
					String strUserId = "";
					int server3 = GateServer.getGameConfig()
							.getServerByCountry(1);

					// 创建角色ReqCreateCharacterMessage
					boolean test = true;
					int add1 = 0;
					strUserId = Integer.toString(MAX_robot - 1);
					if (MAX_robot >= 40002005) {
						return;
					}
					while (test) {
						int sessionId = MAX_robot;
						// character = roledao.selectById(MAX_robot);
						int country = GateServer.getGameConfig()
								.getCountryByServer(1);
						int server = GateServer.getGameConfig()
								.getServerByCountry(country);// (Integer)session.getAttribute(SERVER_ID);
						strUserId = Integer.toString(MAX_robot);
						List<Role> listrole = roledao
								.selectByUser(strUserId, 1);
						if (listrole.size() > 0) {
							character = listrole.get(0);
							if (character != null && character.getSex() == 9) {
								TestReqLoadFinishMessage loadmsg = new TestReqLoadFinishMessage();
								List<Long> roleid = new ArrayList<Long>();
								roleid.add(character.getRoleid());
								loadmsg.setRoleId(roleid);
								boolean success2 = MessageUtil.send_to_game(
										server, sessionId, loadmsg);
								// 1次100个
								if (add1 >= 50) {
									break;
								}
							...
							log.error("Server-robot-Timer num= "
									+ (MAX_robot - 40000005));
						}
						test = false;
						// 发送到游戏服务器
						...
						msg.setUserId(strUserId);
						msg.setUserName("MAX_robot_" + MAX_robot);
						msg.setCreateServer(1);
						int ii = MAX_robot / 10000 + name_robot;
						msg.setName("F" + ii);
						msg.setSex((byte) 9);
						int job = random.nextInt(3) + 1;
						msg.setJob((byte) job);
						msg.setAuto((byte) 0);
						msg.setIcon("");
						msg.setIsAdult((byte) 0);
						msg.setIsGuest((byte) 0);
						msg.setAgentColdatas("");
						msg.setAgentPlusdata("");
						String hostAddress = "";
						msg.setOptIp(hostAddress);
//						 boolean success = MessageUtil.send_to_game(server,
//						 sessionId, msg);
						MAX_robot++;
						name_robot++;

					}
					flowlog.error("Server-robot-Timer" + "MAX_robot"
							+ MAX_robot);
				}
			}, 10 * 1000, 20 * 1000);

			// robot2测试使用
			new Timer("Server-robot2-Timer").schedule(new TimerTask() {
				@Override
				public void run() {
					RoleDao roledao = new RoleDao();
					Role character = null;
					...
					boolean test = true;
					int add2 = 0;
					strUserId = Integer.toString(MAX_robot2 - 1);
					if (MAX_robot2 >= 40004005) {
						return;
					}
					while (test) {
						int sessionId = MAX_robot2;
						// character = roledao.selectById(MAX_robot);
						int country = GateServer.getGameConfig()
								.getCountryByServer(1);
						int server = GateServer.getGameConfig()
								.getServerByCountry(country);// (Integer)session.getAttribute(SERVER_ID);
						strUserId = Integer.toString(MAX_robot2);
						List<Role> listrole = roledao
								.selectByUser(strUserId, 1);
						if (listrole.size() > 0) {
							character = listrole.get(0);
							...
							log.error("Server-robot2-Timer num= "
									+ (MAX_robot2 - 40000005));
						}
						test = false;
						// 发送到游戏服务器
						...
						msg.setGateId(GateServer.getInstance().getServerId());
						msg.setUserId(strUserId);
						msg.setUserName("MAX_robot2" + MAX_robot2);
						msg.setCreateServer(1);
						int ii = MAX_robot2 / 10000 + name_robot2;
						msg.setName("F" + ii);
						msg.setSex((byte) 9);
						int job = random.nextInt(3) + 1;
						msg.setJob((byte) job);
...
//						 sessionId, msg);
	...
	public void doCommand(IoSession iosession, IoBuffer buf) {
		...
		try{
			...
			if(id==100201 || id==100209 || id==100208 ){
				log.error(iosession + "收到登陆消息,时间:" + System.currentTimeMillis());
			}
			
			if(id==100206){
				
				//log.error(iosession + "收到登陆成功消息,时间:" + System.currentTimeMillis());
			}
			
			if(id!=100201 && id!=100209 && id!=100208 && !iosession.containsAttribute(PlayerManager.USER_ID)){
...
//			ConcurrentHashMap<String, IoSession> sessions = null;
//			if(user_sessions.containsKey(server)){
//				sessions = user_sessions.get(server);
//			}else{
//				sessions = new ConcurrentHashMap<String, IoSession>();
//				user_sessions.put(server, sessions);
//			}
...
//		if(user_sessions.containsKey(server)){
//			ConcurrentHashMap<String, IoSession> sessions = user_sessions.get(server);
